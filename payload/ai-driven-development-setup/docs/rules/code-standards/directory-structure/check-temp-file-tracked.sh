...
#   直下を許可の一覧で管理する: 止める（許可の一覧に無いディレクトリが直下に新設され履歴に残ると、構成の逸脱が既成読み取り結果として積み重なるため）
#   深い入れ子を作らない: 止める（既定の上限を超える深い階層が一度作られ履歴に残ると、後から浅くし直す作業が構成変更として重くなるため）
#   名前の付け方を階層内で揃える: 止める（表記形式が揃わないディレクトリが一度新設され履歴に残ると、同じ階層内の不揃いが既成読み取り結果として固定されるため）
#   置き場を役割で分ける: 知らせる（宣言は対象プロジェクトのリバース解析が進めば自然に整い、判定できるようになるため）
#   依存の向きを一方向に保つ: 止める（内側の層から外側の層への取り込みが一度実装され履歴に残ると、依存の逆転が既成読み取り結果として固定されるため）
