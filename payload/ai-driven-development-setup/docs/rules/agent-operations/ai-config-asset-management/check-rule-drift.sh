#!/usr/bin/env bash
# 生成物である。直接編集しない（定義: docs/skills/setup-deriving-rules/scripts/check-rule-drift.sh）
...
#   docs/skills/setup-deriving-rules/references/規約定義と派生生成の設計.md の6節。
#
# 対象（丸ごと生成されるファイルだけ）:
...
#   docs/skills/setup-deriving-rules/references/規約定義と派生生成の設計.md の6節を同時に更新する。
#
