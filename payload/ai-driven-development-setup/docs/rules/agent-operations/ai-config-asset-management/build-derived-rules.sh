#!/usr/bin/env bash
# 生成物である。直接編集しない（定義: docs/skills/setup-deriving-rules/scripts/build-derived-rules.sh）
set -euo pipefail

# build-derived-rules.sh — docs/rules/ の規約定義から派生物を生成する
#
# 設計の定義: docs/skills/setup-deriving-rules/references/規約定義と派生生成の設計.md（5節・6節）
#
# 目的:
...
# check-rule-drift.sh・resolve-applicable-rules.sh の4本を、出力先リポジトリの
# docs/rules/agent-operations/ai-config-asset-management/ へ複製する。複製元は
# 4本とも本スクリプト自身のフォルダ（docs/skills/setup-deriving-rules/scripts/）
# であり、リポジトリ内の固定の別ディレクトリを複製元にする形は取らない。
# 規約を扱うスクリプトは対応する規約のフォルダへ置き、スクリプト専用の
# フォルダを作らない（設計の定義: docs/skills/setup-deriving-rules/references/規約定義と派生生成の設計.md
...
#
# 保守責任者・廃棄条件: docs/skills/setup-deriving-rules/SKILL.md の「## 設計判断」
...
# 宣言（先頭が "# timing:" の行）が複数あれば全行を読み、1宣言につき
# "<いつ>\t<ツール>" を1行ずつ標準出力へ返す（丸括弧が無ければ<ツール>は空）。
# 宣言が1つも見つからない場合は何も出力しない。
checker_read_timing() {
  ...
  while IFS= read -r line; do
    ...
    printf '%s\t%s\n' "$when" "$tools"
  done < <(grep '^# timing:' "$checker_file" 2>/dev/null || true)
}
...
"
  ...
  while IFS= read -r f; do
    ...
    # derivedPath が指定されていれば既定の派生先を上書きする（共通の hook が
    # 固定の場所を読む project-context 等）。上書きするのは (1) の生成先だけで、
    # (2) .cursor/rules/*.mdc と (3) AGENTS.md の索引は既定のまま変わらない
    # （設計の定義: docs/skills/setup-deriving-rules/references/規約定義と派生生成の設計.md 4節）。
    local claude_out
    if [ -n "$v_derived_path" ]; then
      claude_out="${out_root}/${v_derived_path}"
    else
      claude_out="${out_root}/.claude/rules/${scope_dir}/${v_parent}/${v_key}/rule.md"
    fi
...
"
    ...
    # timing 宣言が複数行あれば、それぞれの登録先へ登録する。
    if [ "$v_checkable" = "true" ] && [ -n "$v_checker" ] && [ "$v_checker" != "null" ]; then
      ...
      if [ -z "$timing_spec" ]; then
        ...
        local timing_when timing_tools matcher_json
        while IFS=$'\t' read -r timing_when timing_tools; do
          [ -n "$timing_when" ] || continue
          ...
          # matcher_jsonはこのループの1周（1規約定義の1つのtiming宣言）につき1個作られる
          # 単一のhookエントリオブジェクトであり、規約定義の総数が増えても1個あたりの
          # 大きさは変わらない（check-argjson-unbounded-value.shの許可リストに登録済みの固定長の値）。
...
DEPLOY_NOTICE="生成物である。直接編集しない（定義: docs/skills/setup-deriving-rules/scripts/"

# 複製元は本スクリプト自身のフォルダ（SCRIPT_DIR）だけとする。4本（本体・
# validate-rule-definitions.sh・check-rule-drift.sh・resolve-applicable-rules.sh）
# はいずれも docs/skills/setup-deriving-rules/scripts/ に同居しており、
# リポジトリ内の固定の別ディレクトリ（旧: delivery-payload/templates/rules/checkers/）
# を複製元にする形は取らない。パス解決を引数と規約フォルダに限る方針に合わせる。
deploy_rule_scripts() {
  local out_root="$1"
  ...
  local src name dest
  for name in build-derived-rules.sh validate-rule-definitions.sh check-rule-drift.sh resolve-applicable-rules.sh; do
    src="${SCRIPT_DIR}/${name}"
    dest="${scripts_dir}/${name}"
    if [ ! -f "$src" ]; then
      echo "配備失敗: ${name} の複製元が見つからない（${src}）" >&2
      return 1
    fi
    {
...
EOF
  ...
  # multi-timing: timing宣言を複数行(PreToolUse(Write|Edit|MultiEdit)とStop)持つ検査。
  # 1つの検査が両方の登録先へ登録されることを確認する材料。
  cat > "${root}/code-standards/multi-timing/rule.md" <<'EOF'
---
key: multi-timing
title: 複数timing宣言検査規約（テスト用）
parent: code-standards
summary: timing宣言が複数行ある検査が両方の登録先へ登録されることを確認するための規約。
scope: scoped
paths: ["**/*"]
enforcement: advisory
checkable: true
checker: check-multi-timing.sh
uncheckableReason: null
formatter: none
status: approved
origin: proposal
...
1. 例
EOF
  cat > "${root}/code-standards/multi-timing/check-multi-timing.sh" <<'EOF'
#!/usr/bin/env bash
# timing: PreToolUse(Write|Edit|MultiEdit)
# timing: Stop
exit 0
EOF
  cat > "${root}/code-standards/multi-timing/check-multi-timing.test.sh" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
  chmod +x "${root}/code-standards/multi-timing/check-multi-timing.sh" "${root}/code-standards/multi-timing/check-multi-timing.test.sh"
...
self_test() {
  ...
  # ケース18: timing宣言を複数行(PreToolUse(Write|Edit|MultiEdit)とStop)持つ検査は、
  # 両方の登録先へ登録される（cursor/codexにはPreToolUse側のWrite対象分だけ登録される）
  ok18=1
  if [ -f "${out1}/.claude/settings.json" ]; then
    jq -e '.hooks.PreToolUse[]? | select(.hooks[0].command == "docs/rules/code-standards/multi-timing/check-multi-timing.sh") | .matcher == "Write|Edit|MultiEdit"' "${out1}/.claude/settings.json" >/dev/null 2>&1 || ok18=0
    jq -e '.hooks.Stop[]? | select(.hooks[0].command == "docs/rules/code-standards/multi-timing/check-multi-timing.sh")' "${out1}/.claude/settings.json" >/dev/null 2>&1 || ok18=0
  else
    ok18=0
  fi
  if [ -f "${out1}/.cursor/hooks.json" ]; then
    grep -q 'check-multi-timing.sh' "${out1}/.cursor/hooks.json" 2>/dev/null || ok18=0
  else
    ok18=0
  fi
  if [ "$ok18" -eq 1 ]; then
    echo "  [PASS] ケース18: timing宣言が複数行ある検査は両方の登録先(PreToolUse・Stop)へ登録される"
  else
    echo "  [FAIL] ケース18: 複数timing宣言の検査が両方の登録先へ登録されなかった" >&2
    rc=1
  fi
  ...
  # ケース20: derivedPath が指定された規約は、既定の派生先ではなく derivedPath の
  # 場所へ .claude/rules/rule.md が生成される。.cursor/rules/*.mdc と AGENTS.md の
  # 索引は既定のまま変わらない。
  local src20 out20 ok20=1
  if ! src20="$(mktemp -d "${TMPDIR:-/tmp}/build-derived-rules-case20-src.XXXXXX" 2>/dev/null)" || [ -z "$src20" ]; then
    echo "[UNKNOWN] 一時ディレクトリの作成に失敗したため判定できません（mktempが一時領域へ書き込めませんでした。実行環境の制約が原因である可能性があります）" >&2
    exit 2
  fi
  if ! out20="$(mktemp -d "${TMPDIR:-/tmp}/build-derived-rules-case20-out.XXXXXX" 2>/dev/null)" || [ -z "$out20" ]; then
    echo "[UNKNOWN] 一時ディレクトリの作成に失敗したため判定できません（mktempが一時領域へ書き込めませんでした。実行環境の制約が原因である可能性があります）" >&2
    exit 2
  fi
  bst_write_fixture "$src20"
  awk '{print} /^workUnit: file$/ && !d {print "derivedPath: .claude/rules/always/project-context/rule.md"; d=1}' \
    "${src20}/agent-operations/ai-behavior/rule.md" > "${src20}/agent-operations/ai-behavior/rule.md.new"
  mv "${src20}/agent-operations/ai-behavior/rule.md.new" "${src20}/agent-operations/ai-behavior/rule.md"
  APPLY=1
  run_build "$src20" "$out20" >/dev/null 2>&1
  if [ -e "${out20}/.claude/rules/always/project-context/rule.md" ] \
    && [ ! -e "${out20}/.claude/rules/always/agent-operations/ai-behavior/rule.md" ] \
    && [ -e "${out20}/.cursor/rules/agent-operations-ai-behavior.mdc" ] \
    && grep -q 'docs/rules/agent-operations/ai-behavior/rule.md' "${out20}/AGENTS.md" 2>/dev/null; then
    echo "  [PASS] ケース20: derivedPath が指定された規約は既定の派生先ではなくderivedPathの場所へ生成され、.cursor/rules・AGENTS.mdは既定のまま"
  else
    echo "  [FAIL] ケース20: derivedPath による派生先の上書きが不正" >&2
    rc=1
  fi
  rm -rf "$src20" "$out20"
