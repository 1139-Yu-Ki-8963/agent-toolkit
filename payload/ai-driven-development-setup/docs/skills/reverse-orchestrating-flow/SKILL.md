...
4. 共通処理の詳細設計書を書く機能を呼ぶ前に、門として共通設計文書6つの合格の記録が現在の文書と一致することを確かめる。
   ...
   実行するコマンドは次のとおり（要確認の観点を持つ記録は`--run`が無いと要確認-判定不能で不合格になる。2026-09-06改善）。

    ```
   ../reverse-shared/scripts/check-acceptance-record.sh <対象リポジトリのルート> --design-root <設計書のルート> --common --run <実行フォルダ>
    ```
...
**保守責任者**: 人手（ユーザー）。実行フォルダの作り方・実行順の計算のロジックを直す場合は`reverse-shared/scripts/`の該当スクリプトを直す。直した後、本機能とセットアップの統括の両方でself-testを確かめる。
