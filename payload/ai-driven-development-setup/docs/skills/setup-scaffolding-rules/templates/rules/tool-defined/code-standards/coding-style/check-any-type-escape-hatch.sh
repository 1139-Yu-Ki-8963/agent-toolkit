...
#   - 整形は整形ツールへ任せる: 整形の設定ファイルが実在し、統合の検査に整形の
#     確認が登録されているかを走査する
#
# 判定:
...
#   中身に整形ツールに関する語（prettier / editorconfig / biome / format）が
...
#   - 整形は整形ツールへ任せる: cwd が空・存在しない → 対象外（fail-open）。整形の
...
#   - 整形は整形ツールへ任せる: 整形の設定ファイル名に一致する最初の1件のみを見る。
#     統合の検査への登録は、既知のファイル名（package.json / *.yml / *.yaml /
#     Makefile）の中身に整形ツールの語があるかという近似判定であり、実際に
...
#   整形は整形ツールへ任せる: 止める（設定ファイルだけ用意して統合の検査に登録しない状態が確定すると、整形の不統一が履歴に積み重なり事後に揃え直すコストが増すため）
#
# 逃げ道:
...
check_formatter_registration() {
  ...
  echo "拒否[整形は整形ツールへ任せる]: 整形の設定ファイル（${cfg#"$cwd"/}）は実在するが、統合の検査への登録が見当たりません"
  return 0
}

judge() {
  # $1: file_path, $2: content, $3: cwd（省略可。省略時は「整形は整形ツールへ任せる」を対象外として扱う）
...
${body_lines}}")"; then code=0; else code=$?; fi
  ...
  rm -rf "$tmp15"
  if [ "$code" -eq 2 ] && printf '%s' "$msg" | grep -q '整形は整形ツールへ任せる'; then
