...
日本語名: 共通処理の詳細設計書を書く
description: "調査と検出条件の定義書の共通方式の場所とその実装を読み、共通処理の詳細設計書を方式ごとに書き、未記入・file:lineを機械検査する。工程2-7。"
...
工程2-6（基本設計の完了判定）で全単位と共通設計文書に合格の記録が付いた後、単位ごとの詳細設計より先に使う。基盤層と共通部品の詳細設計書を書く。
...
- 対象リポジトリの `ai-work/records/basic-design-acceptance/common-*.json` に、共通設計文書の合格の記録があること。確認は手順1のコマンドで行う（要確認の観点を持つ記録は`--run`が無いと要確認-判定不能で不合格になる。2026-09-06改善）
- 共通処理の詳細設計書は対象リポジトリの `docs/design/common/` に直接書く。実行フォルダ（統括の実行の開始スクリプトが作る）は本文の書き込み先としては使わないが、合格の記録の確認（要確認のキー照合）のために受け取る

## 手順

1. 次を実行し、終了コードが0であることを確かめる。0でなければ止まり、工程2-6（基本設計の完了判定）へ差し戻す
    ```bash
    bash ../reverse-shared/scripts/check-acceptance-record.sh <対象リポジトリのルート> --common --run <実行フォルダ>
    ```
...
4. 各節について、調査と検出条件の定義書の節7が指す実装（共通方式の実装）を読み、6つの小節（クラス設計・メソッド設計・ロジック設計・戻り値と引数・エラー処理・データ定義）を埋める
5. 本文に実装の位置（file:line）を書かない。仕様か現行実装かを決められない記述は、既定を付けて要確認事項一覧に書き、確認事項に登録する
6. 検査する
   ```bash
...
**必要性**: 共通処理の詳細設計書は調査と検出条件の定義書の節7に対応する節をすべて持たなければならない。方式の数と場所は対象ごとに違うため、機械で調査と検出条件の定義書を読み、節の欠落・小節の欠落・未記入・file:lineを一括で確かめる必要がある。
