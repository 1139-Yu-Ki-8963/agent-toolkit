...
10. 完了時の処理として、単位ごとに文書のレビュー担当（AI）が基本設計書（出力するときは単体テスト設計書も）を読む。観点は `references/basic-phase-viewpoints.md` の6観点である。合否を判定し、合格・不合格のいずれかを次で記録する。
    ...
    ```
    不合格は、否になった観点が名指しする項目を読み取り結果とコードを再度読んで書き出し、4 へ戻る。指摘がある限り何度でも戻る。単位を保留にしない。決められない項目は既定を置いて確認事項に登録し、要確認として合格へ進める。各観点の値は合・否・要確認のいずれかとする。要確認の理由には確認事項一覧のキーを添える。キーが確認事項の記録に無いと終了コード2で記録を作らないため、先に確認事項へ登録する。判定の直前に`shasum -a 256`で基本設計書・単体テスト設計書の値を取り、`--judged`へ渡す
