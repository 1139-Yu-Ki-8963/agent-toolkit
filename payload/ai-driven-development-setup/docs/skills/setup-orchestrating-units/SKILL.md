...
0. 前提の確認。AskUserQuestionで次の6項目を確かめる。
   ...
   答えを`scripts/start-run.sh`へ渡し、実行フォルダを作る。渡す引数は次のとおりである。

   ```
   <作業場所> --project-name <対象プロジェクトの名前> --output-root <作業場所> --units <単位のカンマ区切り> --scope <出力の範囲> --tests <テスト設計書の出力>
   ```

   設計書の展開先が「対象プロジェクトのリポジトリへ展開する」なら`--deploy-to <対象プロジェクトのリポジトリのルート>`を足す。標準出力に出た実行フォルダのパスを渡す。渡す先は、以降の手順とSTEPで呼ぶ機能である。
