...
# 定義を再実装しない）。単位の記録はreverse-shared/scripts/units-status.shの
# 完了判定も更新する（無ければこの更新だけ省く）。
#
# 終了コード:
#   0 = 記録を書いた
...
UNIT_DIR_NAME_SH="${SCRIPT_DIR}/../../reverse-shared/scripts/unit-dir-name.sh"
...
record_unit() {
  ...
  if [ "$kind" = "feature" ]; then
    ...
    docs_json="$(printf '%s\n%s\n' \
      "$(doc_sha_json "${unit_path}/基本設計書.md" "基本設計書.md")" \
      "$(doc_sha_json "${unit_path}/単体テスト設計書.md" "単体テスト設計書.md")" \
      | jq -s 'add')"
  fi
...
self_test() {
  ...
  sha1="$(jq -r '.["文書"]["基本設計書.md"]' "$record_file" 2>/dev/null)"
  sha_expected1="$(shasum -a 256 "$d/docs/design/screens/src_pages_OrderList.tsx/基本設計書.md" | awk '{print $1}')"
  check "単位の記録: 基本設計書のsha256が一致" "$([ "$sha1" = "$sha_expected1" ] && echo 0 || echo 1)"
  ...
  local status1
  status1="$(bash "${SCRIPT_DIR}/../../reverse-shared/scripts/units-status.sh" "$run" get screen "src/pages/OrderList.tsx" 完了判定 2>/dev/null)"
