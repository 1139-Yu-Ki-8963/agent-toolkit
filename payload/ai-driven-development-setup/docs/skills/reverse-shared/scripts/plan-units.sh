...
# plan-units.sh — 機能の宣言（unit・inputs・outputs・requires）から実行順の計画を組み立てる
# （setup単位・reverse単位で共有する部品）
#
# 目的:
#   docs/skills/<機能名>/SKILL.md の front matter を読み、機能名を直書きせずに
#   実行順を導く。setup-orchestrating-units の scripts/plan-setup.sh と
#   reverse-orchestrating-flow の scripts/plan-reverse.sh は、それぞれ
#   --units を固定した入口として本スクリプトを呼ぶ。判定方法の詳細
#   （重なりの近似・既知の限界）は
#   docs/skills/setup-orchestrating-units/references/plan-setup.md を参照する。
#
# 使い方:
#   plan-units.sh <リポジトリのルート> [--units a,b,c] [--target <対象リポジトリのルート>] [--format table|steps|both] [--until <機能名>]
#   plan-units.sh --self-test
#
# 機能の定義の置き場:
...
#   docs/skills/reverse-orchestrating-flow/SKILL.md と
...
self_test() {
  local pass=0 fail=0
  local root
  if ! root="$(mktemp -d "${TMPDIR:-/tmp}/plan-units-self-test.XXXXXX" 2>/dev/null)" || [ -z "$root" ]; then
  ...
  if ! root7="$(mktemp -d "${TMPDIR:-/tmp}/plan-units-self-test-fallback.XXXXXX" 2>/dev/null)" || [ -z "$root7" ]; then
    echo "[UNKNOWN] 一時ディレクトリの作成に失敗したため判定できません（mktempが一時領域へ書き込めませんでした。実行環境の制約が原因である可能性があります）" >&2
    exit 2
  fi
  repo7="${root7}/repo"
