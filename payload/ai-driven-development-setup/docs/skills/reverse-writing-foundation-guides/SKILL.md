...
5. 道標の節 1「調査」（特に依存の定義・ビルド・起動・テストの定義・環境の値の定義・テストの枠組みと場所・実行環境の前提）と基盤設計書、および依存の定義ファイル（package.json 等）を読む。2 文書の各行・各節を埋める。技術スタックの「名前」は依存の定義ファイルに実在する名前をそのまま書く
