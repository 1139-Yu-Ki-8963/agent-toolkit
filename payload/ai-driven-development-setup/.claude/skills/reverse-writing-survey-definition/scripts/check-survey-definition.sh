...
# check-survey-definition.sh — 調査と検出条件の定義書（reverse-writing-survey-definition の成果物）の構造と実在を検査する
#
# 目的:
#   調査と検出条件の定義書.md は AI が読んで書くため文面は毎回変わるが、第二フェーズが機械で読む
...
#   取り出し-欠落    4.1〜4.7の読み取り結果の項目の表の元が埋まっている
...
check_map() {
  ...
  for n in 1 2 3 4 5 6 7; do
    ...
    local readings="${sub_readings[$((n-1))]}"
    ...
    if [ "$mark_value" != "AI の読み取り" ] && [ "$mark_value" != "対象外" ]; then
      ...
      reading_tbl="$(awk '
        ...
        seen {print}
      ' <<< "$subtext")"
      reading_tbl="$(first_table_text "$reading_tbl")"
      local reading_rows
      reading_rows="$(table_data_rows "$reading_tbl")"
      local -a reading_arr
      IFS=',' read -ra reading_arr <<< "$readings"
      local ritem
      for ritem in "${reading_arr[@]}"; do
        local rmatched=""
        while IFS= read -r r; do
          [ -z "$r" ] && continue
          if [ "$(col "$r" 1)" = "$ritem" ]; then rmatched="$r"; break; fi
        done <<< "$reading_rows"
        if [ -z "$rmatched" ]; then
          fail "取り出し-欠落" "${heading}: 「${ritem}」の行がありません"
          ok4=0
          continue
        fi
        local fv
        fv="$(col "$rmatched" 2)"
        if [ -z "$fv" ] || [ "${fv:0:1}" = "<" ]; then
          fail "取り出し-欠落" "${heading}: 「${ritem}」の元が未記入です"
          ok4=0
        fi
      done
    fi
...
${extract_report:+| 読み取り結果の項目 | どの構文・記述から取るか |
|---|---|
${extract_report}
}
### 4.6 外部連携
