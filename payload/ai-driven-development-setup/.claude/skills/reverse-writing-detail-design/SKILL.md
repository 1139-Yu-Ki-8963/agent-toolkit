...
日本語名: 詳細設計書を書く
description: "読み取り結果・基本設計書・共通処理の詳細設計書を読み、コードと1対1の詳細設計書（表はテーブル定義書）を単位ごとに書き、読み取り結果の網羅と基本設計書との整合を機械検査する。工程2-8。"
...
8. 検査する
   ...
   ```
9. 不合格の項目を手順6からやり直す（工程内-やり直し）。不合格の理由が名指しする項目を、読み取り結果と基本設計書とコードを再度読んで書き出す。理由に無い箇所は変えない。欠けが無くなるまで回数の上限なく戻る
...
節の番号は各様式の見出しに付く§番号を指す。値は読み取り結果の値をそのまま転記し、コードの名前だけの記述にしない。
...
| table | 列 | §4 入出力の値（名前の列） |
| table | 型 | §4 入出力の値（型の列） |
| table | 制約 | §4 入出力の値（空値の許容・初期値・有効な範囲の各列） |
| table | 関係 | §1 構成要素（所在の列に参照先を書く） |
...
**必要性**: 詳細設計書はコードと1対1でなければならず、読み取り結果の網羅・対応するファイルの一致・基本設計書との整合という3つの照合は、単位数が対象ごとに違う中で人手で数えると見落とす。
...
**代替案を採用しなかった理由**:
- 読み取り結果の網羅を自己レビューだけで確かめる: 単位数・項目数が対象ごとに違うため、見落としが工程2-9（テスト設計）以降まで気付かれない
