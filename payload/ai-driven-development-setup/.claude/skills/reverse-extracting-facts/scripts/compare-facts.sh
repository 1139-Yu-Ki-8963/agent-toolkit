...
# compare-facts.sh — 2つの facts の親を、単位ごと項目ごとに比べる
#
# 目的:
#   同じコードから2回取り出した事実が一致することを確かめる（工程2-4の
#   完了条件）。extract-facts.sh の --verify から呼ばれるほか、単独でも
#   2つの取り出し結果を比べるのに使える。
#
# 使い方:
#   compare-facts.sh <facts の親A> <facts の親B> [--kind <種別>]
#   compare-facts.sh --self-test
#
# <facts の親> は <種別>/<単位のフォルダ名>.json を持つフォルダ（通常は
#   extract-facts.sh の --out）。--kind を省略すると両方の親に実在する
#   種別フォルダをすべて比べる。集計.json は比較の対象外。
#
# 比較の観点:
#   単位ごとに「事実」の各項目の「値」の集合（順不同）が一致するか。
...
#   1 = 差分1件以上（不一致）
#
# 保守責任者: 人手（ユーザー）。事実ファイルの形（事実.<項目>.値）を変える
#   ときは、extract-facts.sh と本スクリプトと自己テストを同時に直す。
#
...
usage_error() {
  echo "使い方: compare-facts.sh <facts の親A> <facts の親B> [--kind <種別>]" >&2
  echo "        compare-facts.sh --self-test" >&2
  exit 2
}
...
compare_facts() {
  ...
  for k in $kinds; do
    ...
    for f in $files; do
      ...
      local items
      items="$( { jq -r '.["事実"] | keys[]' "$fa" 2>/dev/null; jq -r '.["事実"] | keys[]' "$fb" 2>/dev/null; } | sort -u)"

      local item
      for item in $items; do
        local va vb
        va="$(jq -c --arg i "$item" '(.["事実"][$i]["値"] // []) | sort' "$fa" 2>/dev/null)"
        vb="$(jq -c --arg i "$item" '(.["事実"][$i]["値"] // []) | sort' "$fb" 2>/dev/null)"
        if [ "$va" != "$vb" ]; then
...
run_self_test() {
  local tmp
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/compare-facts-self-test.XXXXXX")" || { echo "一時領域を作成できません" >&2; return 2; }
...
{"種別":"screen","識別子":"src/pages/OrderList.tsx","名前":"OrderList","場所":"src/pages/OrderList.tsx","属するファイル":[],"事実":{"入力項目":{"値":["orderId","name"],"出所":"機械","根拠":["src/pages/OrderList.tsx"]}},"未":[],"取り出した実行":"run-1"}
FIXEOF
...
{"種別":"screen","識別子":"src/pages/OrderList.tsx","名前":"OrderList","場所":"src/pages/OrderList.tsx","属するファイル":[],"事実":{"入力項目":{"値":["name","orderId"],"出所":"機械","根拠":["src/pages/OrderList.tsx"]}},"未":[],"取り出した実行":"run-2"}
FIXEOF
  local rc_order
  ...
  # 値が違えば不一致
  cat > "${b}/screen/OrderList.tsx.json" <<'FIXEOF'
{"種別":"screen","識別子":"src/pages/OrderList.tsx","名前":"OrderList","場所":"src/pages/OrderList.tsx","属するファイル":[],"事実":{"入力項目":{"値":["orderId"],"出所":"機械","根拠":["src/pages/OrderList.tsx"]}},"未":[],"取り出した実行":"run-3"}
FIXEOF
...
{"種別":"api","識別子":"/orders","名前":"orders","場所":"src/api/orders.ts","属するファイル":[],"事実":{},"未":[],"取り出した実行":"run-1"}
FIXEOF
