...
10. 完了時の処理として、単位ごとに文書のレビュー担当（AI）が基本設計書（出力するときは単体テスト設計書も）を読む。観点は `references/basic-phase-viewpoints.md` の6観点である。合否を判定し、合格・不合格のいずれかを次で記録する。
    ```bash
    bash ../reverse-shared/scripts/record-acceptance.sh <対象> --run <実行フォルダ> \
      --kind <種別> --unit <識別子> --verdict <合格|不合格> --viewpoints "<観点=合|否|要確認;...>" \
      --judged "<文書名>=<sha256>;..." [--reason "<理由>"] --design-root <設計書の置き場>
    ```
    不合格は、否になった観点が名指しする項目を読み取り結果とコードを再度読んで書き出し、4 へ戻る。指摘がある限り何度でも戻る。単位を保留にしない。決められない項目は既定を置いて確認事項に登録し、要確認として合格へ進める。各観点の値は合・否・要確認のいずれかとする。要確認の理由には確認事項一覧のキーを添える。キーが確認事項の記録に無いと終了コード2で記録を作らないため、先に確認事項へ登録する。判定の直前に`shasum -a 256`で文書の値を取り`--judged`へ渡す。テスト設計書の出力が「出力する」のときは基本設計書・単体テスト設計書の2件、「出力しない」のときは基本設計書の1件を渡す
11. 全種別が終わったら、単位数・合格数・要確認事項の件数を報告する
...
本節がひな形の冒頭案内・本文の文体の唯一の定義である。14件のひな形はこの節を参照し、内容を複製しない。

生成する本文の自由記述はである調で書く。読み手への語りかけ（です・ます）を混ぜない。

冒頭案内は読み手が各節で何を見て何をするかを示す。複数節では「節｜内容｜読み手へのお願い」の3列表を置き、本文の節ごとに1行を対応づける。

作る側の判断の理由・保管方法・作業の経緯・文書作成の方針・件数の内訳は書かない。

様式は自分の種別以外の種別の名前を定型文の中に書かない。単位をまたぐ概念は「設計単位」の語で書く。この定めは配布用に写した様式にも及ぶ（第1回改善指示書1-20・再検証）。
...
- 次を実行し、終了コードが0であることを確かめる（要確認の観点を持つ記録は`--run`が無いと要確認-判定不能で不合格になる。2026-09-06改善）
  ```bash
  check-acceptance-record.sh <対象> --kind <種別> --unit <識別子> --design-root <置き場> --run <実行フォルダ>
  ```
...
**必要性**: 基本設計書は AI が読み取り結果を写して書くため文面は毎回変わる。完了条件（節の順序・未記入の不在・読み取り結果の転記・確認事項の登録）は機械で確かめられる形を持つ。文面を問わず形と実在だけを検査することで、詳細設計へ進んでよいかを判定できる。
