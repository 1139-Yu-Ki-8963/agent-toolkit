...
日本語名: 基本設計書を書く
description: "読み取り結果を業務の言葉へ写し、基本設計書を書く。テスト設計書の出力時は単体テスト設計書も書く。"
...
読み取り結果の取り出しと要件定義書の裏付けが終わった後、種別ごとに使う。読み取り結果を業務の言葉に写し、基本設計書を書く。テスト設計書の出力が「出力する」のときは単体テスト設計書も対で書く。
...
10. 完了時の処理として、単位ごとに文書のレビュー担当（AI）が `references/basic-phase-viewpoints.md` の6観点で基本設計書（出力するときは単体テスト設計書も）を読む。合否を判定し、合格・不合格・保留のいずれかを次で記録する
    ```bash
    ...
    ```
    不合格は、否になった観点が名指しする項目を読み取り結果とコードを再度読んで書き出し、4 へ戻る。指摘がある限り何度でも戻る。保留はコードを読み切っても既定を置けない不明点を持つ単位だけにし、理由を確認事項に登録する
