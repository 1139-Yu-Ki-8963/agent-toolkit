...
#   規約-不在            対象に設計書の書き方・単体テスト設計書の検査スクリプトが無い
...
run_main() {
  ...
  local heading_script="${target}/docs/rules/documentation-standards/document-writing/check-doc-heading-addendum.sh"
  local unittest_script="${target}/docs/rules/quality-assurance/test-policy/check-unit-test-design-doc-sections.sh"
  if [ ! -f "$heading_script" ] || [ ! -f "$unittest_script" ]; then
    echo "[FAIL] 規約-不在: 対象に設計書の検査スクリプトが配置されていません" >&2
    exit 2
  fi
...
self_test() {
  ...
  make_fixture() {
    ...
    mkdir -p "$d/docs/design/lists" \
             ...
             "$d/docs/rules/documentation-standards/document-writing" \
             "$d/docs/rules/quality-assurance/test-policy"
...
FIXEOF
    ...
    # 対象の書き方の検査は最小の代役を置く（追記・補足で始まる見出しがあれば
    # 拒否・無ければ許可の最小実装）
    cat > "$d/docs/rules/documentation-standards/document-writing/check-doc-heading-addendum.sh" <<'HOOKEOF'
#!/usr/bin/env bash
set -u
input="$(cat)"
content="$(printf '%s' "$input" | jq -r '.tool_input.content // empty')"
if printf '%s' "$content" | grep -qE '^#{1,6}[[:space:]]+(追記|補足|その他)'; then
  exit 2
fi
exit 0
HOOKEOF
    chmod +x "$d/docs/rules/documentation-standards/document-writing/check-doc-heading-addendum.sh"

    # 対象の単体テスト設計書の決まりは、必須見出しの実在有無だけを見る代役
    cat > "$d/docs/rules/quality-assurance/test-policy/check-unit-test-design-doc-sections.sh" <<'HOOKEOF2'
#!/usr/bin/env bash
set -u
if [ "${1:-}" = "--check-file" ]; then
  file="${2:-}"
  [ -f "$file" ] || exit 2
  for h in "§1 テスト観点" "§2 テストケース一覧" "§5 異常系" "§6 境界値"; do
    grep -qF "## ${h}" "$file" || exit 2
  done
  exit 0
fi
exit 0
HOOKEOF2
    chmod +x "$d/docs/rules/quality-assurance/test-policy/check-unit-test-design-doc-sections.sh"
  }
...
CONFEOF
  ...
  run_full_cases() {
    ...
    make_fixture "$d6"
    rm -f "$d6/docs/rules/documentation-standards/document-writing/check-doc-heading-addendum.sh"
    mkdir -p "$run6"
