...
**2026-09-06の変更**: 動詞の一覧の区切りを `tr` で組み立てていた。バイト単位の `tr`（GNU）では多バイトの中黒が壊れ、一覧の全動詞が不一致になる不具合が実行側で見つかった（第1回改善指示書 1-26）。区切りは 1 バイトの制御文字（改行）のまま照合し、多バイト文字を区切りに使わない。規約の置き場にある検査の自己テストは `setup-deriving-skills` の `tests/` から呼び、機能の自己テストの走行に含める。一覧の行末の CR（Windows の改行）は読み込み時に除き、CRLF で保存された規約でも同じ判定にする。
