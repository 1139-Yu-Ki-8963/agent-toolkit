...
① リバース側のポート設定ファイル群を退避 ② オリジナル → リバースへミラーコピー（削除同期あり・除外は Phase 3 準拠・`*.lock` は転写対象（除外しない）。git 操作ではなく**ファイル転写**で行い、両ブランチの歴史を混ぜない） ③ 当該 `<scope>` のスロットから算出したリバース側ポートでポート設定を再生成 ④ `node_modules_strategy` 分岐で依存を再構築（`npm_ci`: `install_command`（既定 npm ci）で各環境に独立インストール／`symlink_main`: source_repo の node_modules へ symlink を張る＝drvfs での npm ci 低速回避。独立性の証明対象からは除外＝§7-2 参照。失敗 = lock 不整合は ERROR） ⑤ `bundler_cache_dirs` のキャッシュを削除。
...
- 動的比較の実行手段は `playwright_exec.mode` 分岐（`mcp`: Playwright MCP／`node-script`: config.yml の `command_template` を Bash 直実行）。navigate 先の URL は必ず `127.0.0.1` を使う（`localhost` だと WSL2 の Chromium が名前解決待ちでタイムアウトする）
...
- WSL2 では worktree を Linux ネイティブ FS（~/ 配下）に置く。/mnt/* 配下（drvfs）は既定で ERROR。ただし config.yml / projects の `allow_mnt_fs: true` で ERROR ではなく性能 WARN に緩和できる（drvfs 上でも動くが低速）。詳細は guide §5
