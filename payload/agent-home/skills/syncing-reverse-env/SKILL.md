...
description: |
  リバース元/設計書由来2環境同期・検証・基準コミット確立。
  TRIGGER when: setup/sync/teardown・reverse-baseline操作。
  SKIP: 設計書修正等（→rebuilding-code-from-docs）。
...
| リバースコード環境 | worktree 名 `reverse-code-<system>`、ブランチ `feature/reverse-code-<system>`。容器は本スキルが用意し、中身は rebuilding-code-from-docs スキルが書く |
