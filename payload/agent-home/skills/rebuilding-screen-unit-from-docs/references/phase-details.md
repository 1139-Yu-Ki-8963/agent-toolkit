...
5. 実測が要る断定（初期表示値・DOM 順・位置・レイアウト等）は stage1 で確定せず `[stage2で実測]` プレースホルダに残す。stage1 で確定するのは静的に決定可能な宣言的契約（型・import・export・Props 名・API 契約形・ロジック構造・定数値）に限る
...
5. `measure-file-diff.sh` の verdict は生シグナルである。差分のうち `[stage2で実測]` 委譲事実（初期表示値・DOM 順・位置・レイアウト起因）に該当する行は stage2 委譲として分類し収束判定から除外し、静的契約差分（import・型・export・Props 名・定数・ロジック）のみで PASS/FAIL を判定する
...
**stage2 委譲差分の扱い**: `[stage2で実測]` に該当する実測断定差分（初期表示値・DOM 順・位置・レイアウト）は確定修正せず、`[stage2で実測]` プレースホルダのまま P7 へ引き継ぐ。収束判定の分母からも除外する。
...
5. `~/agent-home/skills/rebuilding-code-from-docs/scripts/audit-consistency.sh <画面ディレクトリ>` を実行し、移行後の構造的健全性（章マップ・キー突合・プレースホルダ）を確認する
