...
description: "設計書だけからコード再生成し元と突合、欠落発見。 TRIGGER when: 往復品質検証。 SKIP: 同期/実装（stage2）。"
...
- 起動引数は `mode`（`implement` | `judge`）と、mode ごとの args 全量（後述の「起動引数」参照）
  ...
  - `mode=judge`: 起動引数 `compare_result`（管理者が比較〔dry-run〕を実行して取得した結果ブロック）を受け取り、NG 分類 → 修正指示書 → 最終報告までを実行する
...
`mode=implement` は args として `screen_dir`（画面ディレクトリパス。例: `<docs_root>/画面/screen-<画面ID>/詳細設計`）・`scope`・`reverse_worktree`・`ports`・`baseline_tag_status`・`docs_root`・`template_root`・`audit_script_path`・`chapter_map_path`・`user-approved` を受け取る。これらは管理者が事前解決して渡す値であり、本スキル自身は取得・導出しない。`docs_root` が欠落している場合は Phase 1 で `status=BLOCKED` として管理者へ差し戻す。
...
完了条件: 起動引数 `compare_result` を受領し評価している
...
`scripts/check-freeze.sh` で Phase 6 の凍結コミットハッシュと現状（HEAD・作業ツリー）を突合する。不一致なら結果全体を無効宣言する。全証跡・`[未実行]` 0 件・NG 分類を含む最終報告を `references/report-format.md` の書式で作成し、`status=PASS | FAIL | DESIGN-INCOMPLETE | DYNAMIC-UNVERIFIED` を返す。**基準タグ更新は本スキルでは実行しない**。最終報告には「PASS 時の基準タグ更新は管理者が比較エンジンの本番実行（非 dry-run）で行う」旨を明記する。
...
| Phase 6 | Phase 4 STYLE-GATE と Phase 5 全ゲートが PASS・凍結コミットハッシュが確定・`status=NEED-COMPARE` と `compare_request` を返却（`mode=implement` の終了点） |
| Phase 7 | 起動引数 `compare_result` を受領済み（`mode=judge` の入口）・L3 画素比較+L2 ARIA 一致が「達成」判定の必須条件 |
| Phase 8 | 修正指示書または「NG なし」の明示が完了 |
| Phase 9 | 最終報告作成済み（定量欄: トークン適用率・型エラー数・L3 画素差分値を含む）・凍結検証の結果が明示済み |
| **Goal** | 全 Phase 完了（`mode=implement` は Phase 1〜6、`mode=judge` は Phase 7〜9）・`[未実行]` 0 件・凍結検証 PASS・修正指示書または PASS 報告が検証記録に保存済み |
...
- `~/agent-home/skills/orchestrating-reverse-docs-flow/references/contract.md` — 完全仲介方式の契約正本。本スキルはこの契約に準拠し、`mode` を含む args 全量指定で単独起動できる
- 起動引数で受け取る資産の移設先（管理者が args で配布）:
  - `audit_script_path`: `~/agent-home/skills/orchestrating-reverse-docs-flow/scripts/audit-consistency.sh`
  - `template_root`: `~/agent-home/skills/orchestrating-reverse-docs-flow/assets/リバース検証`（画面詳細設計書テンプレート〔章の役割キー 16 種、既定 §1〜§16〕・プロジェクト共通規約〔コーディング規約・命名規約・ディレクトリ構成規約・コンポーネント設計規約〕を含む。Phase 1 でロード、Phase 4 で準拠）
  - `chapter_map_path`: `~/agent-home/skills/orchestrating-reverse-docs-flow/references/chapter-map.md`（章の役割キー→§対応表）
