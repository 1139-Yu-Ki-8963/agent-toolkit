...
Bash で `ls -d ~/.claude-* 2>/dev/null` を実行し、`~/.claude-{name}` パターンに合致するディレクトリを列挙する。各ディレクトリの `settings.json` の有無も確認する。
...
AskUserQuestion でサブフォルダ名を聞く。完成形のディレクトリ名 `~/.claude-{name}` を提示して確認を取る。デフォルト提案は `{provider}-{identifier}` 形式（例: `anthropic-work`）だが、ユーザーが自由に変更できる（例: `myproject`）。

**入力**: Step 2-1 の環境一覧
**完了**: 全環境のディレクトリ名 `~/.claude-{name}` が確定している
...
環境ごとにデフォルト名（`claude-{name}`）を生成し、AskUserQuestion で確認する。
...
環境ごとに Bash で `mkdir -p ~/.claude-{name}` を実行する。既存ディレクトリは作成をスキップする。
...
認証方式に応じた settings.json を動的生成して `~/.claude-{name}/settings.json` に Write する。既存の settings.json がある環境はスキップする。
...
```powershell
# setup-claude-multi-auth managed block start
function {alias} { $env:CLAUDE_CONFIG_DIR = "$env:USERPROFILE\.claude-{name}"; claude @args }
# setup-claude-multi-auth managed block end
```
...
**入力**: Phase 2 の全環境仕様（ディレクトリ名 / エイリアス名 / 認証方式）
...
1. `CLAUDE_CONFIG_DIR=~/.claude-{name} claude --version` で起動確認
