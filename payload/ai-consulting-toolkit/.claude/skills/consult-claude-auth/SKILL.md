...
invocation: consult-claude-auth
type: orchestration
allowed-tools: Read, Write, Edit, Bash, AskUserQuestion
---

# マルチアカウント / Bedrock 環境セットアップ（consult-claude-auth）
...
Step 1-1 で確定したrcファイルを Bash (`grep`) で走査し、`# consult-claude-auth managed block start` / `end` マーカーの有無を確認する。マーカーがあれば既存のエイリアス行を抽出する。
...
rcファイルの管理ブロック（`# consult-claude-auth managed block start` / `end`）内にエイリアスを追記する。既存ブロックがあれば既存エイリアスを保持しつつ新規分を追記する。既存ブロックがなければ新規作成する。

macOS/Linux (zsh/bash):
```bash
# consult-claude-auth managed block start
alias {name}='CLAUDE_CONFIG_DIR="$HOME/.claude-{provider}-{identifier}" claude'
# consult-claude-auth managed block end
```
...
- rcファイルは管理ブロック（`# consult-claude-auth managed block start/end`）内のみ操作する。ブロック外の既存設定に一切触れない
...
**保守責任者**: 人手（consult-claude-auth スキルの利用者・保守者）
