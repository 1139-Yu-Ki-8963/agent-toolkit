...
| レビュー・監査・点検・観点チェック・review・診断 | **review** | `references/<type>/conventions.md` + `references/<type>/reviewing.md`（hooks/rules/subagents は必要に応じ `check-items.md` も） | → test |
...
2. `references/<asset_type>/reviewing.md` を Read（`hooks` / `rules` / `subagents` は `check-items.md` も）
3. 型別の Phase（対象発見・静的解析・実体検証・レポート・自動修正承認）を実行。Phase 数・観点は種別ごとに異なるため `reviewing.md` の定義に従う。**修正を伴う場合は実施前に `ExitPlanMode` または `AskUserQuestion` でプラン承認を取る**
   - **実行主体**: 対象アセットをこのセッションで作成・編集した場合、観点別検査（読み取り専用 Phase）はメインが自己レビューせず investigator に委任する（生成役と検証役の分離）。別セッション・他者が作った資産の点検はメイン実行でよい。修正の適用はどちらの場合もメイン（設定管理）または worker-sonnet が行う
...
- `references/subagents/reviewing.md` — 観点 A〜F・Phase 1〜5
- `references/subagents/check-items.md` — 機械判定可能な観点の grep / bash 検出式と修正前後例
- `references/subagents/testing.md` — 実機検証・機械 lint 前段・境界シナリオ台帳・サブエージェント呼び出し規約
