...
# プロジェクト（ホーム配下の主要 repo + agent-home）
find ~/Projects ~/agent-home -maxdepth 5 -type f \
...
設計面の 5 観点に絞って評価する。`~/agent-home/skills/<name>/SKILL.md`・`~/.claude/rules/<name>-rules/rule.md` と `~/agent-home/skills/<name>/scripts/`・`~/.claude/rules/<name>-rules/` 配下の hook script も読み込む。
...
#### H. 配置アーキ準拠
- [CRITICAL] H1 PostToolUse (line 87) — command path が `~/agent-home/tools/hooks/` flat バケット
...
Agent(
  ...
  prompt: "~/agent-home/skills/managing-agent-configs/references/hooks/testing.md の手順に従い、
...
| H3 | WARN | 配置先の rule.md / SKILL.md 内に `## 設計判断` セクションが存在しない | 設計判断テンプレで新規作成 |
...
旧 `diagnose-hooks` の 5 観点を統合。dry-run モードではこのカテゴリ I〜M のみを評価する。
...
| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| N1 | hook ファイル名が動詞前置 kebab-case か | WARN | ファイル名の先頭が許可動詞（check/cleanup/delete/dispatch/record/validate/notify）で始まるか |
| N2 | 注入タグ名が hook ファイル名の slug と派生一致しているか | CRITICAL | hook 内の `[TAG]` と basename の slug を UPPER 化して突合 |
| N3 | マーカー名が hook ファイル名の slug と派生一致しているか | WARN | `marker_path` 呼び出しの引数と basename の slug を突合 |
| N4 | 多義語（main / gate / guard 等）が単独使用されていないか | WARN | `~/.claude/rules/always/naming/common-principles/naming-values.txt`「多義語表」の語を grep し、複合語でない単独使用を検出 |
| N5 | 禁止動詞（nuke / scrub / kill）が使用されていないか | CRITICAL | ファイル名の先頭を確認 |

**合計 48 項目（A〜H: 28 項目／I〜M: 15 項目／N: 5 項目）**
