...
検出:
```bash
grep -rln 'exit 2' ~/agent-home/tools/hooks/ | wc -l
```
...
検出:
```bash
grep -lE '(^|[^a-zA-Z])node ' ~/agent-home/tools/hooks/*.sh |
...
検出:
```bash
grep -lE 'claude +-p' ~/agent-home/tools/hooks/*.sh |
...
検出:
```bash
grep -rEn '/tmp/\.allow-[a-z-]+' ~/.claude/settings.json ~/agent-home/tools/hooks/ |
  grep -v 'rm -f' || true
```
...
検出:
```bash
grep -rEn 'nohup .* & *disown' ~/.claude/settings.json ~/agent-home/tools/hooks/
```
...
検出:
```bash
grep -rEn '\.env|token|key|secret|credential' ~/agent-home/tools/hooks/*.sh |
...
- GIT-AUTHOR-BLOCK / GIT-AUTHOR-PUSH-BLOCK
...
修正後: check-worker-haiku-file-change.sh
```
## N. 環境依存前提
