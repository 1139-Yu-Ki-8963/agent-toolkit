...
NO-DELEGATION / NO-DEFERRAL / CLAUDE-HOME-ROOT-MARKER-BLOCK
HOOKS-BUCKET-FORBIDDEN / WORKTREE-REQUIRED / WORKTREE-CLEANUP
FLOW-SELECT-REQUIRED / FLOW-SELECT-BLOCK / FLOW-SELFIMPROVE-PENDING
AUTO-COMMIT / PROD-SKILL-READ / DESIGN-DECISION-REQUIRED / SHELL-EVASION-DETECTED
MAIN-AGENT-DIRECT-WORK-BLOCK / SUBAGENT-DELEGATION-HINT
LOOP-DETECTED / SESSION-CONTEXT-LARGE
CURL-EGRESS-BLOCK / WORKER-HAIKU-FILE-CHANGE-BLOCK
PHASE-TASK-BLOCK / STEP-TASK-FORMAT
MANAGING-REVIEW-REQUIRED / MANAGING-GATE-DISABLED
PLAN-TACIT-KNOWLEDGE-GATE-BLOCK
PLAN-DRAFT-WRITE-GATE-BLOCK
```
...
| TAG | 参照箇所 | 用途 |
|-----|---------|------|
| `TEXTLINT` | `~/.claude/rules/always/review-checklist/text-dictionary/rule.md` | textlint エラーを rule 側で自己完結して修正（新規語彙登録時のみ `adding-textlint-dictionary-terms` スキル） |
