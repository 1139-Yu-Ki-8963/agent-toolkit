...
| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| H1 | hook ファイル名が動詞前置 kebab-case かつ禁止動詞（nuke/scrub/kill）を使用していないか | WARN | ファイル名を確認 |
| H2 | 注入タグが hook ファイル名の slug と派生一致しているか | CRITICAL | rule.md 内の `[TAG]` と同ディレクトリの .sh ファイル名を突合 |
| H3 | 多義語（main/gate/guard 等）が単独使用されていないか | WARN | naming-values.txt「多義語表」を参照 |
| H4 | サイドカーの拡張子が `.txt` か（`.md` は常時注入される） | CRITICAL | `find <dir> -name "*.md" -not -name "rule.md"` |

### 観点 I: 参照実在

| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| I1 | rule.md がバッククォートで言及する `~/` 始まりのパス（プレースホルダ・glob を含まないもの）が実在するか | CRITICAL | check-items.md I1 |

背景: 2026-07-10、スキルが観点正本として参照するパスが実在せず「必須参照が構造的に失敗する」欠陥が発見された。参照切れは書いた瞬間には正しくても rename・リポジトリ再編で壊れるため、レビューの都度、機械検証する。
