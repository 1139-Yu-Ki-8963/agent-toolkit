...
**唯一の例外**: `scoped/review-checklist/` 配下のみ **深さ 4**（`scoped/review-checklist/<domain>/<name>/rule.md`）。第 3 階層 `<domain>` はレビュー専門家サブエージェント `<domain>-reviewer` と 1 対 1 対応する（統治規約: `scoped/agent-config/review-checklist/rule.md`）。

```
~/.claude/rules/                    … グローバル rules（深さ 3 固定）
  always/                           … 常時注入（frontmatter なし）
    <topic>/                        … 内容トピック（naming / agent / agent-config / git / gate / placement / session / response / infra）
      <name>/
        rule.md                     … 規約本体（必須）
  ...
  scoped/                           … paths: frontmatter 付き（条件付き注入）
    <topic>/                        … 内容トピック（agent-config / dev-flow / tooling 等）
      <name>/
        rule.md
    review-checklist/               … レビュー観点（例外的に深さ 4）
      <domain>/                     … 専門家 <domain>-reviewer と 1 対 1（code / document）
        <name>/
          rule.md                   … 観点の正本（paths が適用対象を宣言）
...
**禁止**: ルート直下に `<name>.md` を置くこと。グローバルで深さ 3 以外に rule.md を置くこと（`scoped/review-checklist/` 配下の深さ 4 のみ例外）。
...
| 規約本体 | `rule.md`（固定） | — |
| hook script | kebab-case + `.sh` | `check-claude-home-root-marker.sh` |
| サイドカー | kebab-case + `.txt` | `naming-values.txt` |
| 注入タグ | UPPER-HYPHEN | `[CLAUDE-HOME-ROOT-MARKER-BLOCK]` |
...
判定基準:
- 規約が参照するファイル・概念が特定プロジェクト固有（例: 特定プロジェクトのみに存在する内部語彙）なら project
...
~/.claude/rules/always/response/guard/rule.md を参照。"
```
...
設計判断（ADR）は rule.md 内ではなく、同ディレクトリの **`design-notes.txt`（非注入サイドカー）** に書く（2026-07-05 のスリム化で全規約に適用済み）。rule.md 側には上記の参照 1 行だけを残す。設計判断は保守時にしか読まれず、セッション中の行動を変えないため、常時注入から外して context を節約する。
...
同ディレクトリの `design-notes.txt`（非注入サイドカー）の設計判断セクションに記載する 4 項目:
...
| ADR なしで .sh を追加 | 削除判断ができなくなる | 追加時に design-notes.txt へ ADR を書く |
| 設計判断を rule.md 内に書く | 保守時にしか読まれない長文が常時注入される | design-notes.txt サイドカーへ分離 |
