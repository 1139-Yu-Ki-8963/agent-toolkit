...
1. `~/agent-home/skills/managing-agent-configs/references/skills/template-guide.html` を `references/<name>-guide.html` にコピーする
2. `{{...}}` プレースホルダをすべて置換する（スキル名・説明・トリガー・使用タイミング・ワークフロー・予想を裏切る挙動 等）
3. 10 セクション（テンプレート定義分。§2 出力は固定枠）を SKILL.md の正本に基づいて執筆する（SKILL.md に無い仕様の創作禁止）
...
### フロー系スキルの追加チェック項目（Type: orchestration / gateway、Phase 3 つ以上）

- [ ] 各 Phase/Step に完了条件（`完了条件:` 行 or 完了条件テーブル）が記載されているか（F8 CRITICAL）
- [ ] サブエージェントを呼ぶ場合、`## サブエージェント委任仕様` セクションに仕様表（呼び出し箇所・subagent_type・prompt 骨格・期待返却値）があるか（F9 WARN）
- [ ] 反復を含む場合、`## ループ設計` セクションに 3 要素（反復条件・上限回数・停止条件）が定義されているか（F10 WARN）
- [ ] `## 完了条件` セクション末尾に最終成功判定（Goal）が 1 行で記載されているか（F11 WARN）
- [ ] 検証役（生成役と別のエージェント）が指定されているか（F12 INFO）
...
旧 3 分類（手順型 / 条件付き知識型 / 強制型）は 9 Type 分類との対応関係で残す:
