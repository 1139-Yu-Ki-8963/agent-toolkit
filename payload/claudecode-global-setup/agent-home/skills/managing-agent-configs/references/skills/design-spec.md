...
- 型の重なりは `alt_type:` で吸収する。例: 規範と反応の両方で動くスキルは `type: reference, alt_type: reactive` のように宣言する
...
```
<skill>/
├── SKILL.md          # 概要 + 目次（= 型ごとの分割単位）＋ 全体ルール ＋ 予想を裏切る挙動（必須）
...
alt_type: <type の重なりがある場合のみ・第 4 章の 9 種類のいずれか>
  # 例: type: reference, alt_type: reactive のように2軸を併記する
