...
```bash
ls ~/agent-home/skills/*/SKILL.md
# 指定時のみ
ls ~/agent-home/skills/<name>/SKILL.md
```
...
wc -l ~/agent-home/skills/<name>/SKILL.md
# references 参照先の実在
ls ~/agent-home/skills/<name>/references/ 2>/dev/null
```
...
Agent(
  ...
  prompt: "~/agent-home/skills/managing-agent-configs/references/skills/testing.md の手順に従い、
...
| C6 | WARN | `## 予想を裏切る挙動` セクションがない | 直感に反する罠を 1 行でも追記 |
...
| F14 | CRITICAL | Phase 見出しが 3 つ以上あるスキルで、全 Phase に `## Step N-M:` が 1 つ以上存在しない | Step 分割して Phase ごとに手順を明示 |
| F15 | CRITICAL | Step に `**完了**:` 行（完了判定文）がない | 各 Step 末尾に `**完了**: <判定文>` を追記 |
| F16 | WARN | Step 本文にツール名（Read/Write/Edit/Bash/Glob/AskUserQuestion/Skill/SendUserFile）が 1 つも明記されていない | 各 Step に使用ツールを明記 |
| F17 | WARN | `allowed-tools` に書かれたツールが Step で未使用、または Step で使うツールが `allowed-tools` に未記載 | allowed-tools と本文を双方向で整合 |

### G. 登録・整合性（5 項目）

| ID | 重大度 | 検出 | 修正方針 |
|----|--------|------|----------|
| G1 | CRITICAL | 配置が `~/agent-home/skills/<name>/SKILL.md` でない / 本文に絶対パス直書き | 規定パスへ移動・相対参照化 |
| G2 | WARN | `ai-management-portal/data/skill-categories.js` に未登録（other フォールバック） | カテゴリを追記する |
...
| G8 | WARN | `references/<name>-guide.html` の §2 が「出力」でない、またはセクション総数が 10（対象外リストのスキルは 9）でない（`node ~/agent-home/skills/managing-agent-configs/scripts/manage-portal.mjs verify --only guide-テンプレ準拠` で機械検出） | §2 に出力セクションを追加し、以降のセクション番号を繰り下げる |

### 観点 H: 命名・語彙

| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| H1 | スキル名が kebab-case の動名詞形で、What + How/Scope の情報密度基準を満たしているか | WARN | conventions.md §1 の情報密度基準を参照 |
| H2 | scripts/ 内のファイル名が動詞前置 kebab-case か | WARN | scripts/*.sh の先頭が許可動詞で始まるか |
| H3 | 多義語（main/gate/guard 等）が単独使用されていないか | WARN | naming-values.txt「多義語表」を参照 |

### 観点 I: 参照実在

| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| I1 | SKILL.md・references/*.md がバッククォートで言及する `~/` 始まりのパス（プレースホルダ・glob を含まないもの）が実在するか | CRITICAL | check-items.md I1 |

背景: 2026-07-10、reviewing-single-pr-with-inline-comments が観点正本として参照するパスが実在せず、必須参照が構造的に失敗していた欠陥の再発防止。

**合計 44 項目（A〜E・G〜I: 27 項目／F: 17 項目はフロー系のみ）**
...
- `node ~/agent-home/skills/managing-agent-configs/scripts/manage-portal.mjs verify` が exit 0
