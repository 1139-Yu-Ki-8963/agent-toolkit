...
- gerund 形（動詞ing + 名詞）を推奨。名詞句・動詞+名詞も許容
- 汎用動詞（do / handle / run / tune）は避け、そのスキル固有の仕組みを表す語を選ぶ
- 名詞のみ（例: supabase-postgres）は固有名詞の場合のみ許容
...
- **全スキル合計**: `~/agent-home/skills/` 全体の description 合計を **2,000 文字以内** に維持する
...
python3 -c "
import os, re
d = os.path.expanduser('~/agent-home/skills')
t = 0
...
| `gateway` | 入口型 | フロー本体の前段で素材を確定し、確定後にフローへ橋渡し | preparing-manual-mockup, managing-github-issues（pick モード）, parallel-dev-worktree |
| `gate` | 関門型 | フロー内 Phase から呼ばれ go/no-go を返す | flow-reviewing-design-doc, flow-reviewing-impl-quality, managing-github-issues（verify モード） |
...
| `reactive` | 反応型 | hook タグ起動 | reviewing-public-readiness, resolving-conflicts |
| `reference` | 規範型 | 他スキルからロードされる横串ルール集 | frontend-design, render-* 群 |
| `transform` | 生成・変換型 | 入力 → 別形式の出力 | formatting-pr, formatting-issue, subagent-investigation-checklist |
| `action` | 単発操作型 | 1 アクションで副作用ありの完了 | grouping-commits, registering-replacements, launching-battle-base-dev-servers |
...
│   └── <name>-guide.html  # スキルガイド（必須・ポータルからリンクされる）
└── assets/           # 出力用ファイル（テンプレート等）
```

**重要**: `references/` には **そのスキル固有** の詳細情報のみ配置。公式ドキュメントのコピーは禁止。詳細は `folder-structure.md` を参照。

**スキルガイド HTML**: 各スキルの `references/` 直下に `<skill-name>-guide.html` を必ず配置する。命名は `<skill-name>-guide.html`（SKILL.md のスキル名と一致）。テンプレートは `~/agent-home/skills/managing-agent-configs/references/skills/template-guide.html` に従う。単一ファイル自己完結（外部リソース参照禁止）。`manage-portal.mjs verify` が命名・構造・カバレッジを機械検査する。

**§2 出力セクション（必須）**: §1 概要の直後・§3 以降（用語・使い方等の可変枠）の前に `<section id="s2"><h2><span class="sec-num">§2</span>出力</h2>` を固定枠として配置する。導入文1行 + `<div class="def"><dt>成果物名</dt><dd>内容説明</dd></div>`（1〜4項目）で構成し、「このスキルを実行すると何が生まれるか」を結論として示す。ファイル名・パスの羅列ではなく、成果物が「何であるか」「読んだ人が何を得るか」を書く。既存の「報告フォーマット」「完了報告」節（完了時にどう報告するかを扱う）とは観点が異なるため、両方を保持したまま併設する（出力節が既存の報告系セクションを置き換えることはない）。全スキルガイドは s1〜s10 の10セクション構成になる。

### scripts/ 内ファイルの命名

- 形式: kebab-case + `.sh`、動詞前置（`check-` / `update-` / `validate-` / `cleanup-` 等）
- hook と同じ語順規則（`~/.claude/rules/always/naming/common-principles/naming-values.txt`「識別子形式表」節を参照）
- スキル名との関連が名前から読み取れること（例: `orchestrating-dev-flow/scripts/update-flow-status.sh`）
- テストファイルは対象ファイルと同名に `.test.sh` サフィックス（例: `worktree-required-check.test.sh`）
...
停止条件は loop.html の原則に従い、以下の 3 パターンのうち少なくとも 2 つを明記する:

- **収束停止**: 全チェック通過が N 連続（推奨: N=2）
- **リソース上限**: 最大反復回数に到達
- **発散検知**: 同じエラーが M 回連続で再発（推奨: M=2〜3）

検証役（評価役）は生成役と分離する。理想的には別の subagent_type / Read のみ権限を使う（loop.html §5「評価役の設計」参照）。

## 9. 配置ルール

- 配置先: `~/agent-home/skills/<name>/SKILL.md`
- プロジェクト固有なら `<repo>/.claude/skills/<name>/SKILL.md`
- 本文内では絶対パス（`/Users/...`）禁止。`~/agent-home/skills/<name>/` または相対参照を使う
...
- 全スキルは `## 完了報告` 見出しを必須で持つ
- 節の中身は `completion-report-format.md` の共通骨格（作業報告型 / 調査報告型）に従う旨の参照 1 行 + 固有の検証行追加の宣言のみとする（骨格のコピー禁止）
- 見出し名は「完了報告」の 1 種に統一する（出力フォーマット・レポートフォーマット・完了レポート・終端報告などの表記揺れを禁止）。ただし中間出力・成果物そのものの形式定義は対象外
