...
### C6 — 予想を裏切る挙動セクション（WARN）

`## 予想を裏切る挙動` セクションが存在するか。直感に反する罠・隠れた制約・読者が驚く挙動を 1 行でも記載する。

```bash
grep -q '^## 予想を裏切る挙動' "$F" && echo OK || echo "MISSING: ## 予想を裏切る挙動"
```
...
### F14 — Step 存在（Phase 3 以上）（CRITICAL）

Phase 見出しが 3 つ以上あるスキルで、各 Phase 区間内に Step が 1 つ以上あるか。SKILL.md と references/ の両方を走査する。`#{2,3} Step` で h2/h3 の両方を検出する（h3 の場合は F13 が別途捕捉する）。

```bash
PHASES=$(grep -cE '^#{2,3} Phase' "$F")
if [ "$PHASES" -ge 3 ]; then
  STEPS_MAIN=$(grep -cE '^#{2,3} Step' "$F")
  STEPS_REF=0
  for ref in ~/agent-home/skills/<NAME>/references/phase-*.md; do
    [ -f "$ref" ] || continue
    STEPS_REF=$((STEPS_REF + $(grep -cE '^#{2,3} Step' "$ref")))
  done
  STEPS=$((STEPS_MAIN + STEPS_REF))
  [ "$STEPS" -lt "$PHASES" ] && echo "CRITICAL F14: Phase $PHASES 個に対し Step $STEPS 個（不足）"
  # Phase ごとの Step 有無判定（SKILL.md のみ）
  awk '/^#{2,3} Phase/{if(p && !s) print "CRITICAL F14: " p " に Step なし"; p=$0; s=0} /^#{2,3} Step/{s=1} END{if(p && !s) print "CRITICAL F14: " p " に Step なし"}' "$F"
fi
```
...
+...
+
+## Step 3-2: references 固有性の確認
+
+...
```

### F15 — Step 完了判定文（CRITICAL）

全 Step に `**完了**:` 行が存在するか。Step 数と完了行数を比較する。

```bash
STEPS=$(grep -cE '^#{2,3} Step' "$F")
COMPLETIONS=$(grep -c '\*\*完了\*\*:' "$F")
for ref in ~/agent-home/skills/<NAME>/references/phase-*.md; do
  [ -f "$ref" ] || continue
  STEPS=$((STEPS + $(grep -cE '^#{2,3} Step' "$ref")))
  COMPLETIONS=$((COMPLETIONS + $(grep -c '\*\*完了\*\*:' "$ref")))
done
[ "$STEPS" -gt 0 ] && [ "$COMPLETIONS" -lt "$STEPS" ] && echo "CRITICAL F15: Step $STEPS 個に対し完了行 $COMPLETIONS 個（不一致）"
```

```diff
 ## Step 2-1: 対象ファイルの読み取り
 
 Read で SKILL.md を読む。
+
+**完了**: SKILL.md の全文が読み取り完了し、Phase 構造を把握済み
```

### F16 — Step 内ツール名明示（WARN）

各 Step ブロック内に Claude ツール名（Read / Write / Edit / Bash / Glob / AskUserQuestion / Skill / SendUserFile / Agent / TaskCreate）が 1 つ以上明記されているか。
...
/^#{2,3} Step/{if(name && !found) print "WARN F16: " name " にツール名なし"; name=$0; found=0}
/Read|Write|Edit|Bash|Glob|AskUserQuestion|Skill|SendUserFile|Agent|TaskCreate/{found=1}
/^#{2,3}[^#]/{if(name && !found && !/^#{2,3} Step/) print "WARN F16: " name " にツール名なし"; if(!/^#{2,3} Step/){name=""; found=0}}
END{if(name && !found) print "WARN F16: " name " にツール名なし"}
'
```
...
-行数を数える。
+Bash で `wc -l` を実行し行数を計測する。
```

### F17 — allowed-tools 突合（WARN）

frontmatter の `allowed-tools` と Step 本文で実際に言及されたツール名を双方向で突合する。`allowed-tools` 未記載のスキルはスキップ。

```bash
ALLOWED=$(grep '^allowed-tools:' "$F" | sed 's/allowed-tools: *//' | tr ',' '\n' | tr -d ' ' | sort -u)
[ -z "$ALLOWED" ] && exit 0
USED=$({
  cat "$F"
  for ref in ~/agent-home/skills/<NAME>/references/phase-*.md; do
...
} | grep -oE 'Read|Write|Edit|Bash|Glob|AskUserQuestion|Skill|SendUserFile|Agent|TaskCreate|TaskUpdate|TaskGet|TaskList|ExitPlanMode|EnterPlanMode|Monitor' | sort -u)
for t in $ALLOWED; do
  echo "$USED" | grep -qF "$t" || echo "WARN F17: allowed-tools に $t があるが Step で未使用"
done
for t in $USED; do
  echo "$ALLOWED" | grep -qF "$t" || echo "WARN F17: Step で $t を使用しているが allowed-tools に未記載"
done
```
...
### I1: SKILL.md・references が言及するパスの実在

検出対象はバッククォートで囲まれた `~/` 始まりのパス。プレースホルダ（`<repo>`・`<name>` 等）・glob（`*`）・変数（`$`）を含むものは対象外。

```bash
for f in ~/agent-home/skills/*/SKILL.md ~/agent-home/skills/*/references/*.md; do
  [ -f "$f" ] || continue
  grep -oE '`~/[^`]+`' "$f" | tr -d '`' | grep -vE '[<>*$]' | sort -u | while IFS= read -r p; do
    ep="${p/#\~/$HOME}"
    [ -e "$ep" ] || echo "CRITICAL: $f が実在しないパスを参照: $p"
  done
done
```

修正方針: 実在する正本へ差し替えるか、正本を新設してから参照する。参照先を消す判断はユーザー承認を取る。
