# 外部正本・関連スキル

`SKILL.md` 本体の「参照資料」から分離した、種別横断の外部正本と関連スキルの一覧。

## 外部正本

- `~/agent-home/ai-management-portal/design/hooks.html` — hooks 配置 4 象限・禁止 glob
- `~/agent-home/ai-management-portal/design/rules.html` — rules 設計ガイド（§1〜§11）
- `~/agent-home/ai-management-portal/design/subagent.html` — Subagent 設計ガイド（12 セクション）
- `~/agent-home/ai-management-portal/design/loop.html` — Loop 設計ガイド（フロー系 skills の F8〜F12 観点の理論的根拠）
- `~/.claude/rules/scoped/agent-config/placement/rule.md` — 設定層配置判定規約（rules vs skills vs hooks の振り分け）
- `~/agent-home/skills/managing-agent-configs/references/skills/design-spec.md` §12〜§13 — Phase / Step / TaskCreate の書式ルール（orchestration / gateway 型）

## 関連スキル

- `~/.claude/rules/always/naming/commit-branch/naming-values.txt` — スキル・ブランチ・ディレクトリ命名規則の正本
- `update-config` — settings.json への書き込み・移動・権限管理
- `adding-textlint-dictionary-terms` — hooks から呼び出されるサブエージェント側のスキル
