...
判定は Q1〜Q4 の 4 段階の分岐点で構成されるが、Q4 の YES/NO で worker-sonnet と worker-haiku に分かれるため、到達する役割は brain / researcher / investigator / worker-sonnet / worker-haiku の 5 種類になる（下記「既存 6 役割」は report-reviewer を含めた役割総数であり、本フローの到達役割数とは別の数字）。
...
| 事実性検証系 (report-reviewer) | Read, Bash, Grep, Glob | Write, Edit |
...
既存エージェントの一覧と分類（report-reviewer を含む）は `~/agent-home/ai-management-portal/catalog/subagents.html` を参照。report-reviewer は「他者の調査報告の事実性検証（チェックリスト照合・裏取り）」を担う事実性検証系で、4 役割判定フローには登場しないが、調査委任の後段（委任元が調査報告をユーザーへ伝える前）で呼ばれる判定系。code-reviewer / document-reviewer（成果物判定）と並ぶ 3 判定系の 1 つで、合否（PASS/FAIL）を宣言できるのは判定系のみ。
