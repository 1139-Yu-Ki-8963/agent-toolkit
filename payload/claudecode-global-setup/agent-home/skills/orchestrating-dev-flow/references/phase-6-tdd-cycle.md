...
**入力**: Phase 5 のテスト計画（振る舞い一覧）と指摘対応表

以下の TDD サイクルを実行する。水平スライス（テスト一括→実装一括）は禁止し、垂直スライス（1テスト→1実装）を徹底する。

1. **既知 FAIL の固定**: priorFailures を先に再現し、失敗する回帰テストへ変換する。再現不能なら証拠と理由を対応表へ記録する
2. **計画確認**: テスト対象の振る舞い一覧（Phase 5 で策定済み）をユーザーに確認する
3. **Tracer Bullet（最初の 1 サイクル）**: 振る舞い一覧の先頭 1 件について RED→GREEN を達成する
   - RED: パブリックインターフェース経由で失敗するテストを 1 件書く
   - GREEN: テストを通す最小限の実装を書く
4. **Incremental Loop（残りの振る舞い）**: 残りを 1 つずつ RED→GREEN で実装する
5. **新 FAIL の取り込み**: 新しい FAIL が出たら、原因を指摘対応表へ新規行として追加し、回帰テストを追加してから再実装する
6. **Refactor**: 全振る舞いについて GREEN 達成後、テストを壊さない範囲でリファクタリングする
...
- 既知 FAIL が先に再現または再現不能理由付きで固定され、新 FAIL も対応表と回帰テストへ反映済み
- 全テストが通過している
