...
`generating-explainer-html-build_html.py` reads these from a `prompts.json` array and renders the cards into
`index.html`. See `generating-explainer-html-sample-prompts.json` for a working set.
...
`generating-explainer-html-build_html.py` substitutes two tokens in each `prompt` with the **absolute path** of the
...
The prompt text is embedded into `index.html` and scanned by `generating-explainer-html-validate_html.py`. Phrase
...
> prompts are never rewritten by an AI — only `generating-explainer-html-build_html.py` writes them. To change the
