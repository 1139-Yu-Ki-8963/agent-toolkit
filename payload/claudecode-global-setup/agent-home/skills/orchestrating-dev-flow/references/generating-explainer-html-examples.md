...
(produced by `module-generating-explainer-yaml.md`) and shows what goes **inside a view**, how
to **build the bundle**, and how to **add another switchable view** later.

The bundled sample (`generating-explainer-html-sample-iframe.html`,
`generating-explainer-html-sample-prompts.json`) plus the sibling module's
`generating-explainer-yaml-sample-core.yaml` / `generating-explainer-yaml-sample-view.yaml`
implement **Example 1**.
