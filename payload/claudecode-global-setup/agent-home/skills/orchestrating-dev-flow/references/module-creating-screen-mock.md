...
- **必須 section 13 種** を欠かさず描画する（layout-uniformity の正本）
...
skill marker（`<!-- generated-by: creating-mock v1 -->`。検証 hook 用 marker。**file 冒頭 1KB 以内**に配置）は section ではなく file 全体に対するマーカーのため、下表には含めない。
...
| DB設計 | `<section id="db-schema"><h2>DB設計</h2>` | この画面が読み書きするテーブルの定義（表形式）。入力源: 画面基本設計書・既存スキーマファイル・要件ヒアリング結果 |
| ドメインロジック | `<section id="domain-logic"><h2>ドメインロジック</h2>` | 業務ルール（計算式・制約）・状態遷移・バリデーション一覧。入力源: 画面基本設計書・既存スキーマファイル・要件ヒアリング結果 |
| API契約 | `<section id="api-contract"><h2>API契約</h2>` | この画面が呼ぶエンドポイント一覧（メソッド・パス・リクエスト型・レスポンス型・エラー形式）。バックエンド変更を伴わない場合は既存 API の参照一覧 |
| 権限・ロール別表示 | `<section id="role-visibility"><h2>権限・ロール別表示</h2>` | ロールごとに見える/操作できる項目の差分表 |
| 画面遷移マップ | `<section id="screen-flow"><h2>画面遷移マップ</h2>` | 前後の画面と遷移条件（テキストベースの遷移図または表） |
| 受け入れテスト観点 | `<section id="acceptance-tests"><h2>受け入れテスト観点</h2>` | `acceptance` を条件・操作・期待結果のテストケース形式に構造化した表 |

`acceptance` の `<ul>` は必須。それ以外は `<ul>` / `<table>` / `<pre>` / `<p>` 自由。

データ永続化を伴わない画面（API・スクリプト・設定変更等で読み書きするテーブルが無い場合）は `db-schema` / `domain-logic` の両セクションに「対象外」+ 判断理由 1 行を記載する（セクション自体の省略は不可）。同様に、UI 画面が存在しない変更（バックエンド API・ロジックのみ）は `api-contract` に、ロール概念のないアプリは `role-visibility` に「対象外」+ 判断理由 1 行を記載する（例外規定の統一）。
...
|---|---|---|
| 画面あり（新規ページ・ダイアログ等） | `screen` | `<section id="screen-mock"><h2>画面モック</h2>` を 13 セクションの後に追加 |
| 画面なし（API・スクリプト・設定変更等） | `spec` | 13 セクション構成のまま |
...
- `screen-mock` section は必須 13 セクションの **後** に配置し、13 セクションの `id` 属性を変更しない
...
DESIGN.md（flow-values.yml の design_system で指定）を YAML parse し、`#screen-mock` スコープ内に `--app-*` prefix の CSS 変数として注入する。テンプレートの `--mock-*` は spec 部分（13 セクション）専用で据え置き、`#screen-mock` 配下だけが `--app-*` を参照する。
...
13 セクションの DOM 構造は変更せず、`<p>` / `<ul>` / `<table>` で内容を埋める。`creating-screen-mock-conventions.md`「表現規約（認知設計）」節の 6 項目（読み順ナビ・ビフォー/アフタータブ・全体図⇄詳細・重要度バッジ・段階的開示・図解義務）に従うこと。
