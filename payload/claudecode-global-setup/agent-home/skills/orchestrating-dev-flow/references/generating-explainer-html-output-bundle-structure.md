...
`generating-explainer-html-build_html.py` produces and maintains exactly this shape. It is an **assembler**:
...
- **Light by default**; reads `#theme` to support dark. See `generating-explainer-html-html-generation-rules.md`.
...
- See `generating-explainer-html-prompt-template-patterns.md` for the required set and `generating-explainer-html-sample-prompts.json`.
...
- Verified by `generating-explainer-html-validate_html.py` over `index.html` and every `views/*.html`.
