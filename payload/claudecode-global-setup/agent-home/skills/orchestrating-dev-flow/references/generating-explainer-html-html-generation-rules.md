...
These rules apply when an AI generates the HTML for this module. In normal use you author
only the **inner iframe view documents**; the outer shell (`index.html`) is written by the
deterministic `generating-explainer-html-build_html.py`, never by hand. The goal is an **offline,
self-contained, safe** bundle that adapts its explanation UI to the reader, rather than a
fixed template.

The renderer is not the center of this module. The center is: (1) `core.yaml` meaning,
...
Before authoring or presenting any view, run `~/agent-home/skills/reviewing-explanatory-html/`.
Apply all three generated rule copies, then run both `verify-html-static.mjs` and
`verify-html-runtime.mjs` against `index.html` and every `views/*.html`; the bundle-specific validator
below does not replace the common contract. Record the selected mode (`full-explanation`,
`faithful-conversion`, or `summary`), artifact hashes, mechanical JSON, and AI review result before
handing the bundle to the user.
...
`generating-explainer-html-validate_html.py` checks for these patterns and **fails** the build if any are
...
- Provide both a light `:root` palette and a `:root[data-theme="dark"]` palette using CSS
  variables. See `generating-explainer-html-sample-iframe.html` for the exact pattern.
...
4. Include the required content blocks (concepts, relations, reading order, sources,
   next questions).
5. Author/keep the prompt templates for the shell (see `generating-explainer-html-prompt-template-patterns.md`).
6. Build/grow the bundle with `generating-explainer-html-build_html.py --bundle <dir> --view-html
   "ラベル=その.html"` (this adds the view as a switchable tab).
7. Validate with `generating-explainer-html-validate_html.py <dir>/index.html <dir>/views/*.html`; fix any
   finding; re-validate.
