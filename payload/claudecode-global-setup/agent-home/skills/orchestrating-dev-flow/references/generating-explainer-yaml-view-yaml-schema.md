...
- **html_generation_policy.must_not_include** is a safety contract. It always forbids
  external scripts, remote CSS, and network requests — see `generating-explainer-html-html-generation-rules.md`.
...
See `generating-explainer-yaml-sample-view.yaml` for a working example (engineer reviewing a PR).
