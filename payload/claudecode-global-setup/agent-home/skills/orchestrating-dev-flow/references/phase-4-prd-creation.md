...
2. 画面レジストリへ specUrl / screenUrl を登録（内部処理）: `<portal_dir>/data/mocks.js` に `type: screen` のエントリとして screenUrl + specUrl を追記する（`<project>` の正本レジストリは `project-portal/data/mocks.js`）。このレジストリが無いプロジェクトでは、flow-values.yml の portal_dir 配下から specUrl / screenUrl フィールドを持つ実在レジストリを既存エントリ（先例）の grep で特定してから追記する
