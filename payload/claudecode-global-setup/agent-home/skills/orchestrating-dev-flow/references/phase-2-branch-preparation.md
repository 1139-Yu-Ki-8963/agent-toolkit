...
2. 依存パッケージの扱いはルートで分岐する
   ...
   - 他ルートも既定は lockfile・package manager・仮想環境と依存状態の検出・案内のみ。依存導入は、必要性・コマンド・影響範囲を提示し、ユーザーが明示承認した場合だけプロジェクト環境で実行する
   - 拒否・失敗時は既存依存だけで安全に検証できる範囲へ縮退する。完了条件に必要な検証が実行不能なら blocked として不足条件を報告する

**完了**: git status がクリーンで、依存状態・安全な縮退可否が確認され、導入を実行した場合はユーザー承認と再チェック証拠があること
...
- **config-with-review-and-verify**: Phase 3-6 をスキップして Phase D に直行し、D 完了後は Phase 7 → 8 へ合流
