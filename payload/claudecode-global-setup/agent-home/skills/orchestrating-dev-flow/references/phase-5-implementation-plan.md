...
## Step 5-1: 指摘対応表と実装手順の策定

> `bash ~/agent-home/skills/orchestrating-dev-flow/scripts/update-flow-status.sh 5 "実装・テスト計画" 1 3 "実装手順の策定"`

共通 handoff の `userCorrections`、`inputs` 内の priorFailures、画像の actionable findings を全件抽出し、既存の実装計画に次の指摘対応表を作る。独立した新フローは作らない。

| sourceId | source | target | implementation | verification | status |
|---|---|---|---|---|---|
| 意味語キー | 訂正・FAIL・画像指摘 | 修正対象 | 実装内容 | 検証方法 | planned |

対応表と共通 handoff を JSON にした上で、実装開始前に次を実行する。

priorFailures はこの Step 内で先に再現し、失敗する回帰テストへ固定する。再現不能なら実測証拠と理由を対応表へ記録する。これは Phase 6 を通らない refactor ルートでも必須であり、既知 FAIL の回帰化前に実装へ進まない。
...
未対応行、`target`・`implementation`・`verification`・`status` の空欄、または `publication.required: true` なのに公開先・公開検証方法が空なら no-go とし、Phase 6 を開始しない。

Phase 4 の説明用 YAML（core.yaml）（フルルート）またはタスク内容に基づき、対応表の各行と結び付く実装手順を箇条書きで策定する。
...
**完了**: 全入力が対応表へ入り validator が PASS し、実装手順が策定されていること
...
- 指摘対応表に未対応・空欄がなく、validator が PASS
- 実装手順が策定されている
