...
Three worked intents for this module. Each shows the input and the `core.yaml` (meaning) +
`view.yaml` (strategy) you would author. The bundled `generating-explainer-yaml-sample-core.yaml` /
`generating-explainer-yaml-sample-view.yaml` implement **Example 1**.
...
exactly what the downstream `module-generating-explainer-html.md` exploits when it adds new views.
...
After authoring the pair, hand the absolute paths to `module-generating-explainer-html.md` to build the
