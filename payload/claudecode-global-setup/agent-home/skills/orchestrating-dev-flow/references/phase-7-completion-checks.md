...
> `bash ~/agent-home/skills/orchestrating-dev-flow/scripts/update-flow-status.sh 7 "完了チェック" 1 4 "lint 実行"`
...
> `bash ~/agent-home/skills/orchestrating-dev-flow/scripts/update-flow-status.sh 7 "完了チェック" 2 4 "型チェック"`
...
> `bash ~/agent-home/skills/orchestrating-dev-flow/scripts/update-flow-status.sh 7 "完了チェック" 3 4 "テスト全実行"`
...
> `bash ~/agent-home/skills/orchestrating-dev-flow/scripts/update-flow-status.sh 7 "完了チェック" 4 4 "指摘対応表の完了確認"`

指摘対応表の各行を実測結果と照合し、`status` を `verified` に更新する。次の validator が PASS しない限り Phase 8 へ進まない。

```bash
node ~/agent-home/skills/orchestrating-dev-flow/scripts/validate-handoff-coverage.mjs coverage-completion .claude/dev-flow/handoff-and-coverage.json
```

この mode は指摘対応表の完了だけを判定する。Goal や publication の最終完了は判定しない。

**完了**: userCorrections・priorFailures・actionable findings の全行が verified
...
- テスト全通過
- 指摘対応表の全行が verified で coverage-completion validator が PASS
...
- Step 7-4（最後の Step）完了時: 次 Phase（Phase 8）の references を先読みし、Phase 8 の全 Step を TaskCreate
