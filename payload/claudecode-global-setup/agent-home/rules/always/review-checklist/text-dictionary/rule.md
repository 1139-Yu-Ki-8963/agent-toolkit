...
| PreToolUse(Bash) | `always/agent-config/review/check-managing-configs-commit-gate.sh` | `[MANAGING-COMMIT-BLOCK]` | テスト完了マーカーがない状態での `prh.yml` commit を exit 2 で block |
...
### `[MANAGING-REVIEW-REQUIRED]` / `[MANAGING-COMMIT-BLOCK]` 受信

`~/.claude/rules/always/agent-config/review/rule.md` の手順に従い、種別 `rules` で `managing-agent-configs` を実行してからテスト完了後に commit する。
