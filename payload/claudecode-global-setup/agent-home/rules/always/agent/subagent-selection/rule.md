...
| 調査系 | investigator / researcher / plan-comprehension-prober / adversarial-verifier | ローカルの事実収集 ／ 外部情報の収集 ／ 計画文の初見読解の言語化 ／ 成果物への反証所見の収集。証拠付きの事実を返す | 不可 |
| 判定系 | code-reviewer / document-reviewer / business-content-reviewer / report-reviewer | 成果物（コード／文書／顧客・社外提示資料）を rules に照合 ／ 調査報告の事実性を検証。**唯一、合否を宣言できる** | 可 |

- 判定系の内訳は「何を判定するか」と 1 対 1（コード・文書は `~/.claude/rules/scoped/review-checklist/` のドメインフォルダと対応、顧客・社外提示資料は business-content-reviewer、報告は report-reviewer）
...
- **好みに依存する決定の確認義務**: 「決める」工程のうち、唯一の正解がなく好みに依存する決定（表示形式・UI体裁・命名候補・文言のトーン等）は、ユーザーが可否を尋ねる探索的な質問（「〜できますか」等）に答えた直後であっても、それ自体を実装の承認とみなさない。実装前に候補となる選択肢を具体的に提示し、ユーザーの選択を得てから確定する。技術的な実現可否の確認と、実装内容そのものの確定は別の意思決定として扱う
...
Q1b. 成果物・報告の合否判定が必要か？
    成果物（コード・文書・顧客提示資料）→ Skill(reviewing-against-rules) 経由で code-reviewer / document-reviewer / business-content-reviewer
...
- **対象群の切替方式の事前確認**: 「複数の対象を判別・識別・使い分けられるようにしたい」という要件を受けた場合、対象群が実際にどのような仕組み（環境変数・設定ファイルの複数化・プロセス単位の分離等）で切り替わっているかを実装前に確認する。単一の既定情報源（設定ファイル・グローバル状態）を参照するだけの実装が要件を満たすとは限らないため、切替の実現方式を確認せずに実装しない
...
| 成果物の敵対的検証（提示前の反証） | adversarial-verifier | 作成文脈を持たない懐疑者が反証を試み、証拠付きの反証所見を返す（Skill(adversarial-verification) 経由で呼ぶ） |
...
サーバー起動等、サブエージェントのセッション終了後も存続させたいプロセスは、バックグラウンド実行の委任だけでは不十分である。サブエージェントのシェルセッション終了時に、そのシェルから起動されたプロセスも道連れで終了する場合がある。プロセスを明示的に親プロセスから切り離す処理（`nohup`・`setsid`・`disown` 等のデタッチ手段）を併用すること。
