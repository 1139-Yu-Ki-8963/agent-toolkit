...
4. `reverse-docs-engine/generation-engine/scripts/build-portal.sh <--source-root で指定したパス> <docsとproject-portalを子に持つルート> <ポータルの出力先>` を実行し、ポータル本体を作り直す
...
2. `<root>/reverse-docs-engine/` の内部階層（`generation-engine/scripts/`・`delivery-payload/templates/`・`delivery-payload/references/`）が配られたときのままであることを確かめる。階層を動かすと、生成器がテンプレートを見つけられずに止まる
