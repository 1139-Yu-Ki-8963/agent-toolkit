...
#   根拠を式に添える: 止める（根拠を欠いた式が確定すると、後から式の妥当性を検証する手段が失われるため）
...
scan_formula_table_header() {
  ...
  printf '%s' "$header_hit" | grep -qF '根拠' || echo "拒否[根拠を式に添える]: 計算式の表に根拠の欄がありません"
...
judge() {
  ...
  if [ -z "$header_hit" ]; then
    ...
    echo "対象外[根拠を式に添える]: 計算式の欄を持つ表が見当たりません"
  ...
  echo "許可[根拠を式に添える]: 計算式の表に根拠の欄があります"
...
| 消費税 | 税抜額 * 税率 | 切り捨て | 法令 | 円 |'
  ...
  # 系8: 計算式の表に根拠の欄が無い → 拒否（根拠を式に添える）
  local c8='# 計算規則
| 対象 | 計算式 | 丸め | 順序 | 単位 |
|---|---|---|---|---|
| 消費税 | 税抜額 * 税率 | 切り捨て | 1 | 円 |'
  if msg="$(judge "docs/計算規則.md" "$c8")"; then code=0; else code=$?; fi
  if [ "$code" -eq 2 ] && printf '%s' "$msg" | grep -qF '根拠を式に添える'; then
    echo "  [PASS] 系8: 根拠の欄が無い計算式の表は拒否される（${msg}）"
  else
    echo "  [FAIL] 系8: 根拠の欄が無いのに許可、または規則名が含まれない（exit=${code}, ${msg}）" >&2
    rc=1
  fi
...
| 消費税 | 税抜額 * 税率 | 切り捨て | 1 | 法令 |'
  ...
  # 系10: 計算式の表に丸め・順序・根拠・単位のすべての欄がある → 許可
  local c10='# 計算規則
| 対象 | 計算式 | 丸め | 順序 | 根拠 | 単位 |
|---|---|---|---|---|---|
| 消費税 | 税抜額 * 税率 | 切り捨て | 1 | 法令 | 円 |'
  if msg="$(judge "docs/計算規則.md" "$c10")"; then code=0; else code=$?; fi
  if [ "$code" -eq 0 ]; then
    echo "  [PASS] 系10: 4つの欄がすべて揃った計算式の表は許可される（${msg}）"
  else
    echo "  [FAIL] 系10: 4つの欄が揃っているのに拒否された（exit=${code}, ${msg}）" >&2
    rc=1
  fi
...
| 規則 | 内容 | 根拠 | 検査 |
|---|---|---|---|
| 境界の値で確かめる | 試験は *.test.java へ置く | 観測による | 静的解析 |
EOF
