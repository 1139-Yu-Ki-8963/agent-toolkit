...
複数行literal（関数本体・gql本文・型定義・複数行import）はvalue内で改行を `\n` エスケープとして保持する（YAML二重引用符スカラー。物理行は1行のまま＝固定インデント解析と互換）。1行へのsquash/`...` による省略をvalueのいかなる部分でも全面禁止する（盲検再構築者に `{ ... }` の自力再構成を強いるため⑤⑧のvalue不足に該当する）。value には位置情報（行番号・行範囲 L227-251 等・file:line）を書かない。位置情報は evidence フィールド専用。本体 literal を複数行から構成する場合も、行範囲の注記ではなく本文そのものだけを value に置く。facts の value にはコードの本文 literal のみを置き、行番号・行範囲・要約コメント等の注記をコード literal と同一物理行に混在させない（行範囲注記の数値を fact-coverage の値レベル検証がトークンとして誤検出し FAIL した実測に基づく）。
...
## normalize 規則（`generation-engine/scripts/seal-facts.sh normalize`）
...
- `generation-engine/scripts/seal-facts.sh` — 本スキーマの normalize・封印・検証を担う共有スクリプト
