...
const buildPortalPath = path.join(repoRoot, 'generation-engine', 'scripts', 'build-portal.sh');

// build-portal.sh は書込先経路にシンボリックリンクを含む場合に拒否する(assertNoLexicalSymlink)。
// macOSでは/varが/private/varへのシンボリックリンクのため、実体パスへ解決してから使う
// (generation-engine/scripts/build-portal.shのcreate_physical_tmpdirと同じ対処)。
...
(async () => {
  ...
  if (!browserPath) {
    throw markUnavailable(new Error('ChromeまたはChromiumの実行ファイルを検出できない。CHROME_BIN または PLAYWRIGHT_BROWSERS_PATH で実行ファイルの場所を指定すること'));
  }
