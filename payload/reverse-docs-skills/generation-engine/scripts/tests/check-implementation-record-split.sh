...
self_test() {
  ...
  for p in $pairs; do IFS=: read -r k d r <<< "$p"
    jq -e --arg k "$k" --arg d "$d" --arg r "$r" '((.documentTypes[$k][$d].requiredSections|length)>0) and ((.documentTypes[$k][$r].requiredSections|length)>0)' "$R/design-doc-required-sections.json" >/dev/null 2>&1 || { ok=0; echo "  宣言に節の一覧が無い: $k $d / $r"; }
    jq -e --arg k "$k" --arg d "$d" --arg r "$r" '(.kinds[$k].phases.detail|index($d))!=null and (.kinds[$k].phases.detail|index($r))!=null' "$R/design-unit-layout.json" >/dev/null 2>&1 || { ok=0; echo "  配置の宣言に両文書が無い: $k"; }
    jq -e --arg k "$k" --arg d "$d" --arg r "$r" '[.items[]|select(.kind==$k)|.unitDesignDocuments]|.[0]|(index($d)!=null and index($r)!=null)' "$R/deliverable-inventory.json" >/dev/null 2>&1 || { ok=0; echo "  納品物の一覧に両文書が無い: $k"; }
  done
  ...
  if bash "$ROOT/generation-engine/scripts/scaffold-design-unit.sh" api detail "$out" fx 合成API "$ROOT/delivery-payload/templates/リバース検証" >/dev/null 2>&1 \
  ...
  else echo "  [FAIL] 検収2: 2文書が配置されない"; rc=1; fi
  # 分ける処理: 合成の入力で現行実装の節だけが移る
  mkdir -p "$tmp/s"; printf -- '---\nunit_kind: api\n---\n\n# 合成 API詳細設計書\n\n## §1 API概要\n\n**この節の位置づけ: 仕様**\n\n本文\n\n## §2 疑似コード\n\n**この節の位置づけ: 現行実装。理由。作り直す際は引き継がない**\n\n擬似\n\n## §3 関連資料\n\n**この節の位置づけ: 仕様**\n\n§2 を見る。\n' > "$tmp/s/API詳細設計書.md"
  perl "$ROOT/generation-engine/scripts/design-docs/split-implementation-record.pl" "$tmp/s/API詳細設計書.md" >/dev/null 2>&1
