...
if ! tmp="$(mktemp -d "${TMPDIR:-/tmp}/unit-root-layout-e2e.XXXXXX" 2>/dev/null)" || [ -z "$tmp" ]; then
...
|---|---|---|---|---|---|
| root-unit | 1 | 配置上書き | input | pass | fixture.test |
...
MD
cat > "$docs/スクリーン/screen-e2e-root/テスト設計/画面結合テスト設計書.md" <<'MD'
## §1 テスト観点
...
|---|---|---|---|---|---|---|
| root-integration | 1 | 配置連結 | click | input | pass | fixture.test |
MD
