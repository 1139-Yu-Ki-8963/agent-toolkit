...
function validateSampleArtifacts(template) {
  ...
  for (const entry of readdirSync(samplesRoot, { withFileTypes: true })) {
    ...
    const htmlPath = join(portalSamplesRoot, entry.name, 'シーケンス図.html');
...
function executeRuntime(templateRuntime, pageData) {
  ...
  // テンプレートは表示幅の変化を捉えるために window へ購読を登録する。
  // 検査は描画結果だけを見るため、購読の登録は何もしない実装で足りる。
  const window = { addEventListener() {}, removeEventListener() {} };
  // テンプレートは表示幅の再計算を次の描画へ先送りする。検査は描画結果だけを
  // 見るため、渡された処理を即座に実行してよい（呼ばれる処理は再帰しない）。
  const requestAnimationFrame = (callback) => { callback(); return 0; };
  vm.runInNewContext(templateRuntime, { document, window, requestAnimationFrame }, { timeout: 1000 });
