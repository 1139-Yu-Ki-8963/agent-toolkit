#!/usr/bin/env bash
# check-design-doc-references.sh — docs/scripts/check-design-doc-references.sh の
...
# 2箇所だけである。本体(docs/scripts/check-design-doc-references.sh)は
...
# （check-depends-on-kind.sh・check-excluded-deliverables.sh と同じ形）。
#
# 使い方: bash check-design-doc-references.sh --self-test
set -euo pipefail
