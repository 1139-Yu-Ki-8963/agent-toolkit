#!/usr/bin/env bash
# test-skill-allowed-tools-scan.sh — docs/scripts/check-skill-allowed-tools.sh の実データ走査
# （引数なしモード）を第1層の機械検証から拾えるようにする入口。
#
# 背景（2026-08-28実測）: 第1層の機械検証は自己テストの分岐を持つ .sh を
# 集めるが、集めた対象は常にその分岐を通して実行する。本体
# （docs/scripts/check-skill-allowed-tools.sh）は引数なしで実データを走査する本番
...
# うえで、引数なしで実行する。
#
# 使い方: bash test-skill-allowed-tools-scan.sh
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
TARGET="${REPO_ROOT}/docs/scripts/check-skill-allowed-tools.sh"
...
run_docs_script_scan "${TARGET}" "check-skill-allowed-tools.sh"
exit $?
