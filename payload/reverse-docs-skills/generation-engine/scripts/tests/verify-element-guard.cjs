...
const TEMPLATE_PATHS=[
  'delivery-payload/templates/common-doc-template.html',
  'delivery-payload/templates/screen-doc-template.html'
];
...
try{
  for(const templatePath of TEMPLATE_PATHS){
    ...
    const {blocks,targets}=extractTargets(html);
    const missing=targets.filter(target=>target.index<0).map(target=>target.name);
    if(missing.length){console.log('FAIL: '+templatePath+' で対象の処理が見つからない → '+missing.join(', '));process.exit(1);}
    if(new Set(targets.map(target=>target.index)).size!==targets.length){console.log('FAIL: '+templatePath+' で2つの処理が別々のscriptブロックに存在しない');process.exit(1);}
    verifyTocGeneration(blocks[targets[0].index]);
    verifyTocLinks(blocks[targets[1].index]);
  }
  console.log('PASS: '+TEMPLATE_PATHS.length+'件のテンプレートで2つのscriptブロックが検査対象要素の欠落時も例外を投げずに終了する');
  process.exit(0);
}
