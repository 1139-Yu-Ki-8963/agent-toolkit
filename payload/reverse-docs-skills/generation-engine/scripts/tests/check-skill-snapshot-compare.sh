#!/usr/bin/env bash
# check-skill-snapshot-compare.sh — docs/scripts/compare-skill-snapshots.sh の
...
# 2箇所だけである。本体(docs/scripts/compare-skill-snapshots.sh)は
...
#
# 使い方: bash check-skill-snapshot-compare.sh --self-test
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
TARGET="$REPO_ROOT/docs/scripts/compare-skill-snapshots.sh"
