#!/usr/bin/env bash
# check-depends-on-kind.sh — docs/scripts/check-depends-on-kind.sh の回帰テストを
...
# 分からなくなる事態を避けるためである
# （check-skill-snapshot.sh・check-skill-snapshot-compare.sh と同じ形）。
#
# 使い方: bash check-depends-on-kind.sh --self-test
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
TARGET="$REPO_ROOT/docs/scripts/check-depends-on-kind.sh"
