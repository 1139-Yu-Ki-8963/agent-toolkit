...
self_test() {
  ...
  if [ "$(jq -r '.categories[].blueprints[] | select(.kind=="semantic-glossary") | .dir' "$DEFAULT_CATALOG")" != "project-portal/lists/semantic-glossary" ] \
    || [ "$(jq -r '.categories[].blueprints[] | select(.kind=="semantic-glossary") | .discovery.glob' "$DEFAULT_CATALOG")" != "project-portal/lists/semantic-glossary/用語辞書.html" ]; then
    echo "FAIL: semantic glossary must be physically located under project-portal/lists/semantic-glossary" >&2
    exit 1
  fi
