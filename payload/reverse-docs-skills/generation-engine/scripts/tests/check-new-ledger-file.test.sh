#!/usr/bin/env bash
# 第1層の集約へ載せるための入口。
# 本体（.claude/rules/always/tasks/instruction-format/check-new-ledger-file.sh）の自己テストを呼び、終了コードをそのまま返す。
# 判定の中身はここへ写さない。
...
TARGET="$SCRIPT_DIR/../../../.claude/rules/always/tasks/instruction-format/check-new-ledger-file.sh"
if [ ! -f "$TARGET" ]; then
