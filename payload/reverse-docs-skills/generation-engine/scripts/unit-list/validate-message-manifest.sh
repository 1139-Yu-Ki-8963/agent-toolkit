...
EOF
  ...
  # sourceDirは絶対パスのため、build-unit-list.sh側でbasenameへ正規化(1-102対応)されて
  # 埋め込まれる。期待値も同じ規則(sourceDirはbasename・sourceFileはsourceDirプレフィックス
  # 除去)で正規化してから比較する。normPathはsourceFileが配列(message契約)の場合もmapで対応する。
