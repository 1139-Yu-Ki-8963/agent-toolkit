...
# データ設計.md §6 状態遷移表（列: エンティティ|状態|遷移前|契機|遷移後|根拠パス）。
# 「疑似検証エンティティ」が下書き→受付中→完了と遷移する2行を足す
# （extract-entity-state-page-data.sh は 状態→遷移前→遷移後 の初出順でnodesを
# 集めるため、3節2辺になる）。marker は §1 データモデル概要のプレースホルダ行にも
# 現れる「<実測: エンティティ名>」ではなく、§6 の行にしか出現しない「遷移前の状態」を使う。
fill_state_transition_rows() {
  local f="$1"
  local rows
  rows='| 疑似検証エンティティ | 受付中 | 下書き | 疑似検証データの遷移1 | 受付中 | `疑似検証データ` |
| 疑似検証エンティティ | 完了 | 受付中 | 疑似検証データの遷移2 | 完了 | `疑似検証データ` |'
