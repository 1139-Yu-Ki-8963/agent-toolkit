...
#
# 出力:
#   [OK|FAIL] <検査器の名前> — 終了コード <C> の行 + 各検査器の出力の末尾10行を
#   検査器ごとに出し、末尾に 検査器 2 本 / 成功 <P> 本 / 失敗 <F> 本 のサマリ行を出す。
#
# 終了コード: 検査器のうち1本でも失敗すれば1、両方成功なら0。引数不備は2。
#
...
checksound_run() {
  ...
  local ok=0 fail=0 rc1 rc2

  _checksound_run_checker "$name1" "$path1" "$output_dir"
  rc1=$?
  if [ "$rc1" -eq 0 ]; then ok=$((ok+1)); else fail=$((fail+1)); fi

  _checksound_run_checker "$name2" "$path2" "$output_dir"
  rc2=$?
  if [ "$rc2" -eq 0 ]; then ok=$((ok+1)); else fail=$((fail+1)); fi

  echo "検査器 2 本 / 成功 ${ok} 本 / 失敗 ${fail} 本"
