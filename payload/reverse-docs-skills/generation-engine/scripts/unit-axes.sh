...
#   1. リポジトリ既定 <このファイルの親>/../../delivery-payload/references/unit-axes.json（必須。不在なら ERROR）
...
UNIT_AXES_DEFAULT="$UNIT_AXES_DIR/../../delivery-payload/references/unit-axes.json"
...
unit_axes_self_test() {
  if ! tmp="$(mktemp -d "${TMPDIR:-/tmp}/unit-axes-self-test.XXXXXX" 2>/dev/null)" || [ -z "$tmp" ]; then
