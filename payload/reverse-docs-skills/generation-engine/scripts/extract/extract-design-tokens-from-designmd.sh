#!/usr/bin/env bash
# 抽出エンジン(generation-engine/scripts/extract): DESIGN.md からデザイントークンを抽出し、
...
#
# 入力契約:
#   <DESIGN.md>   : delivery-payload/templates/リバース検証/プロジェクト共通/DESIGN.md 準拠のファイル。
...
#   <output.json> : 出力先パス
#
# 出力契約(<output.json>。正本は delivery-payload/templates/detail-pages/detail-t8-design-system.html
...
self_test() {
  ...
  local tmp rc=0
  if ! tmp="$(mktemp -d "${TMPDIR:-/tmp}/extract-design-tokens-self-test.XXXXXX" 2>/dev/null)" || [ -z "$tmp" ]; then
...
EOF
  ...
  local validate_out
  if validate_out="$(bash "$detail_pages_dir/validate-page-data.sh" "$out_json_fm" 2>&1)"; then
...
NODE
  ...
  # ("${TMPDIR:-/tmp}/extract-design-tokens-work.XXXXXX")を使っており、
...
SHIM
  ...
  if PATH="$mktemp_shim_dir:$PATH" bash "$script_path" "$design_md_fm" "$tmp/out-mktemp-check.json" >/dev/null 2>&1 \
    && grep -q -- '-d .*extract-design-tokens-work' "$mktemp_log"; then
...
if ! SCAN_WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/extract-design-tokens-scan.XXXXXX" 2>/dev/null)" || [ -z "$SCAN_WORKDIR" ]; then
  echo "[UNKNOWN] ä¸æãã£ã¬ã¯ããªã®ä½æã«å¤±æããããå¤å®ã§ãã¾ããï¼mktempãä¸æé åã¸æ¸ãè¾¼ãã¾ããã§ãããå®è¡ç°å¢ã®å¶ç´ãåå ã§ããå¯è½æ§ãããã¾ãï¼" >&2
  exit 2
fi
...
if ! TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/extract-design-tokens-work.XXXXXX" 2>/dev/null)" || [ -z "$TMP_DIR" ]; then
  echo "[UNKNOWN] ä¸æãã£ã¬ã¯ããªã®ä½æã«å¤±æããããå¤å®ã§ãã¾ããï¼mktempãä¸æé åã¸æ¸ãè¾¼ãã¾ããã§ãããå®è¡ç°å¢ã®å¶ç´ãåå ã§ããå¯è½æ§ãããã¾ãï¼" >&2
  exit 2
fi
...
extract_role_map() {
  local heading="$1"
  # scan_design_md は to_utf8_for_scan の変換結果だが、エンコーディング判定が UNKNOWN の場合や
  # 変換失敗時は非UTF-8の原本へフォールバックする(改善課題1-131)。LC_ALL=C でバイト単位走査にし、
  # フォールバック時でも「不正なマルチバイト列」警告を避ける(下の extract_components_table も同じ理由)。
