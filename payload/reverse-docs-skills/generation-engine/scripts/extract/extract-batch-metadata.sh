#!/usr/bin/env bash
# 抽出エンジン(generation-engine/scripts/extract): バッチ種別マニフェストへのメタデータ抽出。
...
#
# 出力フィールド(スキーマ正本: delivery-payload/references/manifest-schema-extensions.md「batches」節):
...
EOF
  ...
  jq -n --arg sourceDir "$tmp/src" '{
    ...
    units: [
      {unitKey: "daily-summary", kind: "scheduled", identifier: "jobs/daily_summary.py",
       unitNameGuess: "日次集計", sourceFile: "jobs/daily_summary.py", confidence: "high",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "monthly-report", kind: "scheduled", identifier: "jobs/monthly_report.py",
...
EOF
  ...
  jq -n --arg sourceDir "$tmp/multi" '{
    ...
    detectionSummary: {unitCount: 3, unresolvedCount: 0},
    units: [
      {unitKey: "job-a", kind: "triggered", identifier: "jobs/job_a.py", unitNameGuess: "ジョブA",
       sourceFile: "jobs/job_a.py", confidence: "high", fileCount: 1, detectionMethod: "manual"},
      {unitKey: "job-b", kind: "triggered", identifier: "jobs/job_b.py", unitNameGuess: "ジョブB",
       sourceFile: "jobs/job_b.py", confidence: "high", fileCount: 1, detectionMethod: "manual"},
      {unitKey: "job-c", kind: "triggered", identifier: "job-c", unitNameGuess: "ジョブC",
  ...
  for ((n = 1; n <= 500; n++)); do
    padded="$(printf '%04d' "$n")"
    printf '{"unitKey":"job-%d","kind":"triggered","identifier":"jobs/job_%s.py","unitNameGuess":"ジョブ%d","sourceFile":"jobs/job_%s.py","confidence":"high","fileCount":1,"detectionMethod":"manual"}\n' \
...
USAGE="Usage: extract-batch-metadata.sh <batch-manifest.json> <source-dir> <output.json> [--cron-file <path>] [--table-manifest <path>] [--rules-file <json>]"
MANIFEST="${1:?$USAGE}"
SOURCE_DIR="${2:?$USAGE}"
OUTPUT_JSON="${3:?$USAGE}"
shift 3
...
bash "$_EXTRACT_BATCH_SCRIPT_DIR/finalize-extension-manifest.sh" "$MANIFEST" "$OUTPUT_JSON" --rules-file "$EXTRACTION_RULES_FILE" --rule 'schedule|cron ファイルの5フィールド式' --rule 'targetTables|ジョブ内のテーブル識別子' --rule 'downstreamJobs|enqueue または実行呼び出し' --rule 'execMethod|shebang または main ガード'
