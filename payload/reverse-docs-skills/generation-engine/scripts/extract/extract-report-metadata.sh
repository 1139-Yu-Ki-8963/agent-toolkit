#!/usr/bin/env bash
# 抽出エンジン(generation-engine/scripts/extract): 帳票種別マニフェストへのメタデータ抽出。
...
#
# 出力フィールド(スキーマ正本: delivery-payload/references/manifest-schema-extensions.md「reports」節):
...
EOF
  ...
  jq -n --arg sourceDir "$tmp/src" '{
    ...
    units: [
      {unitKey: "sales-report", kind: "generator", identifier: "jobs/sales_report.py",
       unitNameGuess: "売上帳票", sourceFile: "jobs/sales_report.py", confidence: "high",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "users-export", kind: "generator", identifier: "views/export_users.py",
       unitNameGuess: "ユーザー出力", sourceFile: "views/export_users.py", confidence: "high",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "mixed-output", kind: "generator", identifier: "views/mixed_output.py",
       unitNameGuess: "混在出力", sourceFile: "views/mixed_output.py", confidence: "medium",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "sales-csv", kind: "generator", identifier: "reports/sales_csv.py",
  ...
  jq -S 'del(.units[].format, .units[].trigger)
...
USAGE="Usage: extract-report-metadata.sh <report-manifest.json> <source-dir> <output.json> [--rules-file <json>]"
MANIFEST="${1:?$USAGE}"
SOURCE_DIR="${2:?$USAGE}"
OUTPUT_JSON="${3:?$USAGE}"
...
bash "$_EXTRACT_REPORT_SCRIPT_DIR/finalize-extension-manifest.sh" "$MANIFEST" "$OUTPUT_JSON" --rules-file "$EXTRACTION_RULES_FILE" --rule 'format|PDF CSV Excel 帳票ライブラリ' --rule 'trigger|画面またはバッチからの呼び出し'
