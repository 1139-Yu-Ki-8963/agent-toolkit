#!/usr/bin/env bash
# 抽出エンジン(generation-engine/scripts/extract): 外部連携種別マニフェストへのメタデータ抽出。
...
#
# 出力フィールド(スキーマ正本: delivery-payload/references/manifest-schema-extensions.md「externals」節):
...
EOF
  ...
  jq -n --arg sourceDir "$tmp/src" '{
    ...
    units: [
      {unitKey: "payment-api-client", kind: "client", identifier: "clients/payment_client.py",
       unitNameGuess: "決済API連携", sourceFile: "clients/payment_client.py", confidence: "high",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "payment-webhook-receiver", kind: "webhook", identifier: "hooks/receive_payment.py",
       unitNameGuess: "決済Webhook受信", sourceFile: "hooks/receive_payment.py", confidence: "high",
       fileCount: 1, detectionMethod: "manual"},
      {unitKey: "bank-sftp-upload", kind: "client", identifier: "transfer/bank_upload.py",
...
USAGE="Usage: extract-external-metadata.sh <external-manifest.json> <source-dir> <output.json> [--definition-file <path>] [--declaration-candidates <path>] [--rules-file <json>]"
MANIFEST="${1:?$USAGE}"
SOURCE_DIR="${2:?$USAGE}"
OUTPUT_JSON="${3:?$USAGE}"
shift 3
...
bash "$_EXTRACT_EXTERNAL_SCRIPT_DIR/finalize-extension-manifest.sh" "$MANIFEST" "$OUTPUT_JSON" --rules-file "$EXTRACTION_RULES_FILE" --rule 'direction|送信 client または受信 endpoint' --rule 'protocol|SFTP webhook REST の通信記法' --rule 'authMethod|OAuth API key Basic の認証記法'
