...
#   (スキーマ正本: delivery-payload/references/manifest-schema-extensions.md「apis(API)」節):
#     method       : string   GET/POST/PUT/PATCH/DELETE のいずれか、またはそれらを
#                              / で連結した値（例: GET/POST。空白・空要素は不可）
...
#   identifier のパスは APIRouter(prefix=...) 等のプレフィックス結合後の完全パスで記録される
#   一方、デコレータ側はプレフィックスを除いた部分パスしか書かない。完全パスでの一致を試みて
#   失敗した場合、同ファイル内の APIRouter(prefix="...") 宣言(複数行に分かれた宣言・同一
#   ファイル内の複数ルーター変数が異なるprefixを持つ場合を含む)からプレフィックスを1件ずつ
#   取り出し、identifier のパスから差し引いた残りで再照合する(router_prefixes_from_source)。
#   これでも特定できない場合(非デコレータ方式のルーティング等)は従来のファイル単位
#   検査へフォールバックし、その旨を stderr に WARN 出力する(fail-safe)。
#   既知の限界: 同一ファイル内の異なるprefixを持つ複数ルーターが、プレフィックスを除いた
#   残余パス+methodの組を完全に同じ文字列で共有する場合(例: 2つの別ルーターが両方とも
#   "/list"というGETを持つ)、endpoint_blockはreceiver名を見ないため先に出現した方の
#   デコレータへ誤帰属する。実装コードで残余パスが衝突するケースは稀(通常はルーターごとに
#   異なるリソースパスを持つ)だが、発生した場合は誤帰属の可能性がある。
#
...
#                     認証パターンがあれば true。判定は2系統のOR:
#                       (a) 固定の慣用句(フレームワーク横断で名前がほぼ固定される記法):
#                           Depends(get_current_user / @login_required / requireAuth / verify_token / IsAuthenticated
#                       (b) Depends(...) の依存関数名に認証・認可を意味する語(大小文字無視の部分一致):
#                           auth / admin / permission / login / token / current_user
#                           (対象アプリ固有の関数名を決め打ちで列挙するのではなく、命名慣習に含まれる
#                           意味のある語根で判定する。例: Depends(require_auth_user)・Depends(require_admin)。
#                           "session"は除外語根: DBセッション等の非認証依存(get_session等)に
#                           普遍的に現れ、誤検出の原因になるため含めない)
#                     認証除外パターン(単語境界付き)
#                       AllowAny / public
#                     があれば false。検査範囲が取れない・どちらのパターンも無い場合は付けない。
#                     既知の限界(扱えない記法): ルーター登録時に一括付与される認証依存
#                       (例: include_router(router, dependencies=[Depends(require_auth_user)]))は、
#                     当該ルーターの各エンドポイントの関数ブロックに現れないため検出できない。
#                     この形式で認証を掛けているエンドポイントは authRequired が欠落する(fail-safeとして
#                     欠落を優先し、誤った false を出さない)。将来対応する場合はルーター変数からの
#                     include_router呼び出しを別途走査し、その配下の全エンドポイントへ伝播させる仕組みが必要
...
# Depends(...) の依存関数名に認証・認可を意味する語根が含まれるかを大小文字無視で判定する
# (対象アプリ固有の関数名の決め打ちを避けるための一般化。上記 AUTH_POSITIVE_ERE の固定慣用句と
# OR で判定する。改善課題: 認証要否-依存関数の命名差異を認識しない)
# "session"はDBセッション等の非認証依存(例: get_session)にも普遍的に現れるため語根から除外する
# (実測: 対象アプリでDepends(get_session)が141エンドポイント全件に存在し、含めると全件が
# 誤ってauthRequired=trueになった)。
AUTH_DEPENDS_KEYWORD_ERE='Depends\([[:space:]]*[A-Za-z0-9_]*(auth|admin|permission|login|token|current_user)[A-Za-z0-9_]*[[:space:]]*\)'
...
EOF
  ...
  # 対象アプリ固有の命名(get_current_user等の固定慣用句と一致しない)でも、
  # Depends(...) の依存関数名に認証・認可を意味する語根が含まれれば authRequired=true と
  # なることを検証する(改善課題: 認証要否-依存関数の命名差異を認識しない)。
  cat > "$tmp/src/api/custom_auth_names.py" <<'EOF'
from fastapi import APIRouter, Depends
router = APIRouter()

@router.get("/api/custom-auth", response_model=CustomList)
def list_custom(user=Depends(require_auth_user)):
    return []

@router.post("/api/custom-admin")
def create_custom(admin=Depends(require_admin)):
    return {"ok": True}

@router.get("/api/custom-plain")
def list_plain(db=Depends(get_session)):
    return []
EOF

  # APIマニフェスト(独自ルート方式を含む 32 ユニット)
  ...
  jq -n \
    ...
    --arg customAuthFile "$tmp/src/api/custom_auth_names.py" \
    '{
      generatedAt: "2026-01-01T00:00:00Z",
      sourceDir: $sourceDir,
      unitKind: "api",
      strategy: {extractionMethod: "custom", approvedByUser: true, unitIdRegex: null, excludePatterns: []},
      detectionSummary: {unitCount: 33, unresolvedCount: 0},
      units: [
        ...
        {unitKey: "imported-hono", kind: "endpoint", identifier: "/api/imported-hono",
         ...
         confidence: "high", fileCount: 1, detectionMethod: "manual"},
        {unitKey: "custom-auth-check", kind: "endpoint", identifier: "GET /api/custom-auth",
         unitNameGuess: "対象アプリ固有名の認証チェック", sourceFile: $customAuthFile,
         confidence: "high", fileCount: 1, detectionMethod: "manual"},
        {unitKey: "custom-admin-check", kind: "endpoint", identifier: "POST /api/custom-admin",
         unitNameGuess: "対象アプリ固有名の管理者チェック", sourceFile: $customAuthFile,
         confidence: "high", fileCount: 1, detectionMethod: "manual"},
        {unitKey: "custom-plain-check", kind: "endpoint", identifier: "GET /api/custom-plain",
         unitNameGuess: "DBセッションのみで認証根拠なし", sourceFile: $customAuthFile,
         confidence: "high", fileCount: 1, detectionMethod: "manual"}
      ]
    }' > "$api_manifest"
  ...
  check "対象アプリ固有命名 Depends(require_auth_user): 固定慣用句と不一致でも語根一致でauthRequired=true" \
    '(.units[] | select(.unitKey == "custom-auth-check") | .authRequired) == true'
  check "対象アプリ固有命名 Depends(require_admin): 固定慣用句と不一致でも語根一致でauthRequired=true" \
    '(.units[] | select(.unitKey == "custom-admin-check") | .authRequired) == true'
  check "誤検出防止: Depends(get_session)はDBセッション依存であり認証根拠にならずauthRequiredは欠落" \
    '(.units[] | select(.unitKey == "custom-plain-check") | has("authRequired")) | not'
  ...
  # フォールバック率diagnostics(1-130): 33ユニット中、関数ブロックをデコレータ境界で
  # 特定できたのはusers/ping/posts-list/posts-detail/custom-auth-check/custom-admin-check/
  # custom-plain-checkの7件のみ。残り26件は独自ルート方式(デコレータ非対応)でファイル単位・
  # 近傍窓へフォールバックする。survey-doc-path未指定ならratio=26/33(≈0.788) > 0.5 でwarning: trueになるはず
  check "fallback診断: count=26" '.detectionSummary.diagnostics.fallback.count == 26'
  check "fallback診断: total=33" '.detectionSummary.diagnostics.fallback.total == 33'
...
FastAPI のデコレータでルーティングを定義する。" > "$tmp/callstyle/survey-mismatch.md"
  ...
  # --- APIRouter(prefix=...)によるプレフィックス不一致対応(改善課題) ---
  # identifierはプレフィックス結合後の完全パス("/api/v1/users")を記録する一方、
  # デコレータ側はプレフィックスを除いた部分パス("/users")のみを書く。この修正前は
  # 完全パスでの固定文字列一致に失敗し、関数ブロックを特定できずファイル単位フォールバックへ
  # 落ちてauthRequiredが欠落する。修正後はAPIRouter(prefix="...")宣言からプレフィックスを
  # 取り出し、識別子パスから差し引いた残りで再照合してブロックを特定できることを検証する。
  mkdir -p "$tmp/prefix/src"
  cat > "$tmp/prefix/src/users.py" <<'EOF'
from fastapi import APIRouter, Depends
...
def list_users(current_user=Depends(get_current_user)):
    return []
EOF
  local prefix_manifest="$tmp/prefix/manifest.json" prefix_file="$tmp/prefix/src/users.py"
  jq -n --arg f "$prefix_file" '{
    generatedAt: "2026-01-01T00:00:00Z", sourceDir: "x", unitKind: "api",
    strategy: {extractionMethod: "custom", approvedByUser: true, unitIdRegex: null, excludePatterns: []},
    detectionSummary: {unitCount: 1, unresolvedCount: 0},
    units: [
      {unitKey: "prefixed-users", kind: "endpoint", identifier: "GET /api/v1/users",
       unitNameGuess: "プレフィックス付きユーザー一覧", sourceFile: $f,
       confidence: "high", fileCount: 1, detectionMethod: "manual"}
    ]
  }' > "$prefix_manifest"
  local prefix_out="$tmp/prefix/out.json"
  bash "$script_path" "$prefix_manifest" "$tmp/prefix/src" "$prefix_out" >/dev/null 2>&1
  check_file "プレフィックス不一致対応: フォールバックせず関数ブロックで照合(fallback.count=0)" \
    "$prefix_out" '.detectionSummary.diagnostics.fallback.count == 0'
  check_file "プレフィックス不一致対応: 残余パスでの再照合によりauthRequired=trueを検出" \
    "$prefix_out" '.units[0].authRequired == true'

  # --- プレフィックス不一致対応: 複数行のAPIRouter宣言 + 同一ファイル内の異なるprefixを持つ
  # 複数ルーター変数(実アプリのenemy_formations.py等で実際に見られる形式)を検証する。
  mkdir -p "$tmp/prefix-multi/src"
  cat > "$tmp/prefix-multi/src/enemy_formations.py" <<'EOF'
from fastapi import APIRouter, Depends

router = APIRouter(
    prefix="/api/enemy-formations",
    tags=["enemy-formations"],
)
teams_router = APIRouter(prefix="/api/teams", tags=["enemy-formations"])

@router.get("/list", response_model=FormationList)
def list_formations(current_user=Depends(get_current_user)):
    return []

@teams_router.get("/marked")
def list_marked_teams():
    return []
EOF
  local prefix_multi_manifest="$tmp/prefix-multi/manifest.json" prefix_multi_file="$tmp/prefix-multi/src/enemy_formations.py"
  jq -n --arg f "$prefix_multi_file" '{
    generatedAt: "2026-01-01T00:00:00Z", sourceDir: "x", unitKind: "api",
    strategy: {extractionMethod: "custom", approvedByUser: true, unitIdRegex: null, excludePatterns: []},
    detectionSummary: {unitCount: 2, unresolvedCount: 0},
    units: [
      {unitKey: "formations-list", kind: "endpoint", identifier: "GET /api/enemy-formations/list",
       unitNameGuess: "編成一覧", sourceFile: $f, confidence: "high", fileCount: 1, detectionMethod: "manual"},
      {unitKey: "teams-marked", kind: "endpoint", identifier: "GET /api/teams/marked",
       unitNameGuess: "マーク済みチーム一覧", sourceFile: $f, confidence: "high", fileCount: 1, detectionMethod: "manual"}
    ]
  }' > "$prefix_multi_manifest"
  local prefix_multi_out="$tmp/prefix-multi/out.json"
  bash "$script_path" "$prefix_multi_manifest" "$tmp/prefix-multi/src" "$prefix_multi_out" >/dev/null 2>&1
  check_file "プレフィックス不一致対応(複数行宣言+複数ルーター): fallback.count=0" \
    "$prefix_multi_out" '.detectionSummary.diagnostics.fallback.count == 0'
  check_file "プレフィックス不一致対応(複数行宣言+複数ルーター): 編成側がauthRequired=trueを検出" \
    "$prefix_multi_out" '(.units[] | select(.unitKey == "formations-list") | .authRequired) == true'
  check_file "プレフィックス不一致対応(複数行宣言+複数ルーター): チーム側は別prefixで正しく照合(認証根拠なしで欠落)" \
    "$prefix_multi_out" '(.units[] | select(.unitKey == "teams-marked") | has("authRequired")) | not'
...
USAGE="Usage: extract-api-metadata.sh <api-manifest.json> <source-dir> <output.json> [--screen-manifest <json>] [--table-manifest <json>] [--survey-doc-path <md>] [--rules-file <json>]"
MANIFEST="${1:?$USAGE}"
SOURCE_DIR="${2:?$USAGE}"
OUTPUT="${3:?$USAGE}"
shift 3 || true
...
route_receivers_from_source() {
  ...
  # $src(呼び出し元では scan_file)は to_utf8_for_scan の変換結果だが、エンコーディング判定が
  # UNKNOWN の場合や変換失敗時は非UTF-8の原本へフォールバックする(改善課題1-131)。
  # LC_ALL=C でバイト単位走査にし、フォールバック時でも tr の「不正なマルチバイト列」警告を避ける。
...
route_method_from_source() {
  ...
  # LC_ALL=C の理由は route_receivers_from_source と同じ(非UTF-8フォールバック時の警告回避)
  compact="$(LC_ALL=C tr -d '[:space:]' <<<"$route_source")"
...
# window文字列中の最初の APIRouter(...) 呼び出しを、丸括弧の対応が閉じる位置までだけ取り出す。
# 同じwindowに後続の別のAPIRouter呼び出しが含まれていても、その呼び出しの範囲までは含めない
# (含めると、後続呼び出しのprefixを誤って自分のprefixとして採用してしまう)。
extract_router_call() {
  local window="$1"
  awk -v s="$window" 'BEGIN {
    idx = index(s, "APIRouter(")
    if (idx == 0) { exit }
    depth = 0
    end = 0
    for (i = idx; i <= length(s); i++) {
      c = substr(s, i, 1)
      if (c == "(") depth++
      else if (c == ")") {
        depth--
    ...
    printf "%s", substr(s, idx, end - idx + 1)
  }'
}

# ファイル内の APIRouter(prefix="...") 宣言すべてからプレフィックスを取り出す(改善課題:
# プレフィックス不一致対応)。1ファイルに複数のルーター変数が異なるprefixで宣言される場合
# (例: router = APIRouter(prefix="/api/x") と teams_router = APIRouter(prefix="/api/y") が
# 同一ファイルに共存する)があるため、行単位ではなく宣言単位で1件ずつ返す(重複除去済み)。
# `APIRouter(` と `prefix=` が複数行に分かれる宣言(引数を改行して書く形式)にも対応するため、
# `APIRouter(` を含む行から後続6行までを1つの窓として連結し、extract_router_callで当該呼び出し
# だけに絞ってから抽出する。宣言が無ければ何も出力しない。
router_prefixes_from_source() {
  local src="$1" line_nums ln window call prefix
  line_nums="$(grep -n 'APIRouter(' "$src" 2>/dev/null | cut -d: -f1)"
  [ -z "$line_nums" ] && return 0
  while IFS= read -r ln; do
    [ -z "$ln" ] && continue
    window="$(sed -n "${ln},$((ln + 6))p" "$src" 2>/dev/null | tr '\n' ' ')"
    call="$(extract_router_call "$window")"
    [ -z "$call" ] && continue
    prefix="$(printf '%s' "$call" \
      | sed -nE "s/.*prefix[[:space:]]*=[[:space:]]*[\"']([^\"']*)[\"'].*/\\1/p" | head -1)"
    [ -n "$prefix" ] && printf '%s\n' "$prefix"
  done <<< "$line_nums" | awk '!seen[$0]++'
}
...
while IFS= read -r row; do
  ...
  if [ -n "$scan_file" ]; then
    ...
    # --- プレフィックス不一致対応: APIRouter(prefix=...)結合後の完全パスをそのまま探しても、
    # ソース側のデコレータはプレフィックスを除いた部分パスしか書かない(例: identifier側は
    # "/api/v1/users"、デコレータ側は "/users")。ルーター宣言からプレフィックスが判れば、
    # それを識別子パスの先頭から差し引いた残りで再照合する。 ---
    if [ -z "$block" ]; then
      while IFS= read -r router_prefix; do
        [ -z "$router_prefix" ] && continue
        case "$api_path" in
          "$router_prefix"/*)
            residual_path="${api_path#"$router_prefix"}"
            if [ -n "$residual_path" ]; then
              block="$(endpoint_block "$scan_file" "$residual_path" "$method" || true)"
              [ -n "$block" ] && break
            fi
            ;;
        esac
      done < <(router_prefixes_from_source "$scan_file")
    fi
  ...
  auth=""
  if [ -n "$scan" ]; then
    if printf '%s\n' "$scan" | grep -qE -- "$AUTH_POSITIVE_ERE" \
      || printf '%s\n' "$scan" | grep -qiE -- "$AUTH_DEPENDS_KEYWORD_ERE"; then
      auth="true"
...
bash "$_EXTRACT_API_SCRIPT_DIR/finalize-extension-manifest.sh" "$MANIFEST" "$OUTPUT" --rules-file "$EXTRACTION_RULES_FILE" --rule 'method|HTTP method と framework route call' --rule 'authRequired|認証依存または認証除外パターン' --rule 'callers|画面マニフェストの relatedApis' --rule 'targetTables|関数ブロック内のテーブル識別子' --rule 'ioSummary|入出力モデルの型注釈'
