...
if [ "${1:-}" = "--self-test" ]; then
  ...
  if [ -f "$RUNBOOK" ] && grep -q "RUNBOOK.md" "$REPO_ROOT/.claude/skills/orchestrating-ai-development-setup/references/contract.md"; then ok3=1; else ok3=0; fi
  ...
  if [ -f "$RUNBOOK" ] && grep -q "RUNBOOK.md" "$REPO_ROOT/generation-engine/scripts/check-worktree-commit-guard.sh"; then ok5=1; else ok5=0; fi
