...
# 素直な形（ロケール指定なしの sort）を避け LC_ALL=C を明示している。既定ロケールでの
# 照合順序はロケールデータに依存し環境ごとに違いうるため、record/status の2回の実行や
# 別環境での再実行で列挙順序が変わると diff（check-reproducible.sh の再現判定）が
# ずれる。実測値なし。環境（ロケール設定）に依存する。手元の環境で問題が起きない
# ことを理由に LC_ALL=C を外すな。
list_targets() {
  local root="$1"
...
cmd_record() {
  ...
  # $TMPDIR を無視し書き込みを拒む環境で失敗する。このリポジトリの本ファイル自身が
  # 過去にこの形で実際に失敗した実績がある（コミット
  # 803d84cdaad314cf21f8b1dadb9e2119524e0d15「台帳の道具が一時領域を作れず止まる」）。
  # 環境（サンドボックス実行環境かどうか）に依存する。手元で裸の mktemp -d に戻して
  # 動いて見えても、それを理由に明示テンプレートを外すな。
  tmp="$(mktemp "${TMPDIR:-/tmp}/derived-drift.XXXXXX")" || {
    echo "ERROR: 一時ファイルを作成できない（TMPDIR=${TMPDIR:-/tmp}）" >&2
    exit 1
  }
  echo '{}' > "$tmp"
  local rel h
...
self_test() {
  ...
  t="$(mktemp -d "${TMPDIR:-/tmp}/derived-drift.XXXXXX")" || {
    echo "ERROR: 一時ディレクトリを作成できない（TMPDIR=${TMPDIR:-/tmp}）" >&2
    exit 1
  }
