...
# 呼び出し元の環境変数に LC_ALL=C が前置きされていても上書きされるよう、
# awk 呼び出し自体へ都度明示する（export だけでは上書きされない）。
AGGREGATE_NO_GROUND_LINES="$(LC_ALL=en_US.UTF-8 awk '
...
if [ -z "$REPO_ROOT_O" ]; then
  ...
  # （公開手順を定める配布対象外の規約。公開先リポジトリの同期定義における同期対象は
  # .claude/skills・shared・README.md・docs/guides/reverse-docs-overview.html の4点のみで
  # .claude/rules は含まれない）の
