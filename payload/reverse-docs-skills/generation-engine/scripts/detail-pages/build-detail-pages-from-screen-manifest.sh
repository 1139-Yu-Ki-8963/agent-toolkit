...
# extract-screen-metadata.shのsha256_12()と同じ理由: 特定の1コマンドに固定せず
# 両OSで動くcommand -vの分岐を使う)
manifest_hash() {
  local file="$1"
...
# 素直な形(ext側の宣言hashをそのまま信用する)を避け、rawを自前で再ハッシュして
# 突合する。extの--raw-manifestに古い/別のraw manifestが渡された場合、
# 82〜99行目の「既存edgesの引き継ぎ判定」はhash一致だけを根拠にedgesを
# 引き継ぐため、この突合が無いと無関係なmanifest間でedgesが誤って引き継がれうる。
