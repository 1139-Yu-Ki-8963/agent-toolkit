...
self_test() {
  ...
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/extract-entity-state-selftest.XXXXXX")"
...
EOF
  ...
  t4_root="$(mktemp -d "${TMPDIR:-/tmp}/extract-entity-state-t4.XXXXXX")"
...
EOF
  ...
  t5_root="$(mktemp -d "${TMPDIR:-/tmp}/extract-entity-state-t5.XXXXXX")"
