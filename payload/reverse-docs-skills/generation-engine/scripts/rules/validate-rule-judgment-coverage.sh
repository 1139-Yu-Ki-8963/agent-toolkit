...
#                検査列（表の最後の列。列数は規約ごとに異なりうる。下記
#                「実装判断」を参照）が「不可:」または「判定不能:」で
#                始まらないもの。「## このプロジェクトの規則」の表は
#                対象プロジェクトのリバース解析が起こすものであり、
#                配る側の規則ではないため数えない。
#   判定の数   — 対応する検査スクリプトの中で
...
#     「名前の不一致」として報告される
#
# 実装判断（検査列を固定列番号ではなく末尾からの走査で取る理由）:
#   当初は検査列を固定で $5（4列表 規則|内容|根拠|検査 を想定した位置）
#   から取っていた。ところが配る規約27件の「## 規則」の表はすべて3列
#   （規則|内容|検査）であり、awk -F"|" で分けると $4 が検査列、$5 は
#   行末の "|" が作る空フィールドになる。固定で $5 を読む実装は常に
#   空文字を検査列として扱い、「不可:」「判定不能:」の除外が1件も
#   効かない欠陥だった（2026-08-28 実測。document-writing.md で規則の
#   数が本来5件のところ8件と報告されていた）。将来4列の表が現れる
#   可能性もあるため列数を決め打ちにせず、行末から遡って最初に見つかる
#   非空フィールドを検査列として扱う。この欠陥は自己テストの作例が
#   すべて4列の表を使っていたために長期間検出されなかった
#   （3列の表を使う作例を欠いていたため、自己テストが3列表の欠陥を
#   確認できなかった）。
#
# 使い方:
...
extract_rule_rows() {
  ...
  awk -F'|' '
    ...
    insec == 1 && /^\|/ {
      ...
      if (line ~ /^\|[-: ]+\|[-: ]+\|/) next
      name = $2; gsub(/^[ \t]+|[ \t]+$/, "", name)
      # 検査列: 表の列数は規約ごとに異なる（3列=規則|内容|検査、
      # 4列=規則|内容|根拠|検査 等）。行末は必ず "|" で終わるため
      # awk -F"|" は末尾に空フィールドを1つ作る。列数を決め打ちに
      # せず、行末から遡って最初に見つかった非空フィールドを検査列
      # とする（2026-08-28 実測: 配る規約27件はすべて3列であり、
      # 固定で $5 を取ると常に空になる欠陥があった）。
      check = ""
      for (ci = NF; ci >= 1; ci--) {
        cand = $ci
        gsub(/^[ \t]+|[ \t]+$/, "", cand)
        if (cand != "") { check = cand; break }
      }
      if (name == "規則") next
      print name "\t" check
    }
...
EOF
  ...
  # ケース10: 3列の表（規則|内容|検査。配る規約27件が実際に使う列数）・
  # 判定不能の行を1件含む・判定2件 → 一致。固定 $5 のままなら検査列が
  # 常に空になり不可・判定不能の除外が効かず、規則の数が3件（実際は2件）
  # のまま報告されて不一致になる。2026-08-28実測の欠陥を再現する回帰ケース。
  cat > "$tmp/case10-rule.md" <<'EOF'
# 規約例10（3列）
...
| 規則C | 内容C | 判定不能: 判定できない |
EOF
  cat > "$tmp/case10-checker.sh" <<'EOF'
#!/usr/bin/env bash
echo "拒否[規則A]: だめ"
echo "許可[規則B]: よい"
EOF
  assert_case "ケース10（3列）" "$tmp/case10-rule.md" "$tmp/case10-checker.sh" 2 2 一致 || rc=1

  # ケース11: 実データ（document-writing.md）で検査列の除外が効くことを
  # 確かめる。同ファイルの「## 規則」表はデータ行8件・3列・うち
  # 「判定不能:」が3件のため、規則の数は 8 - 3 = 5 件になるはずである。
  # 固定 $5 のままなら除外が効かず 8 件のまま報告される。
  local doc_writing="${TOOLDEFINED_DIR}/document-writing.md"
  if [ -f "$doc_writing" ]; then
    local dw_count
    dw_count="$(count_rules "$doc_writing")"
    if [ "$dw_count" = "5" ]; then
      echo "  [PASS] ケース11（実データ document-writing.md）: 規則5件"
    else
      echo "  [FAIL] ケース11（実データ document-writing.md）: 期待 規則5件 実際 規則${dw_count}件" >&2
      rc=1
    fi
  else
