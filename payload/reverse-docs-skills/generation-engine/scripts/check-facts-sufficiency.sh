...
#
# 契約:
#   facts.yml は delivery-payload/references/facts-schema.md の固定インデント (sections 配下キー=2スペース、
...
self_test() {
  if ! tmp="$(mktemp -d "${TMPDIR:-/tmp}/check-facts-sufficiency-self-test.XXXXXX" 2>/dev/null)" || [ -z "$tmp" ]; then
