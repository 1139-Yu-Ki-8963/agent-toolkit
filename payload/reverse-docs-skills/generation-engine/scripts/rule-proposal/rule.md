...
`generation-engine/scripts/rule-proposal/` 配下のスクリプトの定義ドキュメント。設計の定義は
`generation-engine/samples/規約提案/サンプルアプリAPI規約提案.html`（意匠の完成形）と
`delivery-payload/templates/rule-proposal/rule-proposal-template.html`（テンプレート）とする。
本ファイルは新規のシェルスクリプトには設計判断の記載が要るという要求に応えるための
...
起こしやすく、`generation-engine/scripts/unit-list/build-screen-list.sh` 等の既存ビルダーと
...
`delivery-payload/templates/rule-proposal/rule-proposal-template.html` を同時に更新する。
...
- `delivery-payload/templates/rule-proposal/rule-proposal-template.html` — テンプレート本体
- `generation-engine/samples/規約提案/サンプルアプリAPI規約提案.html` — 意匠の完成形サンプル
- `generation-engine/samples/規約提案/サンプルアプリAPI規約提案-data.json` — 上記サンプルの入力データ
- `delivery-payload/references/規約定義と派生生成の設計.md` — 取り込み後（判定結果JSON以降）の変換設計
