...
この文書は公開同期の対象外である。同期対象は <配布リポジトリ> の `scripts/<配布リポジトリの同期定義>` が定める。このリポジトリについては次の 5 つに限定されている。
...
同期はこの列挙に載ったパスだけを対象とする仕組みであり、リポジトリ直下の新規 `.md` は対象に入らない。除外設定の追加は不要である。公開手順を定める配布対象外の規約が持つ比較対象にも入らない。
...
| リポジトリ名を改称する | 持ち主の判断により対象外。公開先パス（`<配布先ペイロード>/`）の同期設定の変更を伴わない |
...
- 定義のコミット、`<配布先ペイロード>/` への同期、公開先の統合までを完了させる。手順の定義は `CLAUDE.md` と `RUNBOOK.md` である
