...
既にあるコードから読み取った内容をもとに用語の候補を挙げたいとき、旧い名前のスキルとして呼ばれたとき。

## いつ使わないか

承認済みの用語を更新・公開するとき、用語辞書のページを作るとき。
...
2. `<proposal_output_ref>.diagnostics.json`: 書き起こし元の欠落、未走査層、未解決候補、抽出件数を記録する診断。
...
5. 正式用語は補完しない。根拠不足、書き起こし元欠落、未走査層をdiagnosticsへ記録する。
...
1. `../../../generation-engine/schemas/semantic-glossary/1.0.0/proposal.schema.yaml`に沿って提案を組み立てる。
...
4. headlessでも自動承認・自動公開はせず、候補の扱いを自動的に上げることもしない。
...
- ガイド: `references/guide.html`
- proposal schema: `../../../generation-engine/schemas/semantic-glossary/1.0.0/proposal.schema.yaml`
- 連携仕様: このリポジトリ自身の設計文書（配布対象外）に記載

## 完了報告

`../../../delivery-payload/references/完了報告の書き方.md`の作業報告型に従う。
