...
確定済み facts（facts.yml）とプロジェクト共通文書だけを情報源に、リバース設計書（画面詳細設計書・DESIGN.md・画面テスト設計書・画面単体テスト設計書）を著述する執筆専任スキル。原本コードの読解は上流スキル extracting-unit-facts-from-code が担い、本スキルはその成果物（facts_ref）を受け取って書くだけの役割に限定する。**本スキル実行中に対象リポジトリの原本コードを Read することは全面禁止**（原本を見ないという原則を壊す契約違反）である。
...
画面単位のテスト成果物は、テスト設計/画面テスト設計書.md（1画面の操作・表示）と画面単体テスト設計書.md（関数・メソッド単位）の2文書、および画面だけが持つ操作シナリオ仕様書.mdとする。観点とケースは各設計書へ統合し、旧観点表・旧仕様書4件は生成しない。複数画面・機能をまたぐ結合テストはプロジェクト全体の `docs/test-cases/結合テスト仕様書.md` が担う。
...
| artifacts | 画面詳細設計書・DESIGN.md・画面テスト設計書・画面単体テスト設計書・操作シナリオ仕様書のパス |
...
facts.yml を情報源に、著述工程はテスト設計書2点（画面テスト設計書.md・画面単体テスト設計書.md）の「テスト観点」と「テストケース一覧」、および画面固有の操作シナリオ仕様書.mdの「シナリオ一覧表」を記入する責務を負う。各ケースは同じ文書内の観点キーと1:1または1:多で対応させ、キーは連番禁止（意味キー規約）。

- 画面テスト設計書・画面単体テスト設計書: 各観点キーについて、facts.yml から読み取れる具体的な入力値・操作・期待結果を同じ文書のケース一覧へ記入する。画面テスト設計書では1画面内の手動観点と往復検証観点も記入し、複数画面・機能・APIをまたぐ観点は `docs/test-cases/結合テスト仕様書.md` へ移す。facts.yml から確定できないケースは空行のまま残さず、根拠付き「該当なし」または要確認事項一覧への計上のいずれかで扱う
