...
完了条件: `check-fact-coverage.sh` と `audit-consistency.sh` がともに exit 0・§16 の measurement_pending 計上数（mp-接頭辞キー）が返却ブロック `measurement_pending[]` の件数と一致
...
| Phase 5 | `check-fact-coverage.sh` が exit 0 かつ `audit-consistency.sh` 違反 0 件・§16 measurement_pending 計上数と `measurement_pending[]` 件数が一致 |
