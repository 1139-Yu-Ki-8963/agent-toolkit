...
完了条件: `check-fact-coverage.sh` と `audit-consistency.sh` がともに exit 0・§16 の measurement_pending 計上数（mp-接頭辞キー）が返却ブロック `measurement_pending[]` の件数と一致・frontmatter の実測委譲プレースホルダ検査（`grep -c 実測委譲` が `0`）通過・（prefill-design-from-facts.sh 使用時のみ）`check-prefill-markers.sh` が exit 0・scenarios-input-check.jsonがPASS
