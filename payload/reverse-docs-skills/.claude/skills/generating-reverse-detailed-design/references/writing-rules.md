...
facts.yml から設計書への転記を担当する際の執筆規律の定義。転記先の判定条件・字面転記と要約の境界・実測委譲の書式・禁止事項を定める。12分類のキー付け規則自体は `delivery-payload/references/facts-schema.md` が定義元であり、本ファイルでは重複定義しない。

## 章マップ（章役割キー）準拠の転記先決定

転記先の章は「章の役割キー → §番号」の対応表である起動引数 chapter_map_path（実体: `delivery-payload/references/chapter-map.md`）で解決する。SKILL.md・本ファイルに書く §番号はすべて既定値であり、実際の設計書の前付け（frontmatter）の `chapter_map` が優先する。§番号が食い違う場合は設計書側の `chapter_map` を正とする。対応表は本文の節として書かない（改善課題1-276。顧客向けの本文に生成器の語彙を出さない）。読者向けの本文は必ず §1 画面概要から開始する。
...
画面自身が描画せずテンプレートエンジンへ委譲する構成の場合にのみ値を埋める。委譲しない画面は本見出しと空表を「非該当項目の章単位集約」の手順に従って削除し、その非該当理由に特定の実装方式を前提とした語彙を用いない。

### §16 要確認事項一覧の記入規律

状態列は「未解消」または「解消済み」の固定キーワードのみを書く。`audit-consistency.sh` の検査 i がこの列を判定に使うため、他の表現に言い換えない。
...
facts.yml の `meta` 節は以下の通り frontmatter へ転記する（`delivery-payload/references/facts-schema.md` の `meta` 定義に対応）。
...
`delivery-payload/references/gold-standard/docs/` 配下の正解設計書が存在する場合、各章の執筆直前に同章の gold 版を見本として参照してよい。ただし以下の制約を守る:
