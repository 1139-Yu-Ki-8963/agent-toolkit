...
転記先の章は「章の役割キー → §番号」の対応表である起動引数 chapter_map_path（実体: `delivery-payload/references/chapter-map.md`）で解決する。SKILL.md・本ファイルに書く §番号はすべて既定値であり、実際の設計書の章マップ表（本文末尾の付録B）が優先する。§番号が食い違う場合は設計書側の章マップ表を正とする。読者向けの本文は必ず §1 画面概要から開始し、関連資料の役割分担表と章マップを本文より前へ置かない。
