...
#
# 契約:
#   - facts.yml（delivery-payload/references/facts-schema.md 準拠。sections配下9キーの固定インデント構造）の
...
# 固定インデント契約（delivery-payload/references/facts-schema.md）:
...
check_screen_capture_templates() {
  ...
  basic="$repo_root/delivery-payload/templates/リバース検証/画面/基本設計/画面基本設計書.md"
  detail="$repo_root/delivery-payload/templates/リバース検証/画面/詳細設計/画面詳細設計書.md"
