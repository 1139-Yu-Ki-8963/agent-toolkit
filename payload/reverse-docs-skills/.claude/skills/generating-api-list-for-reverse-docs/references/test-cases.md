...
| ディスパッチ解決-ハンドラをfilesへ | ディスパッチ表を持つ構成がある | Phase 2の抽出を実行する | `files` にハンドラの実装モジュールが含まれ、`sourceFile` がハンドラのモジュールを指す | 手動（合成フィクスチャを要する） |
| ディスパッチ解決-未解決時のlow維持 | ディスパッチ表を解けない構成がある | Phase 2の抽出を実行する | 入口モジュールが記録され `confidence` が `low` のまま残る | 手動（合成フィクスチャを要する） |
| ディスパッチ解決-集中の検知 | `files` が全ユニットで同一ファイルへ集中している構成がある | Phase 2の抽出を実行する | 解決を試みずに確定させない | 手動（合成フィクスチャを要する） |
