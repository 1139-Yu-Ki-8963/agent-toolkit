...
| 調査書の判定 | 意味 | 動作 |
|---|---|---|
| API は実在しない | API を持たないプロジェクト（フロントエンドのみのサブプロジェクト等） | `一覧/API一覧（該当なし）.md` を判定理由の転記付きで生成し、status=`NONE` で正常終了する。呼び出し元は excluded-kinds.json の excludedKinds へ api を記録する |
...
- `artifacts`: `DONE` の場合は生成した API一覧.html のパス、`NONE` の場合は `一覧/API一覧（該当なし）.md` のパス
