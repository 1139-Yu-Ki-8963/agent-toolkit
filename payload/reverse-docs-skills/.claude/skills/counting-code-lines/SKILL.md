...
「コード行数」「行数を測って」「コードを数えて」のように行数の計測を頼まれたとき、または入口のページを作る作業で計測結果が必要なとき。

## いつ使わないか

計測結果のファイル（code-metrics.json）が既にあり、測り直す必要がないとき。
...
| **Goal** | `generation-engine/scripts/validate-code-metrics.sh "$output_dir/code-metrics.json"` の終了コードが 0 であること。終了コード 0 は total/fe/be/file_count/tests/commit/previous/scanScope/unclassified/testDetectionFailed の値が妥当であることの機械的な合格判定であり、自然文の自己申告に代える |
...
- `generation-engine/scripts/validate-code-metrics.sh "$output_dir/code-metrics.json"` の終了コードが 0

## 設計判断

### generation-engine/scripts/validate-code-metrics.sh
...
詳細は `generation-engine/scripts/validate-code-metrics.sh` 本体のヘッダコメントを参照する。
