...
- ハンドラの実装モジュールが `files` に含まれていない場合も、読んだ内容を根拠として記録する。あわせて `files` の不足を §10 要確認事項へ記録し、一覧生成側の再実行が要ることを残す
...
| 項目 | 値 |
|---|---|
| 出力フォルダ | `<output_dir>/API/api-<API 識別子>/詳細設計/` |
| 出力ファイル | `API詳細設計書.md` |
...
`unitKey` をディレクトリ名に使う場合、パス区切り（`/`）と制御文字をそのまま使わない。含む場合はハイフンへ置き換えて使い、置換した事実を当該設計書の §10 要確認事項へ記録する。
...
- **Step 1**: マニフェストの `files` が指すファイルを Read する。`sourceFile` を起点に、そこから呼ばれる関数の定義元まで追う。追跡は 2 段までとし、それ以上は §10 要確認事項へ記録する。`files` と `sourceFile` が入口モジュール 1 本へ集約されている場合は、そのモジュール内のディスパッチ表から当該エンドポイントのハンドラを引く。引いたハンドラの実装モジュールを読解対象へ加える。この解決は追跡の 2 段とは別枠で数え、ハンドラへ到達した地点を 1 段目とする。完了条件: 読解対象のファイル一覧が確定済み
