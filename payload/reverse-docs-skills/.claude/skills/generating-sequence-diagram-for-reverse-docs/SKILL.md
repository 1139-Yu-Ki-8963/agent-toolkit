...
```json
[{"failure_return_to":"orchestrating-reverse-docs-flow","id":"sequence-diagram","inputs":["unit facts"],"outputs":["画面/<unit>/シーケンス図.html"],"stop_conditions":["呼出根拠なし"],"validator":"check-delivery-artifacts.sh"}]
```
