...
orchestrating-ai-development-setup から起動される時、画面数が多くまとめて無人で処理するのが適切な時に使う。

## いつ使わないか

画面が3件以下（通常のセッションで1件ずつ処理する方が速い）の時、個別画面の手動検証を行う時は使わない。
...
本スキルは orchestrating-ai-development-setup から args 全量指定で起動される。AskUserQuestion を発行しない（対話ゼロ契約）。
...
- TRIGGER: orchestrating-ai-development-setup の画面バッチ Phase から起動される時。画面数が4件以上で一括無人処理が適切な時
...
| template_root | 必須 | テンプレートディレクトリパス（delivery-payload/templates/リバース検証/画面/） |
...
| model | 任意 | claude -p に渡すモデル名。既定: claude-sonnet-5（事実確定・往復検証は分析・判断を伴い Haiku では精度不足のため） |
...
- 大量呼び出し時は `--no-session-persistence` で会話記録の肥大化を防ぐ
...
screen_ids が "all" の場合は画面一覧の永続マニフェスト（`<output_dir>/<manifestsRoot>/screen-manifest.json` の `screens[]`。`manifestsRoot` は output-layout の物理配置キー）から全画面IDを抽出する。HTML埋め込みマニフェストは復元用の副次経路であり、展開元には使わない。指定リストの場合はそのまま使う。1行1画面IDのテキストファイル（`<output_dir>/batch-targets.txt`）に書き出す。既検証画面（レジストリで status=baseline-established）をカウントし、未検証件数を確認する。
...
`docs-only` は1画面につき前半の claude -p を1回だけ呼び出し、静的リバースを `STATIC_COMPLETE` として終端する。`single-pass|iterative` は原本を見ない分離（原本コードを読む工程と設計書のみで判定する工程を別プロセスに分離する要件）を満たすため、前半・後半の2回に分けて呼び出す。前半と後半は別プロセスで実行されるため、前半で読んだ原本コードの情報が後半のコンテキストに混入しない。

### 前半（1回目 claude -p）: 静的リバース著述

1. **事実確定**: 画面の開通有無にかかわらず、対象ファイルの facts を抽出・確定する（extracting-unit-facts-from-code）
2. **著述モード判定**: orchestrating-ai-development-setup の契約どおり対象行数・ファイル数から `standard|large-two-pass` を決める
3. **標準著述**: standard は基本設計（authoring_pass=standard）と詳細設計（authoring_pass=full）を著述する
4. **大規模2パス著述**: large-two-pass は詳細設計（detail-only）の `DETAIL_AUTHORED` を先に確認し、その後のパス2で基本設計（large-pass2）と観点表・テスト仕様書（companion-docs）を著述する
5. **静的完了マーカー**: 著述モードごとの全完了statusを確認した管理プロセスがレジストリを `status=authored` に更新する
...
### 後半（2回目 claude -p）: ファイル単位原本を見ない検証・往復検証

前半の動的開通完了（`status: unlocked`）を前提条件として開始する。`authored` のままなら画面開通または frontmatter 完全性ゲートが未完了なので後半へ進まない。原本コードは一切読まない（原本を見ない検証）。

1. **ファイル単位原本を見ない検証**: rebuilding-screen-unit-from-docs で対象ファイルを白紙化し設計書のみから再現する（無人モードでは任意工程ではなく必須工程）
...
**per-item prompt の必須文言**: 無人モードではいかなる工程・用途でも Agent ツールのバックグラウンド起動を使用してはならない（正本は `orchestrating-ai-development-setup` の `references/contract.md` の「無人モード仕様」）。前半・後半それぞれの per-item prompt（`references/loop-design.md` §4 のテンプレート）は、この禁止文言をプロンプト本文へ逐語で含めることを必須とする。プロンプト内で Skill ツール起動を指示する記述と併記し、claude -p が起動する側のサブエージェントにも同じ制約を確実に伝播させる。

## マーカー仕様

- **中間マーカー判定（前半完了）**: 画面レジストリ（`<output_dir>/<screenRegistry>`）内の当該画面エントリで `status` が `authored`
- **完了マーカー判定（後半完了）**: 同エントリで `status` が `baseline-established`
- **CHECK_CMD**: 画面レジストリファイルを読み、`<system>-<screen_id>:` キー（正本: `orchestrating-ai-development-setup` の `references/contract.md` 「画面レジストリ」節。system は `target_repo_path` のディレクトリ名から導出する）のブロック内に `status: baseline-established` が存在するかを grep で判定する（`verification-pass` は廃止済み。旧値が残る台帳の読み替え手順は `orchestrating-ai-development-setup` の `references/contract.md` の「レジストリ移行手順」を参照）
...
- 1画面あたりの処理時間は実測ベースで前半・後半合わせて60分程度を見込む（事実確定・往復検証は分析・判断を伴い時間を要するため、初期見積りの5〜15分から実測値へ改めた）
...
- per-item prompt を orchestrating-ai-development-setup の画面処理パイプライン（前半: 事実→著述モード判定→標準著述または大規模2パス→必要時のみ開通／後半: ファイル単位原本を見ない検証→基準確立→往復検証）に特化。前半・後半を別 claude -p 呼び出しに分離し原本を見ない分離を満たす
