...
| lane_id | 任意 | レーン識別子。複数レーン並列起動時に指定。未指定なら単一レーン運転。ログは `batch-log-<lane_id>.txt`、failed リストは `failed-screens-<lane_id>.txt` にレーン別分離 |
...
- **ログ・failed リストの分離**: レーン別に `batch-log-<lane_id>.txt` / `failed-screens-<lane_id>.txt` を使用。完了報告時にレーン別結果を統合する
