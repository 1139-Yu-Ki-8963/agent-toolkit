...
盲検分離（正本は `orchestrating-ai-development-setup` の `references/contract.md` の「無人モード仕様」の「盲検分離の必須要件」）を満たすため、1画面につき `claude -p` を前半・後半の2回に分けて呼び出す。前半完了でレジストリ `status` が `authored` に、後半完了で `baseline-established` になる。
...
# レジストリのマップキーは `<system>-<screen_id>:`（正本: orchestrating-ai-development-setup の
...
1. 対象画面の著述パイプラインを以下の順に全て実行する:
   ...
   - 対象ファイルの合計行数・ファイル数を実測し、orchestrating-ai-development-setup の契約どおり authoring_mode を判定する
