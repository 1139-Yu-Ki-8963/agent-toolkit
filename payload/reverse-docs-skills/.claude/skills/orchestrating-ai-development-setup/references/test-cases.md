# orchestrating-ai-development-setup テストケース
...
| 画面バッチ経路-4件以上で委譲 | 対象画面が4件以上ある | Step 4-4完了後に経路を選ぶ | running-reverse-screen-batchへglobal Step 17〜28・40〜41を委譲する | 手動 |
...
| APIのみ-テスト派生一覧を連続起動 | APIの2テスト設計書だけが存在し画面が存在しない | Step 4の派生一覧工程を実行する | テスト観点表とテストケース一覧を続けて起動し、両方のHTMLを生成する | 一時入力による結合確認 |
