...
本書は `orchestrating-ai-development-setup/SKILL.md` から段階的開示した運用契約である。統括は Phase 1 の開始前に本文書を Read で全文読み込み、報告・委任・タスク・ループの書式を適用する。工程順序の正本は `delivery-payload/references/リバース工程設計.md`、返却値と状態の正本は `contract.md` とする。
...
| table | 実在しない（理由: …） | `<unitListAbsentMd>`（{label}=テーブル。既定値 `docs/manifests/テーブル一覧（該当なし）.md`） | 基本設計生成済み |
| batch | 実在しない（理由: …） | `<unitListAbsentMd>`（{label}=バッチ。既定値 `docs/manifests/バッチ一覧（該当なし）.md`） | 基本設計生成済み |
| report | 実在しない（理由: …） | `<unitListAbsentMd>`（{label}=帳票。既定値 `docs/manifests/帳票一覧（該当なし）.md`） | 基本設計生成済み |
| external | 実在しない（理由: …） | `<unitListAbsentMd>`（{label}=外部連携。既定値 `docs/manifests/外部連携一覧（該当なし）.md`） | 基本設計生成済み |
...
| global Step 29 | generating-api-detail-design-for-reverse-docs | target_repo_path, output_dir, api-manifest.ext.json由来のAPI一覧 | API詳細設計著述完了 |
| global Step 30 | generating-feature-design-for-reverse-docs | target_repo_path, output_dir, feature-manifest.json由来の機能一覧 | 機能設計著述完了 |
| global Step 31 | generating-api-basic-design-for-reverse-docs | target_repo_path, output_dir, api-manifest.ext.json由来のAPI一覧 | API基本設計著述完了 |
| global Step 32 | generating-table-logical-model-for-reverse-docs | target_repo_path, output_dir, table-manifest.ext.json由来のテーブル一覧 | 論理データモデル著述完了 |
| global Step 33 | generating-table-definition-for-reverse-docs | target_repo_path, output_dir, table-manifest.ext.json由来のテーブル一覧 | テーブル定義書著述完了 |
| global Step 34 | generating-batch-basic-design-for-reverse-docs | target_repo_path, output_dir, batch-manifest.ext.json由来のバッチ一覧 | バッチ基本設計著述完了 |
| global Step 35 | generating-batch-detail-design-for-reverse-docs | target_repo_path, output_dir, batch-manifest.ext.json由来のバッチ一覧 | バッチ詳細設計著述完了 |
| global Step 36 | generating-report-basic-design-for-reverse-docs | target_repo_path, output_dir, report-manifest.ext.json由来の帳票一覧 | 帳票基本設計著述完了 |
| global Step 37 | generating-report-detail-design-for-reverse-docs | target_repo_path, output_dir, report-manifest.ext.json由来の帳票一覧 | 帳票詳細設計著述完了 |
| global Step 38 | generating-external-basic-design-for-reverse-docs | target_repo_path, output_dir, external-manifest.ext.json由来の外部連携一覧 | 外部連携基本設計著述完了 |
| global Step 39 | generating-external-detail-design-for-reverse-docs | target_repo_path, output_dir, external-manifest.ext.json由来の外部連携一覧 | 外部連携詳細設計著述完了 |
| global Step 40 | unlocking-reverse-target-screens / syncing-reverse-env / rebuilding-screen-unit-from-docs / rebuilding-code-from-docs | 動的検証に必要なargs全量 | UNLOCKED / PASS / 再現一致 / NEED-COMPARE |
| global Step 41 | rebuilding-code-from-docs / syncing-reverse-env | mode=judge と compare_result / mode=syncまたはteardown | PASS/FAIL |
...
- screen-batch-route 選択時はglobal Step 17〜28・40〜41を「条件付きStep: 画面バッチ実行」1タスクに集約し、新しいPhase番号を作らない
- 差し戻し発生時は差し戻し先工程を新規 TaskCreate で末尾に追加（既存タスクの状態は変更しない）
- headless=true 時もタスク一覧は同じ形式で生成する（進捗の可視化用途。実行制御は per-item prompt が担う）
- global Step 29〜39 は画面ループの外で1回だけ実行する非画面の工程であり、画面IDを持たない。画面バッチへの委譲の対象外とする。

## ループ設計

| 要素 | 内容 |
|---|---|
| 反復条件 | `verification_mode=iterative` かつ global Step 41 の judge が FAIL → Back-edgeメタデータで分類し、共通文書欠落はglobal Step 14、facts欠落はglobal Step 18、著述不足はglobal Step 26へ戻す |
...
|---|---|---|---|
| screen-batch-route | global Step 16 | 対象画面4件以上 | running-reverse-screen-batchへglobal Step 17〜28・40〜41を委譲 |
| screen-batch-route | global Step 16 | 対象画面3件以下 | 本スキルがglobal Step 17〜28・40〜41を逐次仲介 |
| docs-only-terminal | global Step 28 | verification_mode=docs-only | global Step 29〜39の完了後、静的完了で終端しglobal Step 40〜41を起動しない |
| dynamic-route | global Step 28 | verification_mode=single-pass または iterative | global Step 29〜39を経てglobal Step 40へ進む |
...
**保守責任者**: 人手（ユーザー）。テンプレートのプレースホルダや一覧HTMLのJSON構造を変更する場合は build-portal.sh と portal-template.html を同時に更新する。METRICS_JSON は構造化オブジェクト形式である。形式変更時はテンプレート（portal-template.html）とスクリプト（build-portal.sh）を同一コミットで同時更新する。トークンブロックは portal-template.html と detail-pages テンプレ4本（`delivery-payload/templates/detail-pages/`）で複製している。色定義・テーマ切替を変更する場合は両方を同時に更新する
...
### resolve-flow-state.sh

**必要性**: 改善課題 1-122 が指摘するとおり、16状態の判定は SKILL.md・contract.md の自然文手順のみに依存しており、判定の正しさを確認する手段がない。`resolve-flow-state.sh` は状態判定表と1→16降順の判定フローを1本のスクリプトへ固定し、標準出力の状態キー1行を機械的に返す。16状態それぞれの合成フィクスチャに対する `--self-test` を持ち、状態判定ロジックの変更をリグレッションとして検知できる。

**代替案を採用しなかった理由**:
- Bash ツール直叩き: 16状態の判定条件（ファイル実在・機械ゲート再実行・YAML/JSON読み取り）を毎回手動で追跡すると条件の判定順序がぶれ、リグレッションを自己テストで検知できない
- 既存 Makefile ターゲット拡張: reverse-docs-skills リポジトリに Makefile は存在せず、新規導入は本チェック専用の依存を増やすだけになる
- package.json scripts 追加: 同リポジトリはビルド設定を持たない

**保守責任者**: 人手（ユーザー）。`contract.md` の状態判定表（実在判定列）を変更した場合は、本スクリプトの対応する `check_*` 関数と `--self-test` のフィクスチャを同時に更新する。

**廃棄条件**: orchestrating-ai-development-setup スキル自体が廃止された時、または状態判定をビルド基盤が標準で提供するようになった時
