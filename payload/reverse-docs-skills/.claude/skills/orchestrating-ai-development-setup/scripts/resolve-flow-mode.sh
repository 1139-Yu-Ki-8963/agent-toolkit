...
#
# 必要性: このリポジトリ自身の開発構想文書が定める Phase B（統括-モード分岐）は、コードを持たない
...
# 廃棄条件: orchestrating-ai-development-setup の入口モード分岐が別基盤へ置き換えられた時。
#
# Usage:
...
#
# 判定の材料（優先順）:
#   1. --output-dir が指定され、`<output_dir>/<excludedKinds>`（output-layout.json の excludedKinds
#      キー。既定値 `docs/scope-and-progress/excluded-kinds.json`）が実在する場合、presentKinds に
#      "screen" が含まれるかで画面の有無を判定する（surveying-architecture-for-reverse-docs
#      が返す unit_kinds_present をそのまま永続化した値）。
#   2. --output-dir が指定され、`<output_dir>/<commonRoot>/アーキテクチャ調査書.md`（output-layout.json の
#      surveyDoc キー）が実在し
...
sites_from_survey() {
  ...
  local layout_json survey_doc
  layout_json="$(resolve_output_layout "$output_dir")" || return 0
  survey_doc="$output_dir/$(output_layout_get "$layout_json" surveyDoc)"
...
determine_mode_for_scope() {
  ...
  if [ -n "$output_dir" ]; then
    ...
    layout_json="$(resolve_output_layout "$output_dir" 2>/dev/null)" || layout_json=""
    if [ -n "$layout_json" ]; then
      excluded_json="$output_dir/$(output_layout_get "$layout_json" excludedKinds)"
    else
      excluded_json="$output_dir/一覧/excluded-kinds.json"
    fi
...
TS
  ...
  local out6_layout out6_excluded_rel out6_excluded_path
  out6_layout="$(resolve_output_layout "$out6dir")"
  out6_excluded_rel="$(output_layout_get "$out6_layout" excludedKinds)"
  out6_excluded_path="$out6dir/$out6_excluded_rel"
  mkdir -p "$(dirname "$out6_excluded_path")"
  cat > "$out6_excluded_path" <<'JSON'
{
