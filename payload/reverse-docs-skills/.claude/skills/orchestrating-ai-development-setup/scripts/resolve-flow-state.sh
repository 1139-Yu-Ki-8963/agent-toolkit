...
# resolve-flow-state.sh — orchestrating-ai-development-setup の16状態判定を決定的に固定する
#
...
# 廃棄条件: orchestrating-ai-development-setup スキル自体が廃止された時、または状態判定をビルド基盤が
#   標準で提供するようになった時。
#
...
check_facts_unsealed() {
  ...
  local seal_script="$REPO_ROOT/generation-engine/scripts/seal-facts.sh"
...
self_test() {
  ...
  local COMMON_ROOT SURVEY_DOC COMMON_DESIGN_DOC MESSAGE_DOC DESIGN_DOC \
    FOUNDATION_DOC_PATH UI_COMMON_DOC DATA_DESIGN_DOC EXCLUDED_KINDS_PATH SCREEN_REGISTRY_PATH
  COMMON_ROOT="$(output_layout_get "$LAYOUT_JSON" commonRoot)" || return 1
  SURVEY_DOC="$(output_layout_get "$LAYOUT_JSON" surveyDoc)" || return 1
  COMMON_DESIGN_DOC="$(output_layout_get "$LAYOUT_JSON" commonDesignDoc)" || return 1
  MESSAGE_DOC="$(output_layout_get "$LAYOUT_JSON" messageDoc)" || return 1
  DESIGN_DOC="$(output_layout_get "$LAYOUT_JSON" designDoc)" || return 1
  FOUNDATION_DOC_PATH="$(output_layout_get "$LAYOUT_JSON" foundationDoc)" || return 1
  UI_COMMON_DOC="$(output_layout_get "$LAYOUT_JSON" uiCommonDoc)" || return 1
  DATA_DESIGN_DOC="$(output_layout_get "$LAYOUT_JSON" dataDesignDoc)" || return 1
  EXCLUDED_KINDS_PATH="$(output_layout_get "$LAYOUT_JSON" excludedKinds)" || return 1
  SCREEN_REGISTRY_PATH="$(output_layout_get "$LAYOUT_JSON" screenRegistry)" || return 1
  ...
  mkdir -p "$docs/$COMMON_ROOT"
  cat > "$docs/$SURVEY_DOC" <<'MD'
# アーキテクチャ調査書
...
MD
  ...
  local list_label list_dir
  for list_label in 画面 API テーブル バッチ 帳票 外部連携; do
    list_dir="$(output_layout_get "$LAYOUT_JSON" unitListDir "$list_label")" || return 1
    mkdir -p "$docs/$list_dir"
  done
  mkdir -p "$(dirname "$docs/$EXCLUDED_KINDS_PATH")"
  cat > "$docs/$EXCLUDED_KINDS_PATH" <<'JSON'
{
...
JSON
  local screen_list_html
  screen_list_html="$(output_layout_get "$LAYOUT_JSON" unitListHtml "画面")" || return 1
  : > "$docs/$screen_list_html"
  local absent_label absent_md
  for absent_label in API テーブル バッチ 帳票 外部連携; do
    absent_md="$(output_layout_get "$LAYOUT_JSON" unitListAbsentMd "$absent_label")" || return 1
    mkdir -p "$(dirname "$docs/$absent_md")"
    : > "$docs/$absent_md"
  done
  assert_state "共通未採録" "状態3 共通未採録"

  # 状態4: ポータル未生成（6文書を追加）
  : > "$docs/$COMMON_DESIGN_DOC"
  : > "$docs/$MESSAGE_DOC"
  : > "$docs/$DESIGN_DOC"
  : > "$docs/$FOUNDATION_DOC_PATH"
  : > "$docs/$UI_COMMON_DOC"
  : > "$docs/$DATA_DESIGN_DOC"
...
key: value
YML
  bash "$REPO_ROOT/generation-engine/scripts/seal-facts.sh" seal "$facts_dir" >/dev/null 2>&1
  ...
  mkdir -p "$(dirname "$docs/$SCREEN_REGISTRY_PATH")"
  cat > "$docs/$SCREEN_REGISTRY_PATH" <<YML
${scope}:
  source_ref: abc123
  verification_url: http://localhost:3000/target
  design_doc_path: $SCREEN_UNIT_ROOT/screen-$SCREEN_ID_FIXED/詳細設計/画面詳細設計書.md
  status: authored
YML
...
MD
  ...
  mkdir -p "$undetermined_docs/$COMMON_ROOT"
  cp "$docs/$SURVEY_DOC" "$undetermined_docs/$SURVEY_DOC"
  for list_label in 画面 API テーブル バッチ 帳票 外部連携; do
    list_dir="$(output_layout_get "$LAYOUT_JSON" unitListDir "$list_label")" || return 1
    mkdir -p "$undetermined_docs/$list_dir"
  done
  mkdir -p "$(dirname "$undetermined_docs/$EXCLUDED_KINDS_PATH")"
  cp "$docs/$EXCLUDED_KINDS_PATH" "$undetermined_docs/$EXCLUDED_KINDS_PATH"
  : > "$undetermined_docs/$screen_list_html"
  for absent_label in API テーブル バッチ 帳票 外部連携; do
    absent_md="$(output_layout_get "$LAYOUT_JSON" unitListAbsentMd "$absent_label")" || return 1
    mkdir -p "$(dirname "$undetermined_docs/$absent_md")"
    : > "$undetermined_docs/$absent_md"
  done
  mkdir -p "$undetermined_docs/$COMMON_ROOT"
  : > "$undetermined_docs/$COMMON_DESIGN_DOC"
  : > "$undetermined_docs/$MESSAGE_DOC"
  : > "$undetermined_docs/$DESIGN_DOC"
  : > "$undetermined_docs/$FOUNDATION_DOC_PATH"
  : > "$undetermined_docs/$UI_COMMON_DOC"
  : > "$undetermined_docs/$DATA_DESIGN_DOC"
