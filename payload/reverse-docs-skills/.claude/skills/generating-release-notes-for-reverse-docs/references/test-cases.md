...
| changes種別-6値の部分集合検査 | 全コミットのchanges.type分類が確定済み | Phase 2 Step 4のjq検査（changes[].type版）を実行する | 出力される値がfeat/fix/docs/test/refactor/choreのいずれかのみになる | 手動 |
| グループflow-feature優先 | 同日付グループにfeature分類のコミットとmaintenance分類のコミットが混在する | Phase 2 Step 3のグループflow判定を実行する | グループのflowがfeatureになる | 手動 |
| グループflow-全件docsのみdocs | 同日付グループの全コミットがdocs分類 | Phase 2 Step 3のグループflow判定を実行する | グループのflowがdocsになる | 手動 |
