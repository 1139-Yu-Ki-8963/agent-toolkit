...
**必要性**: アーキテクチャ調査書の品質保証（記載パスの実在・6種別の網羅・推測語の不在・テンプレ残存の不在）を、実行者・実行回ごとにブレる自然文の目視確認に委ねると、Stage 1の完了条件である「2プロジェクトでスキル無改造のまま調査書成立」を機械的に検証できない。5検査すべてを1本の決定的スクリプトへ固定化し、`exit code` のみで合否を判定することで、調査担当（人間・サブエージェント問わず）の記述品質を再現可能な基準で強制する。
