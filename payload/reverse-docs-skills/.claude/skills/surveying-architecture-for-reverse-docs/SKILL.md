...
**必要性**: §4のディレクトリ責務マップは後続の共通文書採録スキル（compiling-project-common-docs）が層化サンプリングの層定義として使用する。§4に記載されていないディレクトリは層化サンプリングから機械的に除外され、そのディレクトリ内の規約・定数等の事実が一切採録されずに後工程へ無言で伝播する。この欠落を調査書確定時に検出するため、§1のfindコマンドから走査範囲を決定的に再現し、直接ファイルを持つディレクトリの網羅性を機械検査する。走査範囲のパス・深度をスクリプトにハードコードせず、調査書自身に記録されたコマンドから導出することで、プロジェクト非依存性を維持する。
