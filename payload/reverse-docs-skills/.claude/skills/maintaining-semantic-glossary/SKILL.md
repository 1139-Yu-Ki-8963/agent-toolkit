...
用語の追加・更新・廃止・重複・類義語・禁止語・命名・未使用・影響調査を行いたい時に使う。

## いつ使わないか

解析候補の抽出だけを行う時、用語集を閲覧するだけの時は使わない。
...
本Skillは独立したCRUD runnerや用語保守CLIではない。AIがRead/Grep/Bashで定義YAML・候補・影響を調べ、Writeで変更要求を作る手順である。`scripts/` はSkillの実行入口ではなく契約検証用テストだけを置く。
...
正式validatorは `../../../generation-engine/scripts/glossary/validate-semantic-glossary.sh` を使う。
...
- 類義語を文字列距離だけで自動統合しない。
- 未使用検出が0件でも削除を提案しない。
- 候補の扱いを正式用語へ直接上げない。承認状態をファイル移動で代替しない。
...
- Write: dry-run reportと変更要求だけを作り、glossary定義を編集しない。

## 参照資料

- 共通契約: `../../../delivery-payload/references/semantic-glossary-contract-v0.1.md`
- 保守仕様・運用ルール: このリポジトリ自身の設計文書（配布対象外）に記載
