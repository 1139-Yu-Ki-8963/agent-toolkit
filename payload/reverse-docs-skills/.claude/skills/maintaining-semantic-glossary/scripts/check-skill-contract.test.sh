...
guide_file="$skill_dir/references/guide.html"
validator="$repo_root/generation-engine/scripts/glossary/validate-semantic-glossary.sh"
validator_py="$repo_root/generation-engine/scripts/glossary/validate-semantic-glossary.py"
projector="$repo_root/generation-engine/scripts/detail-pages/project-semantic-glossary.py"
fixtures="$repo_root/generation-engine/scripts/glossary/fixtures"
...
counts = {name: 0 for name in ("error", "warning", "review_required")}
for finding in report["findings"]:
    counts[finding["severity"]] += 1
    assert re.fullmatch(r"(?:SGS|SGK|SGR|SGL|SGP|SGV|SGD)_[A-Z0-9_]+", finding["code"])
assert counts == report["counts"]
if required_code:
    assert required_code in {item["code"] for item in report["findings"]}
PY
}
...
require '^name: maintaining-semantic-glossary$' 'name'
require '^invocation: maintaining-semantic-glossary$' 'invocation'
require '^type: orchestration$' 'type'
require 'dry-run|dry_run' 'dry-run default'
require 'add.*update.*deprecate.*retire' 'lifecycle operations'
require 'detect-duplicates' 'duplicate detection'
require 'detect-synonyms' 'synonym detection'
require 'detect-forbidden-terms' 'forbidden term detection'
require 'detect-naming-violations' 'naming validation'
require 'detect-unused-terms' 'unused term detection'
require 'analyze-impact' 'impact analysis'
require 'review-proposal' 'proposal review'
require '正式用語集.*直接編集せず|glossary.*直接編集せず' 'no direct glossary mutation'
require '自動統合しない' 'no automatic synonym merge'
require '物理削除しない' 'no physical deletion'
require 'human_check_required' 'human usage check'
require 'SGS.*SGK.*SGR.*SGL.*SGP.*SGV.*SGD' 'diagnostic family handling'
require 'ai_index.*review_required.*not_implemented|review_required.*not_implemented.*ai_index' 'ai_index rejection'
...
tmp="$(mktemp -d "${TMPDIR:-/tmp}/maintaining-semantic-glossary.XXXXXX")"
trap 'rm -rf "$tmp"' EXIT
...
assert not (report["status"] == "valid" and report["counts"]["review_required"] == 0)
PY
...
"$python_bin" - "$fixtures/valid-proposal.yaml" <<'PY'
import pathlib, sys, yaml
proposal = yaml.safe_load(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
assert proposal["proposal"]["approval"]["status"] == "detected"
assert proposal["proposal"]["approval"]["status"] not in {"approved", "merged"}
PY
cmp -s "$target" "$target_before"
...
printf 'PASS: maintaining-semantic-glossary CLI integration and static policy contract\n'
