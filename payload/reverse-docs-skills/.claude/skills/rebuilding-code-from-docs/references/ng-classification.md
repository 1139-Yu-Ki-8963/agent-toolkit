# NG 分類（検出シグナル → 失敗クラス → 章帰着）

Phase 8 で使う分類表。表 A は Phase 7（答え合わせ）・Phase 2（内部整合性監査）・Phase 5（発散確定）から得られる検出シグナルを起点に失敗クラスへ落とす。表 B は失敗クラスから設計書の一次帰着（役割・既定§）とテンプレート昇格条件へ落とす。

キーは連番禁止。内容を要約した意味語で付ける（番号からは情報を得られないため）。

## 表 A: 検出シグナル → 失敗クラス

| 検出シグナルキー | 由来 Phase | 判定内容 | 遷移先 |
|---|---|---|---|
| 静的実差分 | Phase 7（compare_result 静的比較） | ファイル diff に「実差分」として計上された行がある。独立リライトのため変数名・コメント差で恒常的に非ゼロになり、単独では NG の根拠にしない | NG 計上せず参考情報のみ（動的一致〔L2/L2'/L3/L4/該当時 L5〕と env_check が別途確認できていれば NG 扱いしない。動的層にも不一致がある場合はその不一致シグナル側で表 B へ） |
| L2構造差 | Phase 7（動的比較 L2） | Playwright 構造スナップショットが完全一致しない | 表 B の該当失敗クラスへ |
| L3画素差 | Phase 7（動的比較 L3） | 画素比較が config.yml の閾値を超える | 表 B の「スタイル数値差」へ |
| L4コンソール差 | Phase 7（動的比較 L4） | コンソールエラー集合が一致しない | 表 B の該当失敗クラスへ（多くは API 呼び出し条件・型差、イベント処理挙動差） |
| L5操作シーケンス差 | Phase 7（動的比較 L5） | 操作シーケンス実行後の postContent（tables/texts）が両環境で不一致 | 表 B の該当失敗クラスへ（多くは イベント処理挙動差・遷移方式・パラメータ差） |
| P2内部矛盾 | Phase 2（内部整合性監査） | 突合 1〜5 のいずれかで不一致が検出された | 表 B の該当失敗クラスへ。実装を経ずに直接指示書へ |
| P5発散エラー | Phase 5（自己完結チェック） | 同一エラーシグネチャが 3 連続で再発し発散確定 | エラー内容から表 B の該当失敗クラスを推定 |
| 描画未到達(両環境) | Phase 7（`compare_result.status`） | `status` が `DESIGN-INCOMPLETE`（両環境とも同様に render-ready 未到達。引数不足のスピナー等） | 表 B の「scenarios引数不足」へ |
| 動的検証不能 | Phase 7（`compare_result.status`） | `status` が `DYNAMIC-UNVERIFIED`（MCP・Playwright とも不在で動的検証不能） | 表 B 対象外。NG 計上せず最終報告の注記へ（PASS 扱いにしない） |

## 表 B: 失敗クラス → 一次帰着（役割・既定§） → テンプレ昇格条件

| 失敗クラス（意味キー） | 一次帰着（役割・既定§） | テンプレ昇格条件 |
|---|---|---|
| export-import-型不一致 | 実装契約（ファイル分割/型定義/依存。既定 §15.1/15.2/15.3） | 雛形に列不足なら昇格 |
| 状態変数欠落・初期値差 | 状態管理（メイン画面の状態変数/初期値。既定 §5.3/5.9） | 2 画面以上で再発 |
| 表示制御方式差（常時マウント vs 条件レンダー） | 画面構造/領域別仕様（表示制御方式。既定 §3.1/§9） | テンプレ改善候補 |
| イベント処理挙動差 | イベント処理（既定 §8） | 同上 |
| スタイル数値差 | DESIGN.md（画面構造章のスタイル適用パターン経由。既定 §3.6） | 正の分担違反として指摘 |
| 文言差 | メッセージ定義書（エラーハンドリング章の文言定義経由。既定 §11.2） | 同上 |
| API呼び出し条件・型差 | API通信（既定 §7） | — |
| 遷移方式・パラメータ差 | 画面遷移（既定 §12） | — |
| 定数値差 | 定数設定値（既定 §10） | — |
| 空状態・エラー状態差 | データフロー/エラーハンドリング（空状態/エラー状態。既定 §6.6/§11） | — |
| scenarios引数不足 | frontmatter（`scenarios[].query` / `path_params` / `ready`） | 2 画面以上で再発 |

## 使い方

1. Phase 7 の返却ブロック（`status` / `static_diff` / `dynamic` / `env_check` / `hint`）、Phase 2 の監査結果、Phase 5 の発散エラーを検出シグナルとして集める
2. 表 A で検出シグナルを失敗クラスへ写像する
3. 表 B で失敗クラスから一次帰着（役割・既定§）を決め、修正指示書（`references/report-format.md` 参照）の該当欄に記入する
4. テンプレ昇格条件に合致する場合、`references/test-item-patterns.md` に新規エントリを追記し「テンプレ反映状況: 未」で記録する
5. 「正の分担違反」（スタイル数値差・文言差）は、設計書本体ではなく `DESIGN.md` / メッセージ定義書側の記載不足として指摘する（章マップの正の宣言に従う）
6. `status` が `DYNAMIC-UNVERIFIED` の場合は表 A・表 B を経由せず、最終報告（`references/report-format.md`）に「動的未検証のため往復検証完了としない」旨を注記する
7. `status` の PASS/FAIL 判定は転用しない（SKILL.md Phase 7 参照）。`static_diff` の実差分行は動的層がすべて一致していれば NG 計上しない。動的層に不一致が無いのに `status=FAIL` なのは静的実差分のみが原因のケースであり、このときは PASS 相当として扱う
