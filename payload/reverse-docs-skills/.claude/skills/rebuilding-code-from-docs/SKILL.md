...
元のコードと設計書の一致を検証したい時に使う。

## いつ使わないか

通常の機能実装や環境の同期を行う時は使わない。
...
**画面単位（`<screen_dir>/../テスト設計/` 配下）**:
- 画面単体テスト設計書.md（関数・フック・メソッド・コンポーネント単位の観点とケース。必須）
- 画面結合テスト設計書.md（1画面内の操作・表示・領域間連携・API連携・状態切替の観点とケース。必須）
- 操作シナリオ仕様書.md（画面だけが持つ単一画面内の操作シナリオ。必須）
...
- 規約/ 配下の 4 ファイル（コーディング規約.md・命名規約.md・ディレクトリ構成規約.md・コンポーネント設計規約.md）。規約ファイルは frontmatter キーを持たず、`../../../規約/` を固定パスで探索する。存在するファイルをすべてロードし、存在しないファイルは「不在」と記録して続行する（規約ファイルは任意）
...
- `規約/コーディング規約.md` — コードスタイル・フォーマット
- `規約/命名規約.md` — ファイル名・変数名・コンポーネント名のパターン
- `規約/ディレクトリ構成規約.md` — ファイル配置のルール
...
**単体テストコードは自作しない**。単体テストの正本は上流 rebuilding-screen-unit-from-docs が持つ。起動引数 `saved_test_paths`（上流が設計書リポジトリ `<画面ディレクトリ>/テスト設計/テストコード/単体/<basename>/` 配下に保存済みの単体テストコード一覧）を受け取っている場合、そのテストコードを起動引数 `reverse_worktree`（本スキルが操作する対象コードリポジトリ。上流の起動引数 `target_repo_path` とは別スキルの別引数であり同一物ではない）の対応パス（`規約/ディレクトリ構成規約.md` に従う配置先）へ配置して実行する。単体テストコードの新規作成は行わない。

**代替経路（上流未実施画面）**: `saved_test_paths` が空または未提供の画面（基準未確立/往復未検証 開始でファイル単位検証をスキップした画面等、上流の単体テストが存在しない場合）に限り、`<screen_dir>/../テスト設計/画面単体テスト設計書.md` の各観点キーとケースからテストケースを導出して自作する。これは正規の代替経路だが、自作した単体テストは検証用の一時テストであり、テストコード正本ディレクトリ（`<画面ディレクトリ>/テスト設計/テストコード/単体/`）への保存を禁止する。単体テストコード正本の生産者は rebuilding-screen-unit-from-docs のみである（一本化）。自作した場合は、一時テストである旨を検証記録に明記する。

1画面内で完結する外部振る舞いのテストコードは、`<screen_dir>/../テスト設計/画面結合テスト設計書.md` の観点とケースから自作する。複数画面・機能・APIをまたぐ結合テストは画面単位配下から導出せず、プロジェクト全体の `docs/test-cases/結合テスト仕様書.md` を第一ソースとする。

**E2E テストの作成とベースライン実測（本スキルの確定責務）**: E2E（`RT-` / `SM-` / `IT-` / `CMP-` 接頭辞）テストの作成と、元コード（オリジナルコード環境）でのベースライン実測は本スキル（`mode=implement`）が担う。操作シナリオ仕様書・画面結合テスト設計書・設計書 frontmatter の `scenarios` に基づいて E2E テストを作成し、オリジナルコード環境に対して実行してベースライン結果を検証記録に残す（実測はオリジナル環境の起動・操作のみで行い、オリジナルコードの Read は行わない。カンニング禁止は維持される）。作成とベースライン実測の完了可否は Phase 6 で返す `compare_request.scenarios_ready` に反映し、両環境突合としての実行は管理者（orchestrating-ai-development-setup）が dynamic 検査（`scenarios` 引数）として担う。

テストコードのファイル名・配置先は `規約/命名規約.md`・`ディレクトリ構成規約.md` に従う。テストが全て Red（失敗）であることを確認してから Step 3 へ進む。
...
- **規約準拠**: `規約/` 配下の 4 ファイルに記録されたスタイル・命名・配置・設計パターンに従う
...
静的3分類、env_check全項目、Playwright L1〜L5、`hint`、`status`を含む。
L5は`operations`を持つ画面だけを評価する。

起動引数screen_dirも受け取り、Phase 9の検証記録保存先に用いる。
`reverse_worktree`と`freeze_commit`も受け取り、Phase 9の凍結検証に用いる。

設計書frontmatterの`scenarios`は、管理者側の比較実行時に反映済みとする。
対象キーは`path`、`ready`、`query`、`path_params`、`assert`、`mask`である。
比較の自前実装は禁止する。

`query`と`path_params`は、実測済みの場合だけ存在する任意キーである。
未開通画面での省略だけを`DESIGN-INCOMPLETE`の理由にしてはならない。
`DESIGN-INCOMPLETE`の場合は、まず必須の`path`と`ready`の欠落または不整合へ帰着させる。
実測URLの証跡が存在する場合に限り、`query`と`path_params`の不足をPhase 8の修正指示へ含める。

`DYNAMIC-UNVERIFIED`の場合は、静的一致だけでPASS扱いにしない。
Phase 9の最終報告には、動的未検証のため往復検証が未完了である旨を明記する。

**PASS/FAIL 判定は `compare_result.status` をそのまま転用しない**。管理者から渡される比較結果ブロックは計測事実（`static_diff` / `dynamic` / `env_check`）であり、往復検証の PASS/FAIL の意味解釈は本スキル（`mode=judge`）が単独で担う（契約正本 orchestrating-ai-development-setup の `references/contract.md` の解釈責務の規定に従う）。本スキルは Phase 3 の白紙化 + Phase 4 のカンニング禁止実装により設計書だけから独立に書き直す用途のため、変数名・コメント差による実差分は原理的に非ゼロになる。よって mode=judge は `compare_result.static_diff`（実差分行）を PASS/FAIL の判定材料に使わず、`compare_result.env_check`（全項目）と `compare_result.dynamic`（render-ready 到達・内容一致〔L2'〕・L2 ARIA・L3 画素・L4 コンソール・該当時 L5）のみで独自に PASS/FAIL を判定する。判定式: `env_check` 全通過 ∧ 全 scenario で render-ready 到達 ∧ L2' 内容一致 ∧ L2 一致 ∧ L4 一致 ∧ L3 閾値内 ∧（`operations` を持つ scenario がある画面は L5 一致も必須）。`env_check` が正式13項目チェックリスト（正本: `syncing-reverse-env` の `guide.html` の env_check 全項目）の完全実施でない場合（簡略実施・未実施を含む）は、上記判定式を満たしていても `DYNAMIC-UNVERIFIED` として扱い、PASS 判定にしない。`compare_result.status` が `PASS` ならこの判定式は自動的に満たされる。`status` が `FAIL` でも `static_diff` のみが原因で上記判定式を満たす場合は、本スキルとしては PASS 相当として扱う。`static_diff` は Phase 8 で NG の直接的な根拠にせず、動的差異の原因調査を補助する参考情報としてのみ扱う。
...
汎用的な失敗クラスでは、テンプレート改善提案と`references/test-item-patterns.md`への台帳追記も作る。

`DESIGN-INCOMPLETE`の場合は、frontmatterの`scenarios`へ帰着させる。
必須の`path`と`ready`の追加または修正を指示する。
`query`と`path_params`は実測URLの証跡がある場合だけ修正対象に含め、未開通による省略は受け入れる。
...
- 管理者から渡される比較結果ブロックは計測事実（`static_diff` / `dynamic` / `env_check`）であり、往復検証の PASS/FAIL の意味解釈は judge（本スキル `mode=judge`）が単独で担う。解釈責務の規定は契約正本（orchestrating-ai-development-setup の `references/contract.md`）に従い、`compare_result.status` を直接転用せず Phase 7 に記載の判定式（`env_check`/`dynamic` のみ）で判定すること
...
- `references/guide.html` — スキルガイド（単一ファイル自己完結）
- `<reverse_docs_root>/.claude/skills/orchestrating-ai-development-setup/references/contract.md` — 完全仲介方式の契約正本。本スキルはこの契約に準拠し、`mode` を含む args 全量指定で単独起動できる
- 起動引数で受け取る資産の移設先（管理者が args で配布）:
  - `audit_script_path`: `<reverse_docs_root>/generation-engine/scripts/audit-consistency.sh`
  - `template_root`: `<reverse_docs_root>/delivery-payload/templates/リバース検証`（画面詳細設計書テンプレート〔章の役割キー 16 種、既定 §1〜§16〕・プロジェクト共通規約〔コーディング規約・命名規約・ディレクトリ構成規約・コンポーネント設計規約〕を含む。Phase 1 でロード、Phase 4 で準拠）
  - `chapter_map_path`: `<reverse_docs_root>/delivery-payload/references/chapter-map.md`（章の役割キー→§対応表）
