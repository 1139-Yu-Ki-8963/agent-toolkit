...
**E2E テストの作成とベースライン実測（本スキルの確定責務）**: E2E（`RT-` / `SM-` / `IT-` / `CMP-` 接頭辞）テストの作成と、元コード（オリジナルコード環境）でのベースライン実測は本スキル（`mode=implement`）が担う。操作シナリオ仕様書・結合テスト観点表・設計書 frontmatter の `scenarios` に基づいて E2E テストを作成し、オリジナルコード環境に対して実行してベースライン結果を検証記録に残す（実測はオリジナル環境の起動・操作のみで行い、オリジナルコードの Read は行わない。カンニング禁止は維持される）。作成とベースライン実測の完了可否は Phase 6 で返す `compare_request.scenarios_ready` に反映し、両環境突合としての実行は管理者（orchestrating-ai-development-setup）が dynamic 検査（`scenarios` 引数）として担う。
