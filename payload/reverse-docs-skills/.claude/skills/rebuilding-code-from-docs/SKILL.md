...
**後方互換の例外**: `saved_test_paths` が空または未提供の画面（基準未確立/往復未検証 開始でファイル単位検証をスキップした画面等、上流の単体テストが存在しない場合）に限り、単体テスト観点表（`画面/詳細設計/単体テスト観点表.md`）の各観点キーから直接テストケースを導出して自作する。この場合も単体テストの正本は本来上流が持つべきものであることを検証記録に明記する。
