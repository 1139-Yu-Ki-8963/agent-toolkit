...
- 画面テスト設計書.md（1画面内の操作・表示・領域間連携・API連携・状態切替の観点とケース。必須）
...
1画面内で完結する外部振る舞いのテストコードは、`<screen_dir>/../テスト設計/画面テスト設計書.md` の観点とケースから自作する。複数画面・機能・APIをまたぐ結合テストは画面単位配下から導出せず、プロジェクト全体の `docs/test-cases/結合テスト仕様書.md` を第一ソースとする。

**E2E テストの作成とベースライン実測（本スキルの確定責務）**: E2E（`RT-` / `SM-` / `IT-` / `CMP-` 接頭辞）テストの作成と、元コード（オリジナルコード環境）でのベースライン実測は本スキル（`mode=implement`）が担う。操作シナリオ仕様書・画面テスト設計書・設計書 frontmatter の `scenarios` に基づいて E2E テストを作成し、オリジナルコード環境に対して実行してベースライン結果を検証記録に残す（実測はオリジナル環境の起動・操作のみで行い、オリジナルコードの Read は行わない。カンニング禁止は維持される）。作成とベースライン実測の完了可否は Phase 6 で返す `compare_request.scenarios_ready` に反映し、両環境突合としての実行は管理者（orchestrating-ai-development-setup）が dynamic 検査（`scenarios` 引数）として担う。
