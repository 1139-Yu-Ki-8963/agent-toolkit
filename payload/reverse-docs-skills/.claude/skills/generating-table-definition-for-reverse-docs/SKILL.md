...
テーブル一覧が作成済みで、テーブルごとの列や制約を書き起こしたいとき。

## いつ使わないか

テーブルの業務的な意味を書く論理データモデルを作るとき、テーブル同士の関係を示す図を作るとき、テーブル一覧そのものを作るとき。

# 正本: reverse-docs-skills

# テーブル定義書の生成スキル

工程全体は orchestrating-ai-development-setup が案内する。本スキルはテーブル 1 件ごとの定義書の執筆のみを担い、単独起動できる。起動引数は table_manifest_path・source_dir・output_dir・template_root の 4 つで、これらを渡せば動く。`unit_kind` は **table 固定** であり、引数では受け取らない。
...
| 項目 | 値 |
|---|---|
| 出力フォルダ | `<output_dir>/<tableUnitRoot>/table-<テーブル識別子>/詳細設計/` |
| 出力ファイル | `テーブル定義書.md` |
...
- **Step 2**: カラムの名前・型・長さ・必須の別・既定値・制約・索引・外部キーの参照先を原本から抽出する。各項目に `file:line` 形式の根拠を付ける。根拠を付けられない項目は空欄のままとし、1-199で定める根拠を記録する資料へ回す。完了条件: 全カラムの記入材料が根拠付きで揃っている
- **Step 3**: マニフェストの `foreignKeys`・`columnCount`・`mainColumns` と、原本から読み取った内容の整合を確認する。食い違いがあれば1-199で定める根拠を記録する資料へ記録する。完了条件: マニフェストとの整合確認が済んでいる
...
```bash
bash generation-engine/scripts/scaffold-design-unit.sh table detail <output_dir> <識別子> <表示名> <template_root>
```
...
- **Step 1**: `<template_root>/リバース検証/テーブル/テーブル定義書.md` を Read する。読み込んだ内容を `<output_dir>/<tableUnitRoot>/table-<テーブル識別子>/詳細設計/テーブル定義書.md` へ書き出す。`<テーブル識別子>` の決め方は「成果物」節の規約に従う。中間ディレクトリが無ければ作成する。完了条件: 全対象ユニットのファイルが配置済み
...
実装契約として書く項目の定義は `delivery-payload/references/実装契約定義.md` を正とする。テーブルには基本 3 項目のみを書き、種別固有の追加項目は持たない（同文書の判断による）。

- **7.1 ファイルと関数**: 当該テーブルの読み書きを担うファイルと関数を記す（ORM のモデル定義・リポジトリ層等）。§6 データ定義のカラム構造とは重複させない
- **7.2 型定義**: §6.1 カラム定義のうち、原本コードに明示的な型定義（ORM のモデルクラス等）がある場合だけ記す。型定義自体が無い実装では空欄のままとし、1-199で定める根拠を記録する資料には回さない（型定義が無いこと自体は確定できなかった事項ではない）
- **7.3 依存**: マイグレーションファイル・ORM ライブラリ・他の構成要素への参照を記す。他の構成要素と共有する実装がある場合は、同文書の「構成要素をまたぐ実装の扱い」に従い、定義元の設計書パスを根拠として記し、値は再掲しない
...
- **検査5 配置の検査**: `bash generation-engine/scripts/scaffold-design-unit.sh --verify table detail <output_dir> <識別子>` を実行し、必須ファイルの存在・トークンの残存なし・章の完備・配置先の妥当性を確認する
...
| status | 意味 |
|---|---|
| `DONE` | 全対象ユニットのテーブル定義書が記入済み、かつ Phase 4 の 5 検査すべてに合格した |
...
| **Goal** | 対象テーブルごとに、根拠付きのカラム定義と設計書と、確定できなかった事項を持つ根拠記録資料が生成されている |
