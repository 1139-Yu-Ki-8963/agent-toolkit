...
description: |
  テーブル定義書を原本の schema から生成する（unit_kind=table 専用）。
...
```bash
bash shared/scripts/scaffold-design-unit.sh table detail <output_dir> <識別子> <表示名> <template_root>
```
...
- **配置の検査**: `bash shared/scripts/scaffold-design-unit.sh --verify table detail <output_dir> <識別子>` を実行し、必須ファイルの存在・トークンの残存なし・章の完備・配置先の妥当性を確認する
