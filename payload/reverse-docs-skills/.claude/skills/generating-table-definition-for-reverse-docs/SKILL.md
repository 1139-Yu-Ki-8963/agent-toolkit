...
- **Step 2**: カラムの名前・型・長さ・必須の別・既定値・制約・索引・外部キーの参照先を原本から抽出する。対象コードのファイルパス・行番号は記入材料として保持しない。確定できない項目は空欄のまま要確認事項一覧へ回す。完了条件: 全カラムの記入材料が揃っている
...
- **7.2 型定義**: §6.1 カラム定義のうち、原本コードに明示的な型定義（ORM のモデルクラス等）がある場合だけ記す。型定義自体が無い実装では空欄のままとし、要確認事項一覧には回さない（型定義が無いこと自体は確定できなかった事項ではない）
...
| **Goal** | 対象テーブルごとに、コード位置を含まないカラム定義を持つ詳細設計書が生成され、確定できなかった事項が要確認事項一覧へ集約されている |
