...
出力先は `<docs_root>/環境構築手順.html` に固定する（`build-portal.sh` の `FUTURE_FILES` と同値）。
...
- **Step 1** — HTML 生成スクリプトを実行する。完了条件: `<docs_root>/環境構築手順.html` が生成済み
...
| Phase 4 | `<docs_root>/環境構築手順.html` が生成され、指定時は `build-portal.sh` の再実行が完了している |
...
- 対象リポジトリへの書き込み・変更は一切行わない。出力は `docs_root` 配下の環境構築手順.html のみ
