...
| 入力 | 必須/任意 | 取得方法 |
|---|---|---|
| raw画面正本 | 条件付き必須 | `<output_dir>/<manifestsRoot>/screen-manifest.json`を直接入力にする（`manifestsRoot`はoutput-layoutの物理配置キー。既定値`docs/manifests`）。`<output_dir>/<unitListAbsentMd>`（`unitListAbsentMd` は output-layout の物理配置キー。`{label}` に「画面」を代入。既定値 `docs/manifests/画面一覧（該当なし）.md`）が実在する対象では不要とする。どちらも不在ならERRORで停止する。画面一覧HTMLからの逆抽出は旧成果物の移行・復元時だけとする |
...
画面一覧が該当なしの対象（バックエンドのみのリポジトリ等）では、手がかり①のルートprefixが存在しない。この場合に限り、優先度を次のとおり読み替える。

| 優先 | 読み替え後の手がかり | 用途 |
|---|---|---|
| ① の代替 | APIプレフィックス / tags（通常の③） | 大分類境界の決定 |
| ② の代替 | ビルド定義の生成ターゲット・実行ファイル単位 | APIプレフィックスが引けない場合の境界。非 HTTP 系ではこれが主手段 |
| ③ の代替 | ディレクトリ構造（通常の④） | 補助裏取り |

読み替えは上から順に試す。APIプレフィックスで境界が引けるならそこで確定し、引けない場合にビルド定義の生成ターゲットへ移る。どちらか一方が必ず存在するとは限らず、両方とも引けない場合はディレクトリ構造で仮置きして confidence を low とする。

読み替えを適用した場合、その事実を `strategy.boundarySource` に記録する。値は `route-prefix`（通常）・`api-prefix`・`build-target`・`directory` のいずれかとする。

読み替え時の confidence は、異なる2種以上の手がかりが同一の割当を支持する場合に限り high とし、単一の手がかりのみなら medium とする。ディレクトリ構造だけで割り当てた場合は low とする。

入力に画面が1件も無い場合、全機能の `relatedScreens` は空配列になる。これは欠陥ではなく、画面を持たない対象の正常な姿である。参照実在の検査では、`strategy.screenPresence` が `none` の対象について `relatedScreens` の照合を対象外とする。
...
```json
{
  ...
  "strategy": {
    ...
    "unitIdRegex": "^[a-z0-9-]+$",
    "approvedByUser": true,
    "inputManifests": ["<output_dir>/<manifestsRoot>/screen-manifest.json"],
