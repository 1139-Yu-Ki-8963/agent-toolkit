...
| 画面不在-該当なし通過 | `<output_dir>/<unitListAbsentMd>`（`unitListAbsentMd` は output-layout の物理配置キー。`{label}` に「画面」を代入。既定値 `docs/manifests/画面一覧（該当なし）.md`）が実在する対象で、raw画面正本が無い | Phase 1〜6を実行する | raw画面正本が無くても status=DONE で完了し `strategy.screenPresence` が `none` になる | 手動（対象コードベースを要する） |
