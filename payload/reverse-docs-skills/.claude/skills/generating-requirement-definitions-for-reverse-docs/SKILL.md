...
対象コード・原本ファイルパス・関数名・変数名は読まない。各文書が満たすべき要件の業務上の目的・想定利用者・利用頻度は業務の意図である。業務の意図はこの2つの情報源のどちらにも現れない。そのため常に 1-199で定める根拠を記録する資料へ回す。埋まらない欄を推測で埋めない。一般的な内容で補うことも行わない。
...
## Step 3-3: 1-199で定める根拠を記録する資料の記入
...
- Step 4: 記録形式は1-199で定める様式に従う。完了条件: 全キーが意味語で一意である

**完了**: 対象文書ごとに 1-199で定める根拠を記録する資料が記入済みである。行数はユニット数以上（機能・帳票・バッチ・外部連携要件）、または必須3行＋対象外種別（ビジネス概要）である
...
| **Goal** | 「入力なし」以外の対象文書が、業務語彙のみで観測事実と1-199で定める根拠を記録する資料を分離した状態で `<output_dir>/<commonRoot>/` に確定している |
