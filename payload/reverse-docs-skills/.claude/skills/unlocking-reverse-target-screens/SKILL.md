...
完了条件: API一覧（画面本体分＋共有ゲート部品分・オペレーション数20超の場合は初期表示最小集合分）・実名対応表・URLパラメータ/ストア値（充足元の二分類付き）/セッションキー（充足タイミング付き）の一覧が記録済み。複数画面IDを共有する場合はID固有分岐の洗い出しも完了済み。同一シンボルを複数箇所から呼び出す場合は、呼び出し箇所単位のマウント条件記録も完了済み
...
| Phase 1 | API一覧（画面本体＋共有ゲート部品分・大型複合画面は初期表示最小集合分）・実名対応表・URLパラメータ/ストア値（充足元の二分類付き）/セッションキー（充足タイミング付き）一覧が記録済み。複数画面ID共有時はID固有分岐の洗い出しも完了済み。同一シンボルを複数箇所から呼び出す場合は呼び出し箇所単位のマウント条件記録も完了済み |
