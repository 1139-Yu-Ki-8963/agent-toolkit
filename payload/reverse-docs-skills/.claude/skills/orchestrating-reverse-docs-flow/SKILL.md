...
| S0u 画面未開通 | 画面一覧HTML有・画面が未開通（設計書も基準タグも無い新規画面） | unlocking-reverse-target-screens→syncing-reverse-env(registry) |
...
**(a) S0u 画面未開通の場合**: 先に Skill で unlocking-reverse-target-screens を system・screen_id・reverse_worktree・ports・docs_root で起動する。返却 status=UNLOCKED を受けたら、画面レジストリ（`~/agent-home/state/reverse-screen-registry.yml`）へ source_ref・verification_url・design_doc_path を記帳し、続けて Skill で syncing-reverse-env を mode=registry・system・screen_id・reverse_worktree・ports・user-approved で起動して基準タグ確立まで進める（PASS なら画面レジストリの該当エントリを status=baseline-established に更新）。status=BLOCKED / ERROR の場合は hint を確認しユーザーに報告する。
...
| Phase 4（S0u・画面未開通時） | unlocking-reverse-target-screens | system, screen_id, reverse_worktree, ports, docs_root | UNLOCKED |
| Phase 4（S0u・開通後） | syncing-reverse-env | mode=registry, system, screen_id, reverse_worktree, ports, user-approved | PASS |
...
- S0u で画面が未開通の場合、unlocking-reverse-target-screens → syncing-reverse-env(registry) の順で進む。unlocking自体は他スキルを呼ばず、記帳と registry モード起動は管理者が担う
