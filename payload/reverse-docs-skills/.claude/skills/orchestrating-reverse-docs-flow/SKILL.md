...
各エージェントには target_repo_path・docs_root・template_root・survey_doc_path を args として渡す。

**完了**: 6 スキルすべてが完了し、各種別の一覧HTMLが生成されている。失敗したスキルがある場合はエラー内容を報告し、残りの正常完了分で続行するか判断する。

#### Step 1.5-2: excluded-kinds.json を確認する

6 一覧の生成結果を確認し、対象外種別（該当ゼロ件で一覧が空の種別）を excluded-kinds.json に記録する。
...
preflight でアーキテクチャ調査書・画面一覧HTML・プロジェクト共通10文書・画面開通状態・facts封印・画面基本設計書・設計書/対象ファイル・著者スキルの完全性ゲート成果物・当該ファイルの検証記録・⑤setup返却の baseline_tag・⑨judge の status の実在を、上表の順（決定木）で確認し、11状態のいずれかを確定する。確定した状態から必要な工程だけを 1 つずつ TaskCreate する（一括登録禁止）。工程開始時は該当タスクを TaskUpdate で in_progress にし、完了時に completed へ更新する。

完了条件: 状態キー（11状態のいずれか）が確定し、次に起動すべき子スキルが1つ定まっている
...
**(b-2) 基本設計未著述の場合**: 画面は開通済み・facts は封印済みだが画面基本設計書（`<screen_dir>/基本設計/画面基本設計書.md`）が未作成の場合、Skill で generating-reverse-basic-design を screen_dir・docs_root・template_root・scaffold_script_path・facts_ref（(b) で取得、または既に封印済みなら流用）・common_docs_root（Phase 4 で取得）・unit_kind で起動する。返却 status=基本設計著述完了 を受けたら (c) へ進む。status=基本設計著述失敗 の場合は hint を確認しユーザーに報告する。

**(c) 設計書未著述の場合**: 画面は開通済み・facts は封印済みだが対象ファイルの契約が設計書に無い場合、Skill で ⑦generating-reverse-detailed-design を screen_dir・docs_root・template_root・chapter_map_path・audit_script_path・scaffold_script_path・facts_ref（(b) で取得、または既に封印済みなら流用）・common_docs_root（Phase 4 で取得）・mode・target_file_path で起動する。画面ディレクトリのテンプレート展開（雛形生成）はこの起動時に scaffold-screen.sh の実行として行われる。返却 status=AUTHORED を受けたら (d) へ進む。status=BLOCKED の場合は hint を確認しユーザーに報告する。

**(d) ファイル単位未検証の場合**: 著述済みの対象ファイルについて、Skill で ⑧rebuilding-screen-unit-from-docs を screen_dir・target_file_path・docs_root/template_root/audit_script_path/scaffold_script_path/chapter_map_path（資産paths）・env_block・user-approved で起動する。白紙化を伴うため、起動前にユーザーから承認を取得し user-approved として args に含める（管理者が事前確認し、子スキルはユーザーに直接聞かない）。対象ファイル1件ごとに繰り返し、status=再現一致 まで進める。status=差し戻し（設計書に対象契約なし等の内容起因）の場合は (c) 設計書未著述（generating-reverse-detailed-design）へ戻す。status=再現不一致 の場合は instruction_doc を確認しユーザーに報告する。target_repo_path 等の必須引数欠落による起動不可は status=差し戻し を返さない Skill 呼び出し失敗として扱われるため、設計書未著述への差し戻しではなく管理者自身の args 供給を見直す（このループの対象外）。
...
| Phase 1 | 状態キー（11状態のいずれか）が確定し、次に起動すべき子スキルが1つ定まっている |
...
| **Goal** | 全対象画面が status=PASS で基準タグ確立、または NG分類済み修正指示書が保存されている。かつ、最終報告に全6種別の到達状態レポート（生成済み / 対象外 / 後続未対応 の3値）が含まれている |
