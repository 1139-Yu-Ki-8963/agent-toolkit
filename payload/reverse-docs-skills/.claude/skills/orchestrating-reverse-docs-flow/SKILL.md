...
| 一覧未生成 | unit_kinds_present のいずれかの種別について一覧HTMLが不在、または excluded-kinds.json が不在 | generating-unit-list-for-reverse-docs |
...
4. 不在の種別ごとに Skill で generating-unit-list-for-reverse-docs を `source_dir`・`output_dir`・`unit_kind=<種別>` で起動する
...
| Phase 3（一覧未生成時） | generating-unit-list-for-reverse-docs | source_dir, output_dir, unit_kind | DONE |
