...
| S1 設計書不足 | 画面一覧HTML有・画面未開通 or 設計書/対象ファイルが不在（任意工程） | (未開通なら)unlocking-reverse-target-screens→syncing-reverse-env(registry) / rebuilding-screen-unit-from-docs |
...
状態が S1 の場合に実行する。対象画面がまだ開通していない（設計書も基準タグも無い新規画面）場合は、先に Skill で unlocking-reverse-target-screens を system・screen_id・reverse_worktree・ports・docs_root で起動する。返却 status=UNLOCKED を受けたら、画面レジストリ（`~/agent-home/state/reverse-screen-registry.yml`）へ source_ref・verification_url・design_doc_path を記帳し、続けて Skill で syncing-reverse-env を mode=registry・system・screen_id・reverse_worktree・ports・user-approved で起動して基準タグ確立まで進める（PASS なら画面レジストリの該当エントリを status=baseline-established に更新）。status=BLOCKED / ERROR の場合は hint を確認しユーザーに報告する。

画面が既に開通済みの場合は、from-zero 対象画面についてのみ以下を実行する。Skill で rebuilding-screen-unit-from-docs を screen_dir・target_file_path・docs_root/template_root/audit_script_path/chapter_map_path（資産paths）・env_block・user-approved で起動する。白紙化を伴うため、起動前にユーザーから承認を取得し user-approved として args に含める（管理者が事前確認し、子スキルはユーザーに直接聞かない）。対象ファイル1件ごとに繰り返し、status=CONVERGED まで進める。DIVERGED / INTERNAL-CONTRADICTION / BLOCKED の場合は instruction_doc を確認しユーザーに報告する。
...
| Phase 4（画面未開通時） | unlocking-reverse-target-screens | system, screen_id, reverse_worktree, ports, docs_root | UNLOCKED |
| Phase 4（開通後） | syncing-reverse-env | mode=registry, system, screen_id, reverse_worktree, ports, user-approved | PASS |
...
- S1 で画面が未開通の場合、unlocking-reverse-target-screens → syncing-reverse-env(registry) → rebuilding-screen-unit-from-docs の順で進む。unlocking自体は他スキルを呼ばず、記帳と registry モード起動は管理者が担う
