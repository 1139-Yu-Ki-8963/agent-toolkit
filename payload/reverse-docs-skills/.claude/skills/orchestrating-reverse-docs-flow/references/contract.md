...
`verification_mode=docs-only` は、standard では基本設計著述完了+AUTHORED の検収直後（global Step 28）に成立する。large-two-passではDETAIL_AUTHORED+基本設計著述完了+COMPANION_AUTHOREDの検収直後に成立する。この場合、verification_url の有無を問わず「静的リバース完了」として必ず終端する。12〜16 および global Step 29〜30 の動的工程は判定・起動のいずれも行わない。
