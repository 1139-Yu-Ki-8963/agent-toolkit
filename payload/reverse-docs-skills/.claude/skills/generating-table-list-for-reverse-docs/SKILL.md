...
```json
[{"failure_return_to":"orchestrating-reverse-docs-flow","id":"table-list","inputs":["schema source"],"outputs":["一覧/テーブル一覧/テーブル一覧.html"],"stop_conditions":["0件"],"validator":"check-delivery-artifacts.sh"}]
```
