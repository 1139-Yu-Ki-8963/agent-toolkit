...
`マスタ` / `トランザクション` / `関連` 等の業務上の分類は、`kind`（上表の技術的な種類）ではなく任意フィールド `businessClass` へ記録する。スキーマ定義は `delivery-payload/references/manifest-schema-extensions.md` の「tables（テーブル）」節を参照する。
