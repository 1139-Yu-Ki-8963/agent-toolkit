...
表同士のつながりを表す図を作りたいとき、表の関係図のページを追加したいとき。

## いつ使わないか

表の一覧そのものを作るとき（→ generating-table-list-for-reverse-docs を使う）、他の種別の詳細ページを作るとき。

# 正本: reverse-docs-skills

# ER図生成スキル

工程全体は orchestrating-ai-development-setup が案内する。本スキルはポータルの将来ページ受け口のうち ER図（T4）のみを担い、単独起動できる（起動引数を渡せば動く）。
...
- **Step 1** — `<output_dir>/<unitListHtml>`（`unitListHtml` は output-layout の物理配置キーで {label} は「テーブル」。正本レイアウト。不在時のみ後方互換で `<output_dir>/テーブル一覧/テーブル一覧.html`）の実在を確認する。あわせて `<script type="application/json" id="unit-manifest">` の埋め込みも確認する。不在ならハード停止する。この場合 `generating-table-list-for-reverse-docs` の先行実行を案内して終了する。完了条件: manifest の実在確認済み、または不在を報告して停止している
...
- **Step 1** — HTML 生成スクリプトを実行する。完了条件: `<output_dir>/ER図.html` が生成済み
  ...
  ```
  ../../../generation-engine/scripts/detail-pages/build-detail-page.sh <page-data.json> <output_dir> --page er
  ```
...
- 出力先は `<output_dir>/project-portal/diagrams` 直下（`ER図` のような種別専用フォルダは作らない）。`build-detail-page.sh` の `--page er` 固定出力名仕様に従う
...
`../../../delivery-payload/references/完了報告の書き方.md` の作業報告型に従う。固有差分として「検証」テーブルに `validate-page-data.sh` の PASS/FAIL 行と孤児関連の確認結果を追加する。
