...
| 項目 | 値 |
|---|---|
| 出力フォルダ | `<output_dir>/テーブル/table-<テーブル識別子>/基本設計/` |
| 出力ファイル | `論理データモデル.md` |
| テンプレート | `<template_root>/リバース検証/テーブル/論理データモデル.md` |
...
| status | 意味 |
|---|---|
| `DONE` | 全対象ユニットの論理データモデルが業務語彙のみで記入済み、かつ Phase 4 の 4 検査すべてに合格した |
...
| **Goal** | 対象テーブルごとに、業務語彙のみで書かれた論理データモデルと要確認事項一覧が生成されている |
