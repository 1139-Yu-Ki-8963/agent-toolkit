...
orchestrating-ai-development-setup の基盤ページがまだ無い状態から起動されたとき、アイコンカタログを作りたいと言われたとき。

## いつ使わないか

対象のコードにアイコンの使用箇所が一件も無いとき。
...
工程全体は orchestrating-ai-development-setup が案内する。本スキルはポータルの基盤ページ受け口のうちアイコンカタログ（`pageKind: icon-catalog`）のみを担い、単独起動できる（起動引数を渡せば動く）。
...
出力先は `<output_dir>/project-portal/foundation/アイコンカタログ.html` に固定する。
...
| スクリプト | パス（スキルフォルダ基点） |
|---|---|
| アイコン抽出 | `../../../generation-engine/scripts/extract/extract-icon-usage.sh` |
...
**完了**: アイコン参照の 1 件以上の実在確認済み、または 0 件を報告して停止している

## Phase 2: アイコン参照抽出（機械実行）

## Step 2-1: アイコン参照抽出（機械実行）
...
- **Step 1** — 抽出スクリプトを実行する。完了条件: page-data.json が生成済み
  ...
  ```
  ../../../generation-engine/scripts/extract/extract-icon-usage.sh <target_repo_path> <page-data.json>
  ```
...
- **Step 1** — HTML 生成スクリプトを実行する。完了条件: `<output_dir>/アイコンカタログ.html` が生成済み
  ...
  ```
  ../../../generation-engine/scripts/detail-pages/build-detail-page.sh <page-data.json> <output_dir> --page icon-catalog --portal-dir <output_dir>/project-portal
  ```
...
**完了**: `<output_dir>/アイコンカタログ.html` が生成され、指定時は `build-portal.sh` の再実行が完了している
