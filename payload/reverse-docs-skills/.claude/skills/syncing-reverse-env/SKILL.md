...
- `git worktree add` / `remove` / `prune` を実行する際は、リポジトリ親ディレクトリの `.reverse-worktree-ops.lock` を `flock -w 120` で保持して直列化する（契約書 `.claude/skills/orchestrating-reverse-docs-flow/references/contract.md` の「共有資産の排他」節と同一の値。最大120秒待機。worktree の作成・削除はファイル排他より時間を要するため待機上限を長く取る）
