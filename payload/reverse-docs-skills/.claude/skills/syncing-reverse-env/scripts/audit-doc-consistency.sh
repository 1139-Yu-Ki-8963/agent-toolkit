...
#
# 用途:
#   確定仕様の正本 syncing-reverse-env の guide.html と、SKILL.md・
...
if [ "${1:-}" = "--self-test" ]; then
  self_test() {
    ...
    cp "$self_path" "$tmp/good/scripts/audit-doc-consistency.sh"
    {
      ...
      echo "</pre>"
      echo "</body></html>"
    } > "$tmp/good/references/guide.html"
...
GUIDE="${SKILL_DIR}/references/guide.html"
...
preflight_keys="$(
  LC_ALL=C awk '/<h3>プリフライト/{flag=1} flag{print} flag && /<\/table>/{exit}' "$GUIDE" \
...
env_keys="$(
  ...
  LC_ALL=C awk '/<h3>[^<]*環境同一性チェック/{flag=1} flag{print} flag && /<\/table>/{exit}' "$GUIDE" \
    | grep -oE '<td>[0-9]+</td><td>[a-z0-9-]+</td>' \
    | sed -E 's#<td>[0-9]+</td><td>([a-z0-9-]+)</td>#\1#'
)"
