...
| 項目 | 基本設計書 | 単体テスト設計書 |
|---|---|---|
| 出力フォルダ | `<output_dir>/帳票/report-<帳票識別子>/基本設計/` | `<output_dir>/帳票/report-<帳票識別子>/基本設計/` |
| 出力ファイル | `帳票基本設計書.md` | `帳票単体テスト設計書.md` |
| テンプレート | `<template_root>/リバース検証/帳票/帳票基本設計書.md` | `<template_root>/リバース検証/帳票/帳票単体テスト設計書.md` |
...
```bash
bash shared/scripts/scaffold-design-unit.sh report basic <output_dir> <識別子> <表示名> <template_root>
```
...
- **配置の検査**: `bash shared/scripts/scaffold-design-unit.sh --verify report basic <output_dir> <識別子>` を実行し、必須ファイルの存在・トークンの残存なし・章の完備・配置先の妥当性を確認する
...
| status | 意味 |
|---|---|
| `DONE` | 全対象ユニットの帳票基本設計書・帳票単体テスト設計書が業務語彙のみで記入済み、かつ Phase 5 の 6 検査すべてに合格した |
...
| **Goal** | 対象帳票ごとに、業務語彙のみで書かれた基本設計書・単体テスト設計書と要確認事項一覧が生成されている |
