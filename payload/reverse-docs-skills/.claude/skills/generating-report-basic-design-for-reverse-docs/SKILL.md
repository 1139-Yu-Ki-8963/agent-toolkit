...
2. 帳票テスト設計書は、基本設計書の外部仕様・業務仕様・エラーと例外の各章からテストの観点を作り、各観点に由来する基本設計書の章を記録する。基本設計書で確定していない事項からは観点を作らず、要確認事項一覧へ移す
...
| **Goal** | 対象帳票ごとに、業務語彙のみで書かれた基本設計書、出力契約を検証する帳票テスト設計書、詳細設計書または入力資料の関数・メソッドを検証する帳票単体テスト設計書と、要確認事項一覧が生成されている |
