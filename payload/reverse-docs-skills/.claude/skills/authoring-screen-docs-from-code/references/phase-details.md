...
4. 対象リポジトリの原本コードは一切 Read しない。facts.yml に事実の欠落・矛盾を発見した場合も自ら原本で確認せず、extracting-unit-facts-from-code への差し戻し事由として hint に記録する。
...
§15.2 が facts.yml の export_type「型定義なし・リテラル推論型」に基づく根拠付き該当なし文の場合も、audit_script_path はそのまま exit 0 を返す（検査gの型名抽出は無マッチを許容する実装）。型を捏造して検査を通すことは禁止事項1（facts に無い事実の創作）違反であり、絶対に行わない。exit 1 は常に実違反として扱い、出力された「違反:」行に従って設計書を修正して再実行する。
