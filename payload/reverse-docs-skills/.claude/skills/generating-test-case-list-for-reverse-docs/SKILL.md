...
- 1単位以上でテスト設計書（または画面固有の操作シナリオ仕様書）が確定済みで、ポータルにテストケース一覧カードを追加したいとき
...
- **Step 1** — output-layout の各 `*UnitRoot` を解決し、設計単位直下の `テスト設計/` にある2設計書と、画面固有の操作シナリオ仕様書を走査する。画面は新配置を優先し、役割ごとに新文書がない場合だけ旧 `テスト項目書/` の対応文書をfallbackとして数える。全種別で有効入力が0件ならハード停止する。完了条件: 1件以上の実在確認済み、または不在を報告して停止している
