...
| タブ切替なし-単一テーブル | 単体と結合と操作シナリオが混在するケース一覧がある | build-unit-list.shでHTMLを生成する | タブ構成を持たず単一テーブルでテスト種別列により区別する | 手動 |
...
- 機械検証列「手動」は、検証状況（references/generating-test-case-list-for-reverse-docs-guide.html）へ確認結果を残す
