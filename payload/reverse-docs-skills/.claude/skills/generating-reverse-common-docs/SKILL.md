...
基盤設計.md §3（デプロイ構成）はCI/CD設定・デプロイスクリプトから読み取れる範囲に限る。基盤設計.md §7（共通関数の契約）は、2 つ以上の設計単位（API・バッチ・帳票・外部連携）から呼ばれる同一プロジェクト内の関数を対象に、引数・戻り値・呼び出し順序・副作用・例外を 1 関数 1 行で記録する（契約の唯一の正。各設計単位の詳細設計書は再掲せず §12.4 依存で参照する。改善課題1-275）。共通の関数が実在しない場合は「実在しない（理由: …）」で埋める。基盤設計.md §7（共通関数の契約）は、2 つ以上の設計単位（API・バッチ・帳票・外部連携）から呼ばれる同一プロジェクト内の関数を対象に、引数・戻り値・呼び出し順序・副作用・例外を 1 関数 1 行で記録する。各設計単位の詳細設計書はこの節を再掲せず §12.4 依存で参照する。共通の関数が実在しない場合は「実在しない（理由: …）」で埋める。データ設計.md §1（データモデル概要）はORMのスキーマ定義・型定義ファイル等から抽出する。データ設計.md §6（状態遷移表）のキーは連番禁止（内容要約キー規約）とし、状態・契機は業務語彙で書く。対象コードの位置は本文にも別資料にも記録しない。
...
## 対象外の宣言と画面が対象外である根拠の書き方

- 本文で「対象外」と宣言した項目（機械が判定に使う識別子など）を、同じ文書の後続の見出し・箇条書きへ「参考として」再掲しない。宣言と再掲が同居すると、対象外の意味が読み手に伝わらない。`scripts/check-common-docs.sh` の検査9が、宣言の行の逆引用符で囲んだ識別子が後続の見出し・箇条書きへ現れた場合を不合格にする（改善課題1-271）
- 見出しの記法は全ての設計文書と同じ「§＋番号」（例: `## §9 応答の外枠`）に揃える。「番号＋句点」（例: `## 9. 応答の外枠`）で書かない。版面の節番号の解決は「§＋番号」を前提とし、句点の記法は節番号の対応表に登録されず本文の「§」付きの参照が解決できない。`scripts/check-common-docs.sh` の検査11がこの記法を不合格にする（改善課題1-298）
- 画面が対象外である根拠（画面のマニフェスト件数・除外種別の宣言・設計書フォルダの構成などの実測値と出典）は、判定の記録である除外種別の宣言ファイル（`output-layout.json` の `excludedKinds` が指す `excluded-kinds.json`）だけに書く。UI共通設計・デザインの決まりなど画面に依存する文書は、この記録への参照だけを持ち、根拠の実測値・出典を自文書へ書き直さない。検査10が、画面と対象外を同じ行で述べながら記録への参照を持たない行を不合格にする（改善課題1-272）
