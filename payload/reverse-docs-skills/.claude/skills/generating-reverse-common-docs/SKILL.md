...
- テンプレ残存検査（検査4）は11ファイルすべて（規約4種＋共通設計書.md＋メッセージ定義書.md＋DESIGN.md＋基盤設計.md＋UI共通設計.md＋データ設計.md＋サンプル記録.md）を走査する。テンプレの固定文にも検出正規表現（`<実測`/`<FILL`/`TBD`/`TODO`）に一致する語を書いてはならない。`TBD`/`TODO`は開き括弧付きの雛形記法だけでなく地の文への裸の出現も検出対象である（`TODO 記法`のような規約説明の見出し語も含む）ため、コーディング規約等でこれらの語自体を説明する場合は、別の平易な言い換え（例:「未実装マーカー」「保留印」）を使うか、コード例をbacktick無しの引用として書く（1-153: 姉妹スキル`surveying-architecture-for-reverse-docs`側は開き括弧付きの形・行/セル全体一致の形に限定する改修を実施済みだが、本スキルは同種の既存自己テストフィクスチャ（地の文埋め込みの裸`TBD`を検出する陰性ケース）と両立できないため未改修。既存フィクスチャを変更しない制約下での既知の非対称）
