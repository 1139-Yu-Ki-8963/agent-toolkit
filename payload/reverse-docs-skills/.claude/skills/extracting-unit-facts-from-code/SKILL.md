...
対象の画面が持つ元のコードから、後で設計書を書く工程が使う事実を新しく取り出したいとき。検証で事実の取り出し方に不備が見つかり、取り出し方を直したうえで取り出しをやり直したいとき。

## いつ使わないか

詳細設計書を書く作業（→generating-reverse-detailed-design が担当）、共通文書を書き起こす作業（→generating-reverse-common-docs が担当）を行うとき。

# 正本: reverse-docs-skills

# ユニット事実抽出スキル

工程全体は orchestrating-ai-development-setup が案内する。本スキルは対象ユニットの原本コードから宣言的契約 facts を抽出し、独立再計数・確定・再現性検証を通して「執筆工程が原本を読まずに済む」品質の `facts.yml` を確定するところまでを単独で担い、単独起動できる（起動引数を渡せば動く）。後続工程（詳細設計執筆）はこの facts を前提として動くが、本スキル自体は他スキルへの依存を持たない。
...
- 判定（Phase 7 / Step 41）が NG帰着(b)（事実抽出プロファイルが対象コードの挙動を捕捉できていない）と判定し、抽出プロファイルを改訂した上で再抽出したいとき
...
| screen_dir | 必須 | 対象ユニットの論理IDを得る画面ディレクトリ絶対パス。facts抽出時点で不在でもよく、雛形の展開の完了条件にはしない |
...
列挙結果は `orchestrating-ai-development-setup` の契約（`references/contract.md` の「画面完了の定義」）における対象ファイル集合の網羅判定の入力になる。対象ファイルの一部のみを指定する場合（部分スコープ実行）は、`run_id` または facts.yml のメタ情報に対象ファイル数（n件/全m件）を記録し、完全性を欠く旨を明示する。
...
- **確定による改ざん検知**: facts.yml を正規化ハッシュで確定し、以降の改変を機械検知できる状態にする
- **再現性の担保**: 同一 args での抽出結果は（run_id を除き）決定的に一致することを diff で確認する。検証は原本コードに対する独立した2回目の抽出で行い、1回目の facts.yml の複製・コピーを2回目として扱う比較は再現性検証の要件を満たさない
...
`bash <shared>/scripts/seal-facts.sh seal <facts_dir>` を実行し `facts.lock` を生成する。続けて `bash <shared>/scripts/seal-facts.sh verify <facts_dir>` を実行し、確定直後の整合性を確認する。
...
同じ args で、対象ユニットの原本コードに対する**独立した2回目の抽出**（Phase 2〜4 相当。ただし確定は任意）を、`mktemp -d "${TMPDIR:-/tmp}/XXXXXX"` 形式で作成した一時ディレクトリに実行する。「独立した」とは、1回目の facts.yml を読む・コピーする・参照することなく、原本コードだけから再度抽出し直すことを指す。1回目の出力を複製・正規化して2回目として比較に用いることは、再現性検証の要件を満たさない（自身との比較になり非決定性を検出できないため）。両方の `facts.yml` を `seal-facts.sh normalize` でそれぞれ一時ファイルへ書き出し、`diff` で比較する（プロセス置換 `<(...)` はサンドボックス環境で `/dev/fd` アクセスが権限拒否される場合があるため使わない。一時ファイル経由の比較に固定する）。diffが空なら通過。diffに差分がある場合は以下の診断ステップで分類する:
...
返却ブロックを出力する。`確定済み` の場合は `artifacts` に facts.yml・facts.lock・recount-report.txt の絶対パスを、`facts_ref` に facts ディレクトリの絶対パスと facts.lock の sha256 を、`pending_measurements` に ⑨実測委譲キーの一覧を記す。`中断` の場合は hint に Phase 3 で未解消の検査項目・Phase 5 の非決定箇所・非対応プロファイル等、中断理由を記す。
...
| Phase 5 | 独立した2回の抽出の正規化出力の diff が空 |
| Phase 6 | `status` 確定（`封印済み` \| `中断` \| `共通文書帰着`） |
| **Goal** | 対象ユニットの原本コードから抽出したfactsが独立再計数・確定・再現性検証のすべてを通過し、執筆工程が原本を読まずに済む品質で `<verification_dir>/screen-<画面ID>/facts/<run_id>/` 配下に確定していること |
...
- `recount-facts.sh` の各検査はfacts.yml全体を走査する。`references/profile-screen.md`・`delivery-payload/references/facts-schema.md` の記入例（`evidence: "src/screens/Foo/Foo.tsx:1"` 等）はあくまで様式説明であり、Phase 2 実行者がそのままコピーして実データとして残さないよう注意する（コピー由来の evidence は孤児参照や乖離超過の原因になる）
...
- **乖離が「粒度差」ではなく「対象コードに実在する構文パターンをrecount-facts.shの正規表現が構造的に検知できないこと」に起因する場合**（記載件数に関わらず再計数が常に0のまま等）、items を reason へ逃がして回避することは禁止する（`delivery-payload/references/facts-schema.md` の「該当なし」は実在しない事実専用であり、検知漏れの回避手段ではない）。この場合は `references/profile-screen.md` の該当パターンと `scripts/recount-facts.sh` の対応する count_* 関数を実在の構文に合わせて拡張し、再計数させてから収束させる。既知の構造的盲点（Promiseチェーン形式のAPI呼出し・複数行に折り返したJSX開始タグ・カスタムフックの分割代入による状態変数・オブジェクトリテラル定数のフィールド分解漏れ（宣言行1件のみに丸められる）・条件分岐（早期return・三項演算子）が生成する複数レンダリングパスの外殻ラッパー未書き起こし）は既に対応済み。新たな盲点を発見した場合は `references/profile-screen.md` の「再計数パターンの既知の限界」に追記した上でパターンを拡張する
...
- 確定記録（`facts.lock`）は正規化規則の版（`NORMALIZE_VERSION`）を持ち、`seal-facts.sh verify`はまず版の一致を確認する。版が異なる場合は改竄ではなく規則変更として`exit 3`で報告され、再確定を促すメッセージが出る（改竄検知の`exit 1`とは区別される）。旧版の確定に遭遇した場合は、facts.ymlの内容を確認したうえで`seal-facts.sh seal <facts_dir>`で再確定する。過去に確定された記録の一括移行は行わない。遭遇した時点で個別に再確定する
...
固有の検証行:
- facts.yml の確定検証（seal-facts.sh verify）が exit 0・再現性検証の diff 空
...
**必要性**: 確定した facts.yml が後続工程（詳細設計執筆）に渡るまでの間に意図せず改変される事故を防ぐには、確定時点の内容を機械的に記録し、以降いつでも1コマンドで整合性を検証できる仕組みが要る。また Phase 5 の再現性検証（同一argsでの2回の抽出結果比較）は run_id・タイムスタンプ・空白の揺れを正規化してから比較する必要があり、この正規化ロジック（`normalize`）は確定（`seal`/`verify`）と再現性検証の両方が同一の実装を共有しなければ判定基準がズレる。`generating-reverse-detailed-design` が後続で facts.yml の確定検証を再利用する設計（Phase 5詳細のリバース工程設計.mdの位置づけ）のため `generation-engine/scripts/` に配置する。

**代替案を採用しなかった理由**:
- Bashツール直叩き: sha256計算・normalize・確定記録の照合を都度手書きすると、確定（seal）と検証（verify）と正規化（normalize）で異なるロジックを使ってしまう危険があり、改ざん検知の信頼性が保証できない
...
**保守責任者**: 人手（ユーザー）。facts.ymlのフィールド構成・正規化規則（`delivery-payload/references/facts-schema.md`）を変更した時に同時更新する。

**廃棄条件**: facts.ymlのスキーマが廃止された時、または確定による改ざん検知の仕組みそのものが不要になった時。
...
- `delivery-payload/references/facts-schema.md` — facts.ymlのスキーマ正本（12分類・必須フィールド・孤児参照定義・normalize規則）
- `generation-engine/scripts/seal-facts.sh` — facts.ymlの確定・検証・正規化を担う共有スクリプト
- `delivery-payload/references/リバース工程設計.md` — Phase/Step×スキル対応の正本（本スキルの位置づけ: Phase 5 ユニット反復 / Step 18-21）。NG帰着3系統の(b)facts欠落からの差し戻し先でもある
