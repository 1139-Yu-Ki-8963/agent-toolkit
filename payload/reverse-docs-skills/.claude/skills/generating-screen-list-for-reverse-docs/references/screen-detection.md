...
組み込み検出器 `../../../generation-engine/scripts/unit-list/detect-screens.sh` が対応する（高速パス）。組み込み検出器がPhase 1の調査結果と適合しない場合はカスタム抽出パスを使用する。
...
| parentScreen | string/null（任意） | 親画面の screenKey（モーダル/ポップアップの場合） |
| childComponents | array（任意） | 子コンポーネント。`[{screenKey, componentType: "modal|popup|iframe"}]` |
