...
- **Step 1**: `../../../shared/scripts/unit-list/build-unit-list.sh <manifest.json> <output-dir>/一覧/画面一覧/画面一覧.html --unit-kind screen --portal-dir <output-dir>` を実行する。内部で `build-screen-list.sh` に委譲される。`--portal-dir` にはポータル（`index.html`）の配置先＝納品物ルート（output_dir=docs_root）を渡し、「ポータルへ戻る」リンクを実在パスに解決させる。build側が内部でvalidateを再実行するため、検証を経ないmanifestからは生成できない。完了条件: HTML生成済み
