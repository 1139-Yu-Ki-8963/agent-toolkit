...
- **Step 2**: FAIL時はraw作業コピーまたはcanonical rawを指摘に応じて修正する（entryFile不在は`--fix`でunresolved降格可）。rawを修正した場合はStep 2-4へ戻り、hash再計算とext再生成を行ってからStep 1を再実行する。extだけを直接修正しない。3回失敗したら抽出方式の再検討（Phase 2 Step 1）へ差し戻す。完了条件: exit 0
