...
| 出力ファイル | `バッチ詳細設計書.md`・`バッチ実装記録.md`（今の作りの記録。改善課題1-288） |
...
- **Step 1**: `<template_root>/リバース検証/バッチ/バッチ詳細設計書.md` を Read する。読み込んだ内容を `<output_dir>/<batchUnitRoot>/batch-<バッチ識別子>/<unitPhaseDirNames.detail>/バッチ詳細設計書.md` へ書き出す。`<バッチ識別子>` の決め方は「成果物」節の規約に従う。中間ディレクトリが無ければ作成する。完了条件: 全対象ユニットのファイルが配置済み 同じ手順で `<template_root>/リバース検証/バッチ/バッチ実装記録.md` を同じ置き場の `バッチ実装記録.md` へ配置し、今の作りの節（疑似コード・データ定義・実装契約。APIはデータアクセスも）はこちらへ記入する。実装記録は作り直す際に引き継がない情報だけを置き、仕様の節は詳細設計書に残す（改善課題1-288）。
