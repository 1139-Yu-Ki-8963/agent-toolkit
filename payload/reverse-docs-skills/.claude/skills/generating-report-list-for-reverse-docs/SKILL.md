...
```json
[{"failure_return_to":"orchestrating-reverse-docs-flow","id":"report-list","inputs":["report source"],"outputs":["一覧/帳票一覧/帳票一覧.html"],"stop_conditions":["0件"],"validator":"check-delivery-artifacts.sh"}]
```
