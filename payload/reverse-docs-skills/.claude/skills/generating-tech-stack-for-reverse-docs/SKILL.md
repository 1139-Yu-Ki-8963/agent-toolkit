...
出力先は `<output_dir>/技術スタック.html` に固定する（`build-portal.sh` の `FUTURE_FILES` と同値）。
...
- **Step 1** — HTML 生成スクリプトを実行する。完了条件: `<output_dir>/技術スタック.html` が生成済み
  ...
  ```
  ../../../shared/scripts/detail-pages/build-detail-page.sh <page-data.json> <output_dir> --page techstack
  ```
...
| Phase 2 | 「実在する」群の全項目一致を確認し、「実在しない」群を根拠付きで absentRows[] へ分離して page-data.json を保存済み、または乖離を報告して停止している |
| Phase 3 | `validate-page-data.sh --target-repo` が全項目 PASS |
| Phase 4 | `<output_dir>/技術スタック.html` が生成され、指定時は `build-portal.sh` の再実行が完了している |
| **Goal** | 調査書と定義ファイルの実測値が完全一致する項目は rows[] へ、実在しないと判定された項目は根拠パスを保持したまま absentRows[] へ、それぞれ捏造せず技術スタック.html に残り、乖離があれば停止報告されている |
...
- 対象リポジトリへの書き込み・変更は一切行わない。出力は `output_dir` 配下の技術スタック.html のみ

## 予想を裏切る挙動

- 出力先は `<output_dir>` 直下（`テーブル一覧.html` のような種別専用フォルダは作らない）。`build-detail-page.sh` の `--page techstack` 固定出力名仕様に従う
