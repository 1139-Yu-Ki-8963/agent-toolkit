...
| 出力先-種別専用フォルダなし | 技術スタック.htmlを生成する | Phase 4 Step 1を実行する | output_dir直下に固定出力名で生成する | 手動 |

## 機械検証との対応

- 機械検証列「手動」は、検証状況（references/generating-tech-stack-for-reverse-docs-guide.html）へ確認結果を残す
