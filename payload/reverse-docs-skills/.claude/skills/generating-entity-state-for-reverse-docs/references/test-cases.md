...
| portal未反映-カード受け口未配線 | portal_output_dirを指定してbuild-portal.shを再実行する | Phase 4 Step 2を実行する | entity-stateのカード受け口が未配線のためカードが増えないことがある | 手動 |

## 機械検証との対応

- 機械検証が「手動」の行は、generating-entity-state-for-reverse-docs-guide.html の検証状況へ手動確認を記録する
