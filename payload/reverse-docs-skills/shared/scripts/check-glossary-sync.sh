...
# check-glossary-sync.sh — docs/用語集.md とガイドHTML用語パネルの同期を検査する
#
# 使い方:
...
#         という見出しを持つ section）から抽出した用語が docs/用語集.md に登録されているか。
...
if [ "${1:-}" = "--self-test" ]; then
  ...
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/check-glossary-sync-self-test.XXXXXX")"
  trap 'rm -rf "$tmp"' EXIT
  SELF="${BASH_SOURCE[0]}"
  ...
  make_guide() {
    ...
    cat > "${dir}/.claude/skills/${skill}/references/${skill}-guide.html" <<HTML
<!DOCTYPE html>
...
GLOSSARY="${REPO_ROOT}/docs/用語集.md"
if [ ! -f "$GLOSSARY" ]; then
