...
#
# 入力JSONスキーマ(契約。shared/references/規約定義と派生生成の設計.md 4節の
...
#       "scope": "always|scoped", "globs": ["glob"],
...
# 出力は常にbyte一致する(決定的生成)。
#
# 出力: shared/templates/rule-proposal/rule-proposal-template.html を土台に
...
TEMPLATE="$SCRIPT_DIR/../../templates/rule-proposal/rule-proposal-template.html"
VERIFY_SCRIPT="$HOME/agent-home/skills/reviewing-explanatory-html/scripts/verify-html-static.mjs"
...
render_category() {
  ...
  if is_judgable_state "$state"; then
    local scope globs_json enforcement checkable check_method proposed_rule sources_json summary
    scope="$(jq -r '.scope' <<<"$cat_json")"
    globs_json="$(jq -c '.globs' <<<"$cat_json")"
    ...
    printf '      <div class="category" id="cat-%s" data-report-row="%s" data-chapter-index="%s" data-chapter-name="%s" data-cat-name="%s" data-summary="%s" data-parent="%s" data-key="%s" data-scope="%s" data-globs=\x27%s\x27 data-enforcement="%s" data-checkable="%s" data-check-method="%s" data-proposed-rule="%s" data-sources=\x27%s\x27>\n' \
      "$key" "$key" "$chapter_index" "$chapter_name_esc" "$title_esc" "$(html_escape "$summary")" "$parent_slug" "$key" \
      "$scope" "$(html_escape "$globs_json")" "$enforcement" "$checkable" \
...
self_test() {
  local script_path="$0"
  local tmp rc=0
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/build-rule-proposal-self-test.XXXXXX")"
  ...
  jq -n '{
    ...
    chapters: [{
      ...
      categories: [
        {key: "cat-a", title: "カテゴリA", state: "proposal", summary: "カテゴリAの要約文。", observations: ["<code>a.ts:1</code> の観測1件。"], proposedRule: "規約A案", sources: ["a.ts:1"], scope: "scoped", globs: ["a/**"], enforcement: "advisory", checkable: true, checkMethod: "検査方法A", reason: ""},
        {key: "cat-b", title: "カテゴリB", state: "na", summary: "", observations: [], proposedRule: "", sources: [], scope: "always", globs: [], enforcement: "advisory", checkable: false, checkMethod: "", reason: "対象外の理由B"},
        {key: "cat-c", title: "カテゴリC", state: "common", summary: "", observations: [], proposedRule: "", sources: [], scope: "always", globs: [], enforcement: "advisory", checkable: false, checkMethod: "", reason: "共通規約を参照する理由C"}
      ]
    }]
  }' > "$data_min"
