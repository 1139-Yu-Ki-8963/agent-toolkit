...
const builder = path.join(root, 'shared/scripts/detail-pages/build-detail-page.sh');
const validator = path.join(root, 'shared/scripts/detail-pages/validate-page-data.sh');
const projector = path.join(root, 'shared/scripts/detail-pages/project-semantic-glossary.py');
const glossaryFixtures = path.join(root, 'shared/scripts/glossary/fixtures');
const canonicalRegistry = path.join(glossaryFixtures, 'canonical-registry');
const template = path.join(root, 'shared/templates/detail-pages/detail-t2-dictionary.html');
const sampleFixture = path.join(root, 'shared/scripts/detail-pages/fixtures/semantic-glossary-sample-page-data.json');
const sampleRegenerator = path.join(root, 'shared/scripts/detail-pages/regenerate-semantic-glossary-sample.sh');
const sampleHtml = path.join(root, 'shared/samples/一覧/用語辞書/用語辞書.html');
const glossaryPython = path.join(root, 'shared/scripts/glossary/.venv/bin/python');
...
try {
  ...
  assert.match(source, /TOP<\/a> ／ 一覧・設計図 ／/);
  ...
  assert.equal(fs.existsSync(path.join(root, 'shared/samples/用語辞書.html')), false, 'obsolete root-level glossary page must not exist');
