...
self_test() {
  ...
  build_baseline() {
    ...
    rm -rf "$dir"
    mkdir -p "$dir/一覧/画面一覧" \
             "$dir/画面/screen-alpha/基本設計" "$dir/画面/screen-alpha/詳細設計" \
             "$dir/画面/screen-beta/基本設計" "$dir/画面/screen-beta/詳細設計"
    ...
    printf '%s' "$manifest_json" | jq -c . > "$dir/一覧/画面一覧/screen-manifest.json"
...
EOF
    ...
    write_screen_page "$dir/画面/screen-alpha/基本設計/画面基本設計書.html" "アルファ画面" "画面基本設計書" " · コミット番号: aaaaaaa"
    write_screen_page "$dir/画面/screen-alpha/詳細設計/画面詳細設計書.html" "アルファ画面" "画面詳細設計書" " · コミット番号: aaaaaaa"
    write_screen_page "$dir/画面/screen-beta/基本設計/画面基本設計書.html" "ベータ画面" "画面基本設計書" " · コミット番号: aaaaaaa"
    write_screen_page "$dir/画面/screen-beta/詳細設計/画面詳細設計書.html" "ベータ画面" "画面詳細設計書" " · コミット番号: aaaaaaa"

    cat > "$dir/画面/screen-alpha/詳細設計/画面詳細設計書.md" <<EOF
---
doc_id: screen-alpha
...
# アルファ画面 画面詳細設計書
EOF
    cat > "$dir/画面/screen-beta/詳細設計/画面詳細設計書.md" <<EOF
---
doc_id: screen-beta
type: screen-detail-design
target_screen: ベータ画面
...
EOF
  ...
  build_baseline "$t/c2"
  jq '.screens[0].screenNameGuess = "アルファ画面2"' "$t/c2/一覧/画面一覧/screen-manifest.json" > "$t/c2/tmp.json" \
    && mv "$t/c2/tmp.json" "$t/c2/一覧/画面一覧/screen-manifest.json"
  ...
  build_baseline "$t/c3"
  sed -i.bak 's/アルファ画面 画面基本設計書/アルファ画面X 画面基本設計書/' "$t/c3/画面/screen-alpha/基本設計/画面基本設計書.html"
  rm -f "$t/c3/画面/screen-alpha/基本設計/画面基本設計書.html.bak"
  ...
  build_baseline "$t/c4"
  sed -i.bak 's/コミット番号: aaaaaaa/コミット番号: bbbbbbb/' "$t/c4/画面/screen-alpha/詳細設計/画面詳細設計書.html"
  rm -f "$t/c4/画面/screen-alpha/詳細設計/画面詳細設計書.html.bak"
  ...
  build_baseline "$t/c5"
  sed -i.bak 's#src/pages/Alpha.tsx:42#src/pages/Alpha.tsx:999#' "$t/c5/画面/screen-alpha/基本設計/画面基本設計書.html"
  rm -f "$t/c5/画面/screen-alpha/基本設計/画面基本設計書.html.bak"
  ...
  build_baseline "$t/c6"
  rm -f "$t/c6/一覧/画面一覧/screen-manifest.json"
