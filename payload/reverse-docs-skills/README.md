...
対象プロジェクトへ「AI駆動開発の基盤一式」を納品するツール群の正本リポジトリ。区分はセットアップ・リバース・保守（納品物の維持）・現場運用（納品先の開発サイクル）の4つ。リバースでは既存コードから設計書を書き起こし、その設計書だけからコードを再生成し、原本と一致するまで機械で照合する（往復検証）。
...
|---|---|---|---|
| API | `API基本設計書` | `API詳細設計書` | `API結合テスト設計書`・`API単体テスト設計書` |
| テーブル | `論理データモデル` | `テーブル定義書` | `テーブル結合テスト設計書`・`テーブル単体テスト設計書` |
| バッチ | `バッチ基本設計書` | `バッチ詳細設計書` | `バッチ結合テスト設計書`・`バッチ単体テスト設計書` |
| 帳票 | `帳票基本設計書` | `帳票詳細設計書` | `帳票結合テスト設計書`・`帳票単体テスト設計書` |
| 外部連携 | `外部連携基本設計書` | `外部連携詳細設計書` | `外部連携結合テスト設計書`・`外部連携単体テスト設計書` |
| 機能 | `機能設計書` | なし | `機能結合テスト設計書`・`機能単体テスト設計書` |
...
外部の検証で繰り返し指摘され、現時点で完全には解決していない事項を [未解決事項.md](generation-engine/scripts/verification/未解決事項.md) にまとめている。現象・現在の実測・なぜ解決していないか・解決の条件を項目ごとに記載する。資料参照切れの検査など、機械検査の常設によって解消済みの事項もあわせて記載する。
