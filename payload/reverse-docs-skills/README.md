...
事前ヒアリングで対象パス・出力先・画面範囲を確定後、以下の状態機械で進行する。Phase 1B では 6 一覧を Agent 並列実行し、Phase 4C では画面数 4 件以上の場合に running-reverse-screen-batch で無人バッチ実行する。`docs-only` はバッチでも静的リバースだけで終端する。基本設計と詳細設計は facts 封印後に並列著述する。
