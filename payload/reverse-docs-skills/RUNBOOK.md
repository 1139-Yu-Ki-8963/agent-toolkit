...
本節の禁止事項は、配布先で本スキル群を無人モードで走行させる実行側（実行環境）に向けたものである。このリポジトリ自身（reverse-docs-skills）を保守・改善する側には及ばない。改善側が従うべき規約は、リポジトリ直下の `CLAUDE.md` が定めるグローバル規約（配布対象外）である。
...
orchestrating-ai-development-setup が Skill ツールで呼び出す agent-home スキル。グローバル規約（配布対象外）が定める main-未反映検査（コミット直後に自動発火する PostToolUse hook）で公開先リポジトリの同期定義への登録有無を検査する。
...
Skill ツールを介さず、agent-home 配下のスクリプトを直接実行する依存。同期定義への登録の対象ではなく、実行環境にそのスクリプトが在ることが前提になる。固定の絶対パスは本体に持たず、環境変数で指定する（このリポジトリ自身の自立の判定（配布対象外の検査）が、固定パスの直書きを実行マシン固有の依存として検出するため）。
...
- 一時領域を作れない環境で落ちる検査（`tests/check-portal-catalog.sh`・`unit-list/detect-screens.sh`）。これらは引数なしの `mktemp` を使っており、書き込みを拒む環境では失敗する
