...
| [`managing-routines`](skills/managing-routines/SKILL.md) | クラウドルーティンのライフサイクル（作成・レビュー・テスト・クラウド登録） |

## 設計仕様（人間用）

| ドキュメント | 内容 |
|---|---|
| [`docs/config-placement-design.html`](docs/config-placement-design.html) | 設定層 配置判定ガイド — 「どこに書くか」を決定木で判定する横断ガイド。7 層の使い分けを一枚で参照 |
| [`docs/claude-md-design.html`](docs/claude-md-design.html) | CLAUDE.md 設計ガイド — ロードタイミング・サイズ制約・書く/書かない判定フロー |
| [`docs/rules-design.html`](docs/rules-design.html) | Rules 設計ガイド — eager/lazy 注入戦略・カテゴリ分類・hook script 同居原則 |
| [`docs/skill-design.html`](docs/skill-design.html) | Skill 設計ガイド — 定義 / 判定 / 分類 / 起動モデル / 命名 / description 規律 / ツール選択 / フォルダ構造 |
| [`docs/hooks-design.html`](docs/hooks-design.html) | Hooks 設計ガイド — 基本原則 / 禁止配置 / 配置決定 2 軸 / 4 象限 / 機械強制 / 命名・設計判断 |
| [`docs/subagent-design.html`](docs/subagent-design.html) | Subagent 設計ガイド — 4 役割アーキテクチャ・frontmatter 仕様・references 構造・品質基準 |
| [`docs/loop-design.html`](docs/loop-design.html) | Loop 設計ガイド — 繰り返し実行の設計原則・5 アクション・6 パーツ・評価役の分離 |
...
│   ├── config-placement-design.html
│   ├── claude-md-design.html
│   ├── rules-design.html
│   ├── skill-design.html
│   ├── hooks-design.html
│   ├── subagent-design.html
│   ├── loop-design.html
│   └── style.css
└── skills/
    ├── managing-skills/
    ...
    └── managing-routines/
        ├── SKILL.md
        └── references/
            ├── conventions.md       # ルーティン共通規約
            ├── creating.md
            ├── reviewing.md
            ├── testing.md
            └── cloud-operations.md  # クラウド登録・運用手順
```
