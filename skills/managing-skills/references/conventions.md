...
| フィールド | 制限 | 説明 |
|-----------|------|------|
| `name` | kebab-case、最大 64 文字 | スキルの一意識別子 |
...
| slug | 日本語 | 一文定義 | 典型例 |
|---|---|---|---|
| `orchestration` | フロー本体型 | Phase/Step で順序を強制し、複数下位スキルを束ねる | flow-feature, auto-ship, pr-review-workflow, **managing-skills** |
| `gateway` | 入口型 | フロー本体の前段で素材を確定し、確定後にフローへ橋渡し | preparing-manual-mockup, picking-issues, parallel-dev-worktree |
| `gate` | 関門型 | フロー内 Phase から呼ばれ go/no-go を返す | reviewing-design-doc, reviewing-impl-quality, verifying-issue-scope |
| `audit` | 静的監査型 | 既存物の読み取り中心の点検レポート | （hooks 系は `managing-hooks` の review モードに統合） |
| `verification` | 実機検証型 | 実機で動かして観察 | test-e2e（hooks 系は `managing-hooks` の test モードに統合） |
...
| `action` | 単発操作型 | 1 アクションで副作用ありの完了 | grouping-commits, registering-replacements, dev-launch |
