...
description: |
  エージェント構成 6 種の作成・レビュー・検証ハブ。
  TRIGGER when: スキル/ルール/ルーティン/エージェント/フック作成・レビュー・テスト・診断・発火検証、/schedule・CronCreate 登録、ワークフロー作成。
  SKIP: 命名のみ（→naming-conventions）、単独スキル追加のみ、settings.json の編集・権限管理（→update-config）。
...
スキル・hooks・rules・ルーティン・サブエージェント・ワークフロー文書という 6 種のアセットについて、**作成 → 静的レビュー → 実機検証** を 1 つの動線で担うオーケストレーター。旧 `managing-skills` / `managing-hooks` / `managing-rules` / `managing-routines` / `managing-subagents` / `documenting-workflows` を統合し、対象種別を判定した上で種別ごとの `references/<type>/` を必要時のみロードする。

## 設計思想

- **作りっぱなしを許さない**: create したら review → test まで自動連鎖する（workflows 種別を除く）
...
- **6 種の統合であって画一化ではない**: 種別ごとに配置先・観点・完了条件は異なる。共通化するのは「フロー構造」であり「内容」ではない
...
|---|---|---|
| スキル・SKILL.md・create a skill | `skills` | `~/.claude/skills/<name>/SKILL.md` |
...
| ルーティン・routine・/schedule・CronCreate | `routines` | `<project>/routines/<name>/` |
| サブエージェント・agent・`~/.claude/agents/` | `subagents` | `~/.claude/agents/<name>/<name>.md` |
| ワークフロー・スキル連携図・どう使い分ける | `workflows` | `<project>/skills/workflows/<name>.md` |

判定が曖昧な場合はユーザーに確認する。誤判定のまま進めると誤った `references/<type>/` をロードしてしまう。

## モード判定

種別が決まったら、動詞・キーワードからモードを判定する。`workflows` のみ create 単発（連鎖なし）で、他 5 種は create → review → test の自動連鎖を持つ。

| 動詞・キーワード | モード | ロードする references | 自動連鎖 |
|---|---|---|---|
| 作る・追加・設計・改善・新規・create | **create** | `references/<type>/conventions.md` + `references/<type>/creating.md` | → review → test（workflows は連鎖なし） |
...
4. **外部正本の参照**（種別による）: `hooks` は `docs/hooks-design.html`、`rules` は `docs/rules-design.html`、`subagents` は `docs/subagent-design.html` も併せて参照する
...
6. **ポータル更新**（本リポジトリの `docs-portal/` が存在する場合のみ実行。存在しなければスキップ）: `docs-portal/index.html` の該当サマリ・対応表にエントリを追加・更新する
7. `workflows` 種別: ここで完了。review / test には連鎖しない
8. `workflows` 以外: **自動連鎖** で review モードへ、review 完了後さらに test モードへ

連鎖をスキップしたい場合は `AskUserQuestion` で「ここで終了する／レビューまでで止める／テストまで連鎖する」を確認する。デフォルトは **テストまで連鎖**（`workflows` を除く）。
...
- 対象種別（skills / hooks / rules / routines / subagents / workflows）・対象アセット名・通過モード
...
- 種別判定を誤ると誤った `references/<type>/` をロードしてしまう。ユーザー発話の対象語（スキル/フック/ルール/ルーティン/エージェント/ワークフロー）を直接見る
- test モードは **必ず新規サブエージェント**（`routines` のみ例外でメインセッションが `ScheduleWakeup` で直接回す）。セルフ再読は禁止
- create 後の自動連鎖は既定 ON（`workflows` を除く）。OFF にするのはユーザーが明示的に止めた時のみ
- `workflows` 種別は review / test に連鎖しない単発モード。作成後チェックリスト（`references/workflows/workflow-documentation.md` 末尾）で完了を確認する
- references/ のロードは「必要になってから」。ハブ本体だけで判断できる時は Read しない（トークン節約）
- 統合前の `managing-skills` / `managing-hooks` / `managing-rules` / `managing-routines` / `managing-subagents` / `documenting-workflows` を呼ぶ箇所が外部にある場合は本スキルへ参照置換すること
...
- `references/workflows/workflow-documentation.md` — ワークフロー文書の作成手順・テンプレート・Mermaid 記法リファレンス・作成後チェックリスト
