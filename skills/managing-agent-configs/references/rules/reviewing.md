...
| # | チェック | 重要度 | 検出方法 |
|---|---|---|---|
| B1 | グローバル rule が特定プロジェクト固有の概念に依存していないか | CRITICAL | rule.md 内に自プロジェクト固有の内部語彙（独自の worktree/セッション概念・内部ツール名・社内ポータル名等）を grep |
...
- [CRITICAL] B1 `role-boundary-rules/` がグローバルに配置されているが
  自プロジェクト固有の内部概念（独自の worktree/セッション語彙等）に依存
