...
# プロジェクト（ホーム配下の主要 repo）
find ~/Projects -maxdepth 5 -type f \
...
設計面の 5 観点に絞って評価する。`~/.claude/skills/<name>/SKILL.md`・`~/.claude/rules/<name>-rules/rule.md` と `~/.claude/skills/<name>/scripts/`・`~/.claude/rules/<name>-rules/` 配下の hook script も読み込む。
...
#### H. 配置アーキ準拠
- [CRITICAL] H1 PostToolUse (line 87) — command path が `<project>/tools/hooks/` flat バケット
