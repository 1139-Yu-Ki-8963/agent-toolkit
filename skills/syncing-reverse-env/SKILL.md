...
description: |
  リバース元と設計書由来の 2 環境を同期・検証し基準コミットを確立する。
  TRIGGER when: リバース環境の整備・検証・同期・片付け、setup/sync/teardown 指定、基準タグ reverse-baseline の操作。
  SKIP: 設計書修正・リバースコード環境の中身の再構築（→リバースを行うスキル）、通常開発の worktree 作成。
...
オリジナルコード環境（リバース元・常に「正」）と、リバースコード環境（設計書だけから再構築）の 2 つの worktree を用意・名前管理し、「ポート番号以外は完全同一」であることを検証して、一致証明済みの基準コミット（基準タグ `reverse-baseline/<system>`）を確立する。

仕様の正は `references/syncing-reverse-env-spec.html`（確定仕様）。

## 使用タイミング

- リバースを行うスキル（将来作成）が Skill ツールで呼び出す。args 全量指定・**対話ゼロで完走**が契約（AskUserQuestion を発行しない）
...
| 名前 | 実体 |
|---|---|
| オリジナルコード環境 | worktree 名 `original-code-<system>`。設計書 frontmatter の `source_repo` / `source_ref` から用意する読み取り専用コピー。内容を変更しない |
| リバースコード環境 | worktree 名 `reverse-code-<system>`、ブランチ `feature/reverse-code-<system>`。容器は本スキルが用意し、中身はリバースを行うスキルが書く |
| 基準タグ | annotated tag `reverse-baseline/<system>`。基準コミットの唯一の記録。タグメッセージに検証日と判定サマリを持つ |
| `<system>` | 設計書 frontmatter の `source_repo` 末尾のリポジトリ名から機械的に導出する（`doc_id` は画面粒度のため使わない） |

固定ポート（`config.yml` が正・プロジェクト非依存。両 worktree は初回に一度だけ作成し、teardown まで使い回す）:

| 環境 | ベース | サービス別 |
|---|---|---|
| オリジナルコード環境 | 9100 | backend 9100 / frontend 9101 / docs 9102 / DB tools 9103 |
| リバースコード環境 | 9110 | backend 9110 / frontend 9111 / docs 9112 / DB tools 9113 |
...
| `teardown` | 検証終了時 | ポート帯一括 kill + worktree 削除。**ユーザーの明示依頼がある時だけ** |

sync のオプション: `dry-run`（整列・タグ更新なし）/ `reset-first`（開始前に `git reset --hard reverse-baseline/<system>` で基準状態へ復帰）。
...
入力は `design-doc`（画面基本設計書パス）のみ必須（**全 mode 共通**。teardown でも `<system>` 導出に使う）。任意: `mode` / `dry-run` / `reset-first` / `user-approved`（teardown 時のユーザー依頼発話の引用）/ `routes`（省略時は設計書 frontmatter の `route`）/ `max-loop`（既定は config.yml の値）。mode 別の必須 args 検証は Phase 1 で行い、Phase 2 以降は環境操作に徹する。
...
args を検証し、本スキルフォルダの `config.yml` を Read する。設計書 frontmatter から `<system>`・`source_repo`・`source_ref`・`routes` を導出する。
完了条件: mode・design-doc・config 値がすべて確定している

### Phase 2: 環境確保 + プリフライト

命名規則で両 worktree を検出し、無ければ `git worktree add` で本スキルが直接作成する（初回のみ。以後は再発見して使い回す。オリジナルは `--detach` で `source_ref` 固定、リバースは `-b feature/reverse-code-<system>`。配置は `source_repo` の親ディレクトリ直下。通常開発用の worktree 管理の仕組みとは命名・配置規約が異なるため委譲しない）。`source_repo` はローカルリポジトリのパスを想定し、リモート URL の場合は clone を済ませてから worktree 化する。仕様書 §5 のプリフライト 7 項目を検証し、NO-GO なら停止・報告する（#6 の依存インストール確認は sync のみ適用。setup 直後の空容器では Playwright 可用性の確認だけ行う）。

- `mode=setup`: 両環境のパス・ブランチ・確定ポートを返して終了
- `mode=teardown`: 明示依頼を確認（人間なら AskUserQuestion、スキル経由なら args の `user-approved`）できた場合のみ、固定ポート帯（9100〜9119）を一括 kill → worktree を削除して終了（基準タグとブランチは残す＝基準コミットは消えない）。確認できなければ何も削除せず status=ERROR で差し戻す
- `mode=sync`: 基準タグの有無を確認（`git tag -l` + 参照先実在。なし・参照先不明は初回扱い）。`reset-first` 指定かつタグありなら `git reset --hard reverse-baseline/<system>` してから Phase 3 へ

完了条件: 両環境が存在し、プリフライト 7 項目がすべて GO

### Phase 3: 静的比較

config.yml の `diff_exclude` を除外して全ファイルを diff し、残差分行にポート正規化判定を適用する（オリジナル側 9100 番台 ↔ リバース側 9110 番台の期待対応のみ許容。数字が違うだけでは許容しない）。結果を「一致 / ポート差のみ / 実差分」の 3 分類で集計する。
完了条件: 3 分類の集計とファイル別内訳が記録されている

### Phase 4: 整列（実差分あり かつ dry-run でない時のみ）

① リバース側のポート設定ファイル群を退避 ② オリジナル → リバースへミラーコピー（削除同期あり・除外は Phase 3 準拠。git 操作ではなく**ファイル転写**で行い、両ブランチの歴史を混ぜない） ③ 固定ベース 9110 からポート設定を再生成 ④ 依存を再インストール。
...
```bash
git tag -af "reverse-baseline/<system>" -m "<検証日> 検証PASS: 実差分0 env_check 10/10 L1-L4全一致"
```
...
機械向け返却ブロック（`status` / `mode` / `original_code` / `reverse_code` / `baseline_tag` / `static_diff` / `dynamic` / `env_check` / `artifacts` / `hint`）と、ユーザー向け定型文（確認 URL・検証内容・環境情報・再起動手順・片付け方法）の両方を出力する。書式の正は仕様書 §8。setup / teardown / ERROR の早期終了時も返却ブロックの形を保ち、未実施フィールドは「未実施」と記す。FAIL 時の `hint`（差分ファイル → 設計書章マップの対応推定）がリバースを行うスキルの修正ループ入力になる。
...
- 固定ポートのため検証環境は同時に 1 組だけ。並行させる場合のみ config.yml の `projects` 上書きで別帯域を割り当てる
...
- ポート差の許容は「固定ポートの期待値どおりの対応」のみ。数字が違うだけの行を許容すると、ポートのハードコードミスを見逃す

## 参照資料

- `config.yml` — ポート・起動コマンド・diff 除外・L3 閾値・max_loop の可変値（本スキルフォルダ直下）
- `references/syncing-reverse-env-spec.html` — 確定仕様（プリフライト 7 項目・env_check 10 項目・報告書式の正）
